...
def bf_load(dct: _JSONDict, registry: Mapping[str, Any]) -> Any:
    ...
    del schema
    # Marshmallow  `.load()` method loads the same algorithm multiple times
    # if a list of algorithms with the same fields is provided,
    # so we iterate through them and load them separately instead
    algorithms_list = []
