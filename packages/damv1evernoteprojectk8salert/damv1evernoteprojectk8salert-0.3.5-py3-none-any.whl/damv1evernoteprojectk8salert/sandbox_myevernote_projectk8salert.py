...
class sanbox():
    ...
    ## 
    def evernote_generate_report_v2(self, _contexid, _nameof_msg_rpt, _lst_grplines, _tmplt_wrapper, _tmplt_restart_deploy, _tmplt_another1 = '', **kwargs):
        ...
        try:
            note.content += _tmplt_wrapper.format(\
                ...
                value_str_restart_deploy_succeed = _tmplt_restart_deploy, \
                value_strlst_grplines = ''.join(_lst_grplines)
            )
            # - - - - - | prepared created Note
            created_note = noteStore.createNote(note)
