...
class Validator:
	def __init__(self, data: dict | list | None = None, exact_keys: bool = False):
		self.__exact_keys = exact_keys
		self.__schema = {}
		for key, val in self.__class__.__dict__.items():
			if key[0] == "_":
				continue
			if type(val) == Rule:
				self.__schema[key] = val
		if data is not None:
			self._init_validation_result = self.__call__(data)

	def __iter__(self):
		for key, val in self.__schema.items():
			yield (key, val)
	...
	def __call__(self, data, *args, **kwargs) -> tuple[bool, Any, str]:
		if self.__exact_keys:
			for key in data:
				if key not in self.__schema.keys():
					return False, ValidatorError.INVALID_SCHEMA, ""
		for key, rule in self.__schema.items():
			if key not in data:
				if rule.optional:
					continue
				else:
...
class RequestValidator(Validator):
	...
	def __call__(self, method, *args, **kwargs) -> Callable:
