...
class TransformerParams:
    ...
    def get(self):
        ...
        out: Circuit = Symbol((SEQ, HIDDEN), UUID("ece2bb5d-c6d6-4b7a-93a5-32b5ac264888"), f"t.input")
        assert "t.input" not in circs
        circs["t.input"] = out
        for i, block in enumerate(blocks):
            out = module_new_bind(block.body, (f"b.input", out), name=f"b{i}.call")
    ...
    def bind_to_input(
        ...
        if self.causal_mask:
            mask = Array(
                ...
                f"t.a.c.causal_mask",  # prefix doesn't apply to constants
            )
        else:
            mask = Scalar(1.0, shape=(seq_len, seq_len), name=f"t.a.c.full_mask")
