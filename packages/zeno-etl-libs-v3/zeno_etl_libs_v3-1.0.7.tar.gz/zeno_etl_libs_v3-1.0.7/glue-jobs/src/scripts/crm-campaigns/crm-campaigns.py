...
def data_premium_customer_program():
    ...
    data_q = f"""
            select
                ...
                ps."3m-total-spend" / 3 as "acv",
                pm."last-bill-date",
                nvl(sh."net-sales", 0) "net-sales-mtd",
               	"acv" - "net-sales-mtd" as "opportunity",
	            rank() over (partition by "store-id", ps."assign-pharmacist" order by "opportunity" desc) as "priority"
            from
...
if datetime.today().date() == datetime.today().date().replace(day=1) :
    ...
    # Mandatory steps
    # data = campaign_mandatory_steps(data, run_date, last_n_days_bill_param=0,
    #                                 last_n_days_call_param=0, exclude_store=[0])
