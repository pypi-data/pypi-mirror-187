...
class MetricVisualizer:
    ...
    def violin_plot(
        ...
        for i, metric_name in enumerate(metrics.keys()):
            ...
            # get the colors
            colors = kwargs.get("colors", "C{}".format(i))

            # get the xticks
            xticks = np.arange(len(metric.keys()))

            xtick_labels = list(metric.keys())
        ...
        # set the legend
        violins = [violin["bodies"][0] for violin in violins]
        ax.legend(violins, metric.keys())

        plt.setp(
            ax.get_xticklabels(),
            rotation=kwargs.get("rotation", 0),
            horizontalalignment=kwargs.get("horizontalalignment", "right"),
            **kwargs.get("xticklabels_kwargs", {})
        )

        ax.minorticks_on()
        ax.grid(which="major", axis="y", **kwargs.get("y_grid_kwargs", {}))
        ax.grid(which="major", axis="x", **kwargs.get("x_grid_kwargs", {}))
    ...
    def trajectory_plot(
        ...
        for i, metric_name in enumerate(metrics.keys()):
            ...
            x = np.array(
                [[i for i, label in enumerate(metric)] for _ in range(y.shape[1])]
            )
            ...
            ax.plot(
                ...
                color=kwargs.get("color", "C{}".format(i)),
            ...
            ax.minorticks_on()
            ax.grid(which="major", axis="y", **kwargs.get("grid_kwargs", {}))
            ax.grid(which="major", axis="x", **kwargs.get("minor_grid_kwargs", {}))
    ...
    def bar_plot(
        ...
        for i, metric_name in enumerate(metrics.keys()):
            ...
            metric_num = len(metrics.keys())
            width = total_width / metric_num
            x = xticks - (total_width - width) / 2
            x = x + i * width
            Y = np.array(
                [
