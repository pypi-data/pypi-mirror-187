    ...
    def _fit_regularized(self, K_X, K_Y):
        ...
        if self.svd_solver =='randomized':
            ...
            if np.max(np.abs(U.imag)) > 1e-8:
                warn(f"Computed projector is not real. Imaginary part as high as {np.max(np.abs(U.imag)):.3f}. The Kernel matrix is either severely ill conditioned or non-symmetric, discarting imaginary parts.")
