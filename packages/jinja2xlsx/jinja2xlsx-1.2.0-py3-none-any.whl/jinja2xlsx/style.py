...
class Style:
    ...
    bg: PatternFill = field(default_factory=PatternFill)
    ...
    def union(self, style: 'Style') -> 'Style':
        ...
        if self.bg and style.bg:
            bg = union_dicts(vars(self.bg), vars(style.bg))
        else:
            bg = {}

        return Style(Border(**border_data), Alignment(**alignment_data), Font(**font_data), PatternFill(**bg))
...
def extract_style(style_attr: str) -> Style:
    ...
    bg = _build_bg(style_dict)

    return Style(border, alignment, font, bg)
...
def _build_bg(style_dict):
    """
    >>> fill = _build_bg({'background': '#858585'})
    >>> fill.start_color.rgb
    '00858585'
    """
    bg: str = style_dict.get('background')
    if not bg:
        return None

    bg = f'00{bg[1:]}'
    return PatternFill(start_color=bg, end_color=bg, fill_type=FILL_SOLID)
