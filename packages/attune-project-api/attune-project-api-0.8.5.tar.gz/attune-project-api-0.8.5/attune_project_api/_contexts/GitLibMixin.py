...
class GitLibMixin:
    ...
    def pushToRemote(
        ...
        # Reload the remoteBranch reference
        remoteBranch = self._repo.branches[remoteBranchName]

        if remoteBranch.target != checkedOutBranch.target:
            raise Exception(
                "Remote rejected push. Please check if you have permissions "
                "to push to the remote or file sizes in Git are not too big"
            )
