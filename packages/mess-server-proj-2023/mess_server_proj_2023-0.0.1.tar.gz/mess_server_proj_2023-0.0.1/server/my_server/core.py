...
class MessageProcessor(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                Serv_log.info(f'Установлено соедение с ПК {client_address}')
                client.settimeout(5)
            ...
            try:
                ...
                Serv_log.error(f'Ошибка работы с сокетами: {err.errno}')
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        ...
                        Serv_log.debug(f'Getting data from client exception.', exc_info=err)
    ...
    def remove_client(self, client):
        ...
        '''
        Serv_log.info(f'Клиент {client.getpeername()} отключился от сервера.')
    ...
    def autorize_user(self, message, sock):
        ...
        Serv_log.debug(f'Start auth process for {message[USER]}')
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            response = RESPONSE_400
            response[ERROR] = 'Имя пользователя уже занято.'
            try:
                Serv_log.debug(f'Username busy, sending {response}')
                send_message(sock, response)
            except OSError:
                Serv_log.debug('OS Error')
                pass
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            try:
                Serv_log.debug(f'Unknown username, sending {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            Serv_log.debug('Correct username, starting passwd check.')
            ...
            digest = hash.digest()
            Serv_log.debug(f'Auth message = {message_auth}')
            try:
                # Обмен с клиентом
                send_message(sock, message_auth)
                ans = get_message(sock)
            except OSError as err:
                Serv_log.debug('Error in auth, data:', exc_info=err)
                sock.close()
                return
