    ...
    with pytest.raises(DatasetHeadOutOfDateException):
        with responses.RequestsMock() as resp:
            ...
            if function == "push_file":
                gantry_dataset_obj.push_file(input, parent_commit_id=parent_commit_id)
            elif function == "push_tabular_file":
                gantry_dataset_obj.push_tabular_file(input, parent_commit_id=parent_commit_id)
            elif function == "push_dataframe":
                gantry_dataset_obj.push_dataframe(input, parent_commit_id=parent_commit_id)
...
def test_add_item_good_parent(gantry_dataset_obj, function, commit_history, add_file_commit):
    ...
    with responses.RequestsMock() as resp:
        ...
        if function == "push_file":
            new_commit = gantry_dataset_obj.push_file(input, parent_commit_id=parent_commit_id)
        elif function == "push_tabular_file":
            new_commit = gantry_dataset_obj.push_tabular_file(
                input, parent_commit_id=parent_commit_id
            )
        elif function == "push_dataframe":
            new_commit = gantry_dataset_obj.push_dataframe(input, parent_commit_id=parent_commit_id)
