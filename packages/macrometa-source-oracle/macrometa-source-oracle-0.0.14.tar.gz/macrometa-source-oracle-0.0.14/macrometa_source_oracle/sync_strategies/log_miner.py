...
def sync_tables(conn_config, streams, state, end_scn):
   ...
   # total_poll_seconds = 15 * 60 # keep polling for 15mins without receiving any data
   polling_interval = POLLING_INTERVAL # polling interval in seconds, when no data is received

   while True:
      ### For now we keep polling forever as we don't want to stpo the workflow when no data is recieved. ###
      # Disconnect when no data received for total_poll_seconds
      # poll_duration = (datetime.datetime.utcnow() - scn_received_timestamp).total_seconds()
      # if poll_duration > total_poll_seconds:
      #    LOGGER.info('Breaking - %i seconds of polling with no data', poll_duration)
      #    break

      state, found = sync_tables_logminer(conn_config,cur, streams, state, start_scn, end_scn)
