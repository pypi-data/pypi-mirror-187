    ...
    async def run_task(self, data: ExecutorTask) -> None:
        ...
        async with self._counter:
            if self._counter.locked():
                self._stream._consumers[self.executor_topic]._unlocked.clear()  # TODO: lock stream when no counter
            for n, cmd in enumerate(data['commands']):
                if self._closing:
                    break
                stage = data['stage'] + n
                cmd = Template(cmd).fill(template_data)
                _, result = await self._rpc.call(cmd, headers, session=ServerSessionFlag)
