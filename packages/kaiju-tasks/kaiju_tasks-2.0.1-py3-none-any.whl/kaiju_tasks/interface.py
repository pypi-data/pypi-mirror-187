...
task_command = j.Object({
    ...
    'method': j.String(),
    'params': j.Object()
}, additionalProperties=False, required=['method'])
...
task_schema = j.Object({
    ...
    'restart_policy': j.Enumerated(
        enum=[RestartPolicy.CURRENT.value, RestartPolicy.FIRST.value]),
    'notify': j.Boolean(),
    'system': j.Boolean(),
    'name': j.String(),
...
class TaskService(SQLService, AbstractRPCCompatible):
    ...
    async def restart_task(self, task_id: str) -> None:
        ...
        if status in {TaskStatus.FINISHED, TaskStatus.FAILED}:
            sql = self.table.update().where(self.table.c.id == task_id).values({
