...
class TaskManager(ContextableService, AbstractRPCCompatible):
    ...
    def __init__(
        self,
        app,
        database_service: SQLService.database_service_class = None,
        ...
        self._db = self.discover_service(database_service, cls=SQLService.database_service_class)
    ...
    async def write_stage(
        ...
        if error:
            ...
            if result:
                ...
                await self._notify(id, result['job_id'], result['user_id'], TaskStatus.FAILED.value, result['result'])
        elif stage == stages - 1:
            ...
            if result:
                result = result._asdict()
                await self._notify(id, result['job_id'], result['user_id'], TaskStatus.FINISHED.value, result['result'])
        else:
    ...
    async def _queue_suspended_and_idle(self):
        ...
        async with self._db.begin() as conn:
            ...
            queued_tasks = [r._asdict() for r in queued_tasks.all()]
    ...
    async def _restart_cron_tasks(self):
        ...
        async with self._db.begin() as conn:
            ...
            cron_tasks = [r._asdict() for r in cron_tasks.all()]
            if cron_tasks:
                sql = (
                    self.table.update()
    ...
    async def _notify(self, id: str, job_id: uuid.UUID, user_id: uuid.UUID, status: str, result: list):  # noqa
        """Notify user of task results."""
        notification = Notification(
            message=f'task.{id}.{status}', task_id=id, job_id=job_id, user_id=user_id, status=status, result=result
        )
