...
class AFPSportsParameters(ProcessorParameters):
    ...
    ignore_qids: List[str] = Field("Q2736", description="List of wikidata QIDs to ignore", extra="advanced")
...
class AFPSportsProcessor(ProcessorBase):
    ...
    def process(
        ...
        ignore_qids = params.ignore_qids or []
            ...
            with add_logging_context(docid=document.identifier):
                if document.altTexts:
                    ...
                    if fingers and len(fingers) > 0:
                        finger = fingers[0]
                        wikidatas = [t for t in finger.split() if t.startswith('Q') and t not in ignore_qids]
