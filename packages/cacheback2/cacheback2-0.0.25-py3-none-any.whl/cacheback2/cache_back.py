...
def cache_from_list():
    try:
        ...
        # if no tables were added for caching, cache all
        # else, cache only the tables added
        if not cached_objects:
            for df_name in sorted(globals()):
                if not df_name.startswith('__'):
                    if type(eval(df_name)) == pd.DataFrame:
                        print(f"inserting {df_name} ...")
                        df_table = generate_var_name(df_name)
                        insert(eval(df_name), destination_db_table=df_table)
