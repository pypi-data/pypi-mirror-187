...
def plot_mesh(mesh, transform='sqrt', figure=None):
    ...
    if transform == 'sqrt':
        ...
        img_plot.pcolormesh(mesh.bins_mz, mesh.bins_rt, img, shading='auto')
...
def default_parameters(instrument, avg_fwhm_rt):
    pastaq_parameters = {
        ...
        'num_samples_mz': 5,
        'num_samples_rt': 5,
        'smoothing_coefficient_mz': 0.8,
        'smoothing_coefficient_rt': 0.8,
        #
        # Warp2D.
        #
        'warp2d_slack': 30,
    ...
    if instrument == 'orbitrap':
        pastaq_parameters['instrument_type'] = 'orbitrap'
        pastaq_parameters['resolution_ms1'] = 70000
        pastaq_parameters['resolution_msn'] = 30000
        pastaq_parameters['reference_mz'] = 200
    elif instrument == 'tof':
        pastaq_parameters['instrument_type'] = 'tof'
        pastaq_parameters['resolution_ms1'] = 30035
        pastaq_parameters['resolution_msn'] = 22248
        pastaq_parameters['reference_mz'] = 600.7
...
def detect_peaks_ms1(params, output_dir, save_grid=False, logger=None, force_override=False):
    ...
    for file in params['input_files']:
        ...
        peaks = pastaq.find_peaks(raw_data, grid,
                centroid = params['centroid_ms1'],
...
def dda_pipeline(
    ...
    detect_peaks_ms1(pastaq_parameters, output_dir, save_grid, logger, force_override)
...
def save_dia_mgf(file_path, linked_peaks, stem):
    prev_stdout = sys.stdout
    with open(file_path, 'w') as f:
        sys.stdout = f
        i = 0
        for linked_spectra in linked_peaks:
            print("BEGIN IONS")
            print("TITLE=id:{} feature_{} File:\"{}\"".format(i, linked_spectra.feature_id, stem))
            print("RTINSECONS={}".format(linked_spectra.monoisotopic_rt))
            print("PEPMASS={} {}".format(linked_spectra.monoisotopic_mz, linked_spectra.monoisotopic_height))
            print("CHARGE={}+".format(linked_spectra.charge_state))
            for peak in linked_spectra.spectra:
                print("{} {}".format(peak.mz, peak.height))
            print("END IONS")
            i += 1
        sys.stdout = prev_stdout

def extract_dia_spectra(params, output_dir, logger=None, force_override=False):
    _custom_log('Starting linking of DIA peaks to MS1 features', logger)
    time_start = time.time()

    for file in params['input_files']:
        stem = file['stem']

        out_path_pseudospectra = os.path.join(output_dir, 'linking', "{}.dia_pseudospectra.link".format(stem))
        if os.path.exists(out_path_pseudospectra) and not force_override:
            continue
        ...
        _custom_log("Separating ms2 windows: {}".format(stem), logger)
        windows = pastaq.separate_dia_windows(raw_data)

        # Store separated ms2 windows.
        for window, raw_data_window in windows.items():
            mz = window.precursor_mz
            width = window.window_wideness
            if params['dia_window_size'] != 0.0:
                width = params['dia_window_size']
            out_path = os.path.join(output_dir, 'raw', "{}.ms2_window_{}_{}".format(stem, mz, width))
            if os.path.exists(out_path) and not force_override:
                continue
            _custom_log('Writing MS2 window ({}, {}): {}'.format(mz, width, out_path), logger)
            raw_data_window.dump(out_path)

        # Read MS1 features
        in_path_features = os.path.join(output_dir, 'features', "{}.features".format(stem))
        features = pastaq.read_features(in_path_features)
        all_linked_peaks = []
        for window, raw_data_window in windows.items():
            mz = window.precursor_mz
            ...
            in_path = os.path.join(output_dir, 'raw', "{}.ms2_window_{}_{}".format(stem, mz, width))
            out_path = os.path.join(output_dir, 'peaks', "{}.ms2_window_{}.peaks".format(stem, mz))

            # Detect peaks in ms2 windows.
            # TODO: Only resample if the peaks are not found!
            if os.path.exists(out_path) and not force_override:
                peaks = pastaq.read_peaks(out_path)
            else:
                _custom_log("Resampling: {} ({})".format(stem, mz), logger)
                grid = pastaq.resample(
                    raw_data_window,
                    params['num_samples_mz'],
                    params['num_samples_rt'],
                    params['smoothing_coefficient_mz'],
                    params['smoothing_coefficient_rt'],
                    params['resolution_msn'],
                )
                _custom_log("Finding peaks: {} ({})".format(stem, mz), logger)
                peaks = pastaq.find_peaks(raw_data_window, grid,
                        centroid = params['centroid_ms2'],
                        max_peaks = params['max_peaks'])
                _custom_log('Writing peaks: {} ({})'.format(out_path, mz), logger)
                pastaq.write_peaks(peaks, out_path)

            # Linking ms2 peaks to features.
            _custom_log("Linking peaks from window: {}".format(mz), logger)
            # TODO: Need some more parameters here (Sigma).
            linked_peaks = pastaq.link_dia_peaks_to_features(features, peaks,
                    mz - width / 2, mz + width / 2, 1.0)
            all_linked_peaks += linked_peaks

        # Save binary pseudospectra.
        _custom_log("Writing extracted pseudospectra: {}".format(stem), logger)
        pastaq.write_dia_pseudospectra(all_linked_peaks, out_path_pseudospectra)

        # Convert to mgf
        _custom_log("Writing mgf: {}".format(stem), logger)
        out_path_mgf = os.path.join(output_dir, 'mgf', "{}.mgf".format(stem))
        save_dia_mgf(out_path_mgf, all_linked_peaks, stem)

    elapsed_time = datetime.timedelta(seconds=time.time()-time_start)
    _custom_log('Finished linking of DIA peaks to MS1 features in {}'.format(elapsed_time), logger)

def create_dia_consensus(params, output_dir, logger=None, force_override=False):
    _custom_log('Starting dia consensus creation', logger)
    time_start = time.time()

    # TODO: Only do this if consensus file is not present
    _custom_log('Reading feature clusters', logger)
    in_path_feature_clusters = os.path.join(output_dir,
            'metamatch', 'features.clusters')
    feature_clusters = pastaq.read_feature_clusters(in_path_feature_clusters)

    # Prepare list of pseudospectra and data structure for pre-consensus.
    # target_list = {
    #    "file_1": {
    #       "feature_ids": [...],
    #       "cluster_ids": [...],
    ...
    #    ...
    # }
    _custom_log('Preparing target list', logger)
    target_list = {}
    for cluster in feature_clusters:
        ids = cluster.feature_ids
        for id in ids:
            if id.file_id not in target_list:
                target_list[id.file_id] = { 'feature_ids': [], 'cluster_ids': [] }
            target_list[id.file_id]['feature_ids'] += [id.feature_id]
            target_list[id.file_id]['cluster_ids'] += [cluster.id]

    # target_clusters = {
    #     cluster_1 : [ps1, ps2, ps3 ...],
    #     cluster_2 : [ps1, ps2, ps3 ...],
    # }
    target_clusters = {}
    _custom_log('Preparing clusters spectra list', logger)
    for file_id in target_list.keys():
        stem = params['input_files'][file_id]['stem']
        _custom_log('Reading extracted pseudospectra: {}'.format(stem), logger)
        in_path_pseudospectra = os.path.join(output_dir,
                'linking', "{}.dia_pseudospectra.link".format(stem))
        spectra_list = pastaq.read_dia_pseudospectra(in_path_pseudospectra)
        feature_ids = target_list[file_id]['feature_ids']
        cluster_ids = target_list[file_id]['cluster_ids']
        for i, feature_id in enumerate(feature_ids):
            for spec in spectra_list:
                if feature_id == spec.feature_id:
                    if cluster_ids[i] not in target_clusters:
                        target_clusters[cluster_ids[i]] = []
                    target_clusters[cluster_ids[i]] += [spec]
                    break

    _custom_log('Performing consensus', logger)
    all_consensus_spectra = []
    for cluster in target_clusters.keys():
        _custom_log('Generating consensus for cluster: {}'.format(cluster), logger)
        all_spectra = target_clusters[cluster]
        if len(all_spectra) == 0:
            continue

        if len(all_spectra) == 1:
            # target_clusters[cluster] = all_spectra[0]
            # all_consensus_spectra += all_spectra
            continue

        consensus = pastaq.generate_dia_consensus(all_spectra)
        target_clusters[cluster] = consensus
        all_consensus_spectra += [consensus]

    # Save consensus pseudospectra in binary form as well as mgf
    out_path_mgf = os.path.join(output_dir, 'mgf', "consensus_spectra.mgf")
    save_dia_mgf(out_path_mgf, all_consensus_spectra, "consensus_spectra")

    elapsed_time = datetime.timedelta(seconds=time.time()-time_start)
    _custom_log('Finished dia consensus creation in {}'.format(elapsed_time), logger)
...
def dia_pipeline(
    ...
    if not os.path.exists(os.path.join(output_dir, 'quant')):
        os.makedirs(os.path.join(output_dir, 'quant'))
    if not os.path.exists(os.path.join(output_dir, 'mgf')):
        os.makedirs(os.path.join(output_dir, 'mgf'))
    ...
    calculate_similarity_matrix(pastaq_parameters, output_dir, 'peaks', logger, force_override)
    perform_rt_alignment(pastaq_parameters, output_dir, logger, force_override)
    # TODO: Perform alignment on MS2 peaks as well?
    calculate_similarity_matrix(pastaq_parameters, output_dir, 'warped_peaks', logger, force_override)
    perform_feature_detection(pastaq_parameters, output_dir, logger, force_override)
    match_peaks_and_features(pastaq_parameters, output_dir, logger, force_override)
    extract_dia_spectra(pastaq_parameters, output_dir, logger, force_override)
    # create_dia_consensus(pastaq_parameters, output_dir, logger, force_override)
    # parse_mzidentml_files(pastaq_parameters, output_dir, logger, force_override)
    # link_peaks_msms_idents(pastaq_parameters, output_dir, logger, force_override)
    # create_quantitative_tables(pastaq_parameters, output_dir, logger, force_override)
    # generate_qc_plots(pastaq_parameters, output_dir, logger, force_override)
