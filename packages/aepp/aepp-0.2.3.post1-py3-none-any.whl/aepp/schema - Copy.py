    ...
    def __transformationDict__(self,mydict:dict=None,typed:bool=False,dictionary:dict=None)->dict:
        ...
        for key in mydict:
            if type(mydict[key]) == dict:
                if mydict[key].get('type',mydict[key].get('meta:xdmType')) == 'object' or 'properties' in mydict[key].keys():
                    print(key)
                    ...
                    if properties is not None:
                        if key != "property" and key != "customFields":
                            dictionary[key] = {}
                            print(dictionary)
                            dictionary[key] = self.__transformationDict__(mydict[key]['properties'],typed,dictionary=dictionary[key])
                        else:
                    ...
                    if levelProperties is not None:
                        dictionary[key] = [{}]
                        dictionary[key][0] = self.__transformationDict__(levelProperties,typed,dictionary[key][0])
                    else:
                        if typed:
