...
class Controller(controller.Controller):
    ...
    def verificar_se_existe(self, linha, sigla_uf):
        filter_dict = {
            "ncm": linha['ncm'],
            "uf": sigla_uf,
            "chave": linha['chave'],
    ...
    def get_erro(self, linha, sigla_uf):
        filter_dict = {
            "ncm": linha['ncm'],
            "uf": sigla_uf,
            "chave": linha['chave'],
    ...
    def cadastrar_por_arquivo(self):
        ...
        if tipo == 'CSV':
            ...
            for linha in linhas:
                if line_count != 0:
                    if linha['tipo'] == 0:
                        entity = self.verificar_se_existe(linha, sigla_uf)
                        try:
                            if entity is None:
                                self.manager.driver.\
                                    transaction_manager.count = 0
                                self.manager.create(**linha)
                                qtd_sucesso += 1
                                erro = self.get_erro(linha, sigla_uf)
                                if erro is not None:
                                    self.manager.api.ncm_ibpt_erros().update(
                                        id=erro.id, **{'corrigido': True})
                            else:
        ...
        # tasks.cadastrar_ncm_ibpt_erro(erros, self.get_user_id())
