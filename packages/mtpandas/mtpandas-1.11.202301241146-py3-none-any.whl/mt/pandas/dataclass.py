...
def pack_dataclass(df, output_col: str, klass, field_map: Optional[dict] = None, drop: bool = False):
    ...
    if not dataclasses.is_dataclass(klass):
        raise TypeError("The provided class is not a dataclass: '{}'.".format(type(klass)))
    ...
    for col in field_map:
        if not col in df.columns:
            raise ValueError("Field '{}' does not exist in the given dataframe with columns '{}'.".format(col, df.columns))
...
def unpack_dataclass(df, input_col: str, klass: Optional[object] = None, field_map: Optional[dict] = None, drop: bool = False):
    ...
    if input_col not in df.columns:
        raise ValueError("Field '{}' does not exist in the given dataframe with columns '{}'.".format(input_col, df.columns))
