...
def daniell(*args, **kwargs):  # pylint: disable=too-many-branches
    ...
    kwargs : `dict`
        ...
        number_averages: `int`, optional
            Window size for averaging (number of bins) in frequency domain. Must be an odd number.
            If "binning" is specified to be "logarithmic", window size will depend on bin number
             as `number_averages + floor(log_base ** bin_number)`
        binning: `str`, optional
            Either "constant" for constant averaging window size = `number_averages` (default) or "logarithmic"
            for window size depending on bin number as `number_averages + floor(log_base ** bin_number)`
...
def daniell_average(frq, CSD, timeseries, ap):
    ...
    CSD : `np.array` of `float`
        powers array
    timeseries: `TimeSeries`
        one (any of the two) of the original time series from which CSD was calculated.
        Needed to extract optional information such as channel name etc.
...
class AveragingParameters:  # pylint: disable=too-many-instance-attributes
    ...
    def bias_correction(self, CSD):
        ...
        bias_correction : `list` of `float` or `float`
            either list of float for different correction in each bin,
            or a single float for same correction in every bin.
        """
