...
class SettingsStringList(SettingsValueList):
    ...
    def create_from_dict(data: dict):
        s = SettingsStringList(data["_value"], data["_min_length"], data["_max_length"], data["_min_number_of_elements"], data["_max_number_of_elements"], data["_read_only"], data["_visible"], data["_description"], data["_can_be_input"], 
                     data["_is_input"], data["_full_input_name"])
        s._id = data["_id"]
        return s

    def __eq__(self, other: SettingsStringList):
        return super(SettingsStringList, self).__eq__(other) and self._compare_lists(self._value, other._value) and \
    ...
    # Name of the type.
    _type_str = "SettingsStringList"

    # Datatype attributes.
    _attributes = Attributes("SettingsStringList", version=0, member_count=12, known_types=[])

    # Serialization.
    def serialize(self, writer_fun: Callable[[Any, str, str, Optional[CustomSerializer]], None]):
        super(SettingsStringList, self).serialize(writer_fun)
    ...
    def deserialize(reader_fun: Callable[[str, Optional[CustomSerializer]], Any]):
        data = super(SettingsStringList, SettingsStringList).deserialize(reader_fun)
