...
class HeroContours(HeroData):
    ...
    def create_from_dict(data: dict):
        return HeroContours(data["_contour_set"], data["_name"], data["_metadata"])

    def __eq__(self, other: HeroContours):
        if not super(HeroContours, self).__eq__(other):
            return False
    ...
    # Name of the type.
    _type_str = "HeroContours"

    # Datatype attributes.
    _attributes = Attributes("HCs", version=0, member_count=3, known_types=["Contour"])

    # Serialization.
    def serialize(self, writer_fun: Callable[[Any, str, str, Optional[CustomSerializer]], None]):
        super(HeroContours, self).serialize(writer_fun)
    ...
    def deserialize(reader_fun: Callable[[str, Optional[CustomSerializer]], Any]):
        data = super(HeroContours, HeroContours).deserialize(reader_fun)
