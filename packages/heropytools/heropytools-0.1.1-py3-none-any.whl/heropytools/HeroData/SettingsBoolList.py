...
class SettingsBoolList(SettingsValueList):
    ...
    def create_from_dict(data: dict):
        s = SettingsBoolList(data["_value"], data["_min_number_of_elements"], data["_max_number_of_elements"], data["_read_only"], data["_visible"], data["_description"], data["_can_be_input"], 
                     data["_is_input"], data["_full_input_name"])
        s._id = data["_id"]
        return s

    def __eq__(self, other):
        return super(SettingsBoolList, self).__eq__(other) and self._compare_lists(self._value, other._value)
    ...
    # Name of the type.
    _type_str = "SettingsBoolList"

    # Datatype attributes.
    _attributes = Attributes("SettingsBoolList", version=0, member_count=10, known_types=[])

    # Serialization.
    def serialize(self, writer_fun: Callable[[Any, str, str, Optional[CustomSerializer]], None]):
        super(SettingsBoolList, self).serialize(writer_fun)
        writer_fun(self._value, "Boolean[]", "V", None)

    # Deserialization.
    @staticmethod
    def deserialize(reader_fun: Callable[[str, Optional[CustomSerializer]], Any]):
        data = super(SettingsBoolList, SettingsBoolList).deserialize(reader_fun)
