...
from PyQt5.QtWidgets import QWidget, QFrame, QPushButton, QGridLayout, QLayout, QHBoxLayout, QVBoxLayout, QTabWidget\
    , QStatusBar, QButtonGroup, QAbstractButton, QRadioButton, QComboBox, QTextEdit
...
class Element:
    ...
    def create(self):
        if self.name in self.useFunc.keys():
        ...
        for i in range(len(rows)):
            ...
            for j in range(len(cols)):
                ...
                if len(col.e.childs) == 0:
                    continue
                # if row.span > 1 or col.span > 1:
                #     es.append([col.e.childs[0].qt, row.num, col.num, row.span, col.span])
                # else:
                #     es.append([col.e.childs[0].qt, row.num, col.num])
