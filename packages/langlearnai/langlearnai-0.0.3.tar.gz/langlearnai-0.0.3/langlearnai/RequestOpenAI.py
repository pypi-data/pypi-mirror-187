...
from langlearnai.TTSandNLTK import get_keywords_in_phrase, generate_audio_text
import pycountry
...
def generate_language_lesson(lesson_requests, target_language='english', 
    ...
    for request in lesson_requests:
        ...
        elif mode == 'custom': ## custom_request in format ('key', 'full request')
            lesson_request=custom_request[1]
        response = openai.Completion.create(
          model="text-davinci-003",
        ...
        if mode != 'custom' :
            ...
            lesson_dict[custom_request[0]]=fullresponse
