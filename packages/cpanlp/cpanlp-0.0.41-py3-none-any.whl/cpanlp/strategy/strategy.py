...
class Strategy:
    def __init__(self, name, market_focus, strategy):
        self.name = name
...
class FinancialStrategy(Strategy):
    def __init__(self, name, market_focus, strategy):
        super().__init__(name, market_focus, strategy)
        self.leverage_ratio = strategy
...
def leverage_strategy(total_debt, total_equity):
...
def poison_pill(shares_outstanding, dilution_factor):
    new_shares_outstanding = shares_outstanding * (1 + dilution_factor)
    return new_shares_outstanding
def main():
    a=FinancialStrategy("huawei","gowth",leverage_strategy(10000,2000))
    print(a.leverage_ratio)
    b=poison_pill(1000,0.1)
    print(b)
