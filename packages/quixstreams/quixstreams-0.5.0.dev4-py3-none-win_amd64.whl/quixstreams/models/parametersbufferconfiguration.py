...
class ParametersBufferConfiguration(object):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p = None):
        ...
        if net_pointer is None:
            self.__interop = pbci(pbci.Constructor())
        else:
            self.__interop = pbci(net_pointer)
    ...
    def custom_trigger_before_enqueue(self, value: Callable[[ParameterDataTimestamp], bool]):
        ...
        if value is None:
            self.__interop.set_CustomTriggerBeforeEnqueue(None)
            return
        ...
        self.__interop.set_CustomTriggerBeforeEnqueue(callback)
