...
class ParameterDataRaw(object):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p = None):
        ...
        if net_pointer is None:
            self.__interop = pdri(pdri.Constructor())
        else:
            self.__interop = pdri(net_pointer)
    ...
    def from_panda_frame(data_frame: pd.DataFrame, epoch: int = 0) -> 'ParameterDataRaw':
        ...
        parameter_data_raw.__set_values(
            epoch=epoch, 
    ...
    def __set_values(
        ...
        def _map_dictionary(dct, type, mapper=None):
            d = cc.dict_to_net(dct, mapper)
            return d


        #set epoch
        self.__interop.set_Epoch(epoch)
    ...
    def epoch(self) -> int:
        ...
        return self.__interop.get_Epoch()
    ...
    def timestamps(self) -> [int]:
        ...
        if self.__timestamps is None:
            array_uptr = self.__interop.get_Timestamps()
            self.__timestamps = ai.ReadLongs(array_uptr)
    ...
    def numeric_values(self) -> Dict[str, List[float]]:
        ...
        if self.__numeric_values is None:
            array_hptr = self.__interop.get_NumericValues()
            self.__numeric_values = InteropUtils.invoke_and_free(array_hptr, di.ReadStringNullableDoubleArrayDictionary)
    ...
    def string_values(self) -> Dict[str, List[str]]:
        ...
        if self.__string_values is None:
            array_hptr = self.__interop.get_StringValues()
            self.__string_values = InteropUtils.invoke_and_free(array_hptr, di.ReadStringStringArrayDictionary)
    ...
    def binary_values(self) -> Dict[str, List[bytes]]:
        ...
        if self.__binary_values is None:
            array_hptr = self.__interop.get_BinaryValues()
            self.__binary_values = InteropUtils.invoke_and_free(array_hptr, di.ReadStringByteArray2Dictionary)
    ...
    def tag_values(self) -> Dict[str, List[str]]:
        ...
        if self.__tag_values is None:
            array_hptr = self.__interop.get_TagValues()
            self.__tag_values = InteropUtils.invoke_and_free(array_hptr, di.ReadStringStringArrayDictionary)

        return self.__tag_values

    def convert_to_parameterdata(self) -> ParameterData:
        ptr = pdi.Constructor2(self.__pointer)
