...
class StreamPropertiesReader(object):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p):
        ...
        def __on_first_changed_sub():
            ref = self.__interop.add_OnChanged(__on_changed_net_handler)
            self.__cfuncrefs.append(ref)

        def __on_last_changed_unsub():
            # TODO do unsign with previous handler
            self.__interop.remove_OnChanged(__on_changed_net_handler)


        self.on_changed = EventHook(__on_first_changed_sub, __on_last_changed_unsub, name="StreamPropertiesReader.on_changed")
        """
    ...
    def __finalizerfunc(self):
        if self.__metadata is not None:
            InteropUtils.free_hptr(self.__metadata.get_net_pointer())
        if self.__parents is not None:
            InteropUtils.free_hptr(self.__parents.get_net_pointer())
        self.__cfuncrefs = None

    @property
    def name(self) -> str:
        """ Gets the name of the stream """
        return self.__interop.get_Name()

    @property
    def location(self) -> str:
        """ Gets the location of the stream """
        return self.__interop.get_Location()

    @property
    def time_of_recording(self) -> datetime:
        """ Gets the datetime of the recording """

        ptr = self.__interop.get_TimeOfRecording()
    ...
    def metadata(self) -> Dict[str, str]:
        ...
        if self.__metadata is None:
            ptr = self.__interop.get_Metadata()
            self.__metadata = NetDict.constructor_for_string_string(ptr)
    ...
    def parents(self) -> List[str]:
        ...
        if self.__parents is None:
            list_ptr = self.__interop.get_Parents()
            self.__parents = NetList.constructor_for_string(list_ptr)
