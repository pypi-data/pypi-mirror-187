...
class ParameterData(object):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p = None):
        ...
        if net_pointer is None:
            self.__interop = pdi(pdi.Constructor())
        else:
            self.__interop = pdi(net_pointer)
    ...
    def __finalizerfunc(self):
        if self.__timestamps is not None:
            [pdstamp.dispose() for pdstamp in self.__timestamps]
    ...
    def add_timestamp(self, time: Union[datetime, timedelta]) -> ParameterDataTimestamp:
        ...
        if isinstance(time, datetime):
            netdate = dtc.datetime_to_dotnet(time)
            return self.__add_to_timestamps(ParameterDataTimestamp(self.__interop.AddTimestamp( netdate)))
        if isinstance(time, timedelta):
            nettimespan = dtc.timedelta_to_dotnet(time)
            return self.__add_to_timestamps(ParameterDataTimestamp(self.__interop.AddTimestamp2( nettimespan)))
    ...
    def add_timestamp_milliseconds(self, milliseconds: int) -> ParameterDataTimestamp:
        ...
        return self.__add_to_timestamps(ParameterDataTimestamp(self.__interop.AddTimestampMilliseconds(milliseconds)))
    ...
    def add_timestamp_nanoseconds(self, nanoseconds: int) -> ParameterDataTimestamp:
        ...
        return self.__add_to_timestamps(ParameterDataTimestamp(self.__interop.AddTimestampNanoseconds(nanoseconds)))

    def __add_to_timestamps(self, pdts):
        """
    ...
    def _init_timestamps(self):
        ...
        try:
            timestamps_hptr = self.__interop.get_Timestamps()
            refs = ei.ReadReferences(timestamps_hptr)
            self.__timestamps = [_converter_to_python(refs[i]) for i in range(len(refs))]
        finally:
            iu.free_hptr(timestamps_hptr)
