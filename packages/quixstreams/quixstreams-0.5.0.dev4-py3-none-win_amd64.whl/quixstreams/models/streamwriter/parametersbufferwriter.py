...
class ParametersBufferWriter(ParametersBuffer):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p):
        ...
        self.__interop = pbwi(net_pointer)
        ParametersBuffer.__init__(self, self.__interop._get_interop_ptr())  # TODO is this necessary?
    ...
    def add_timestamp(self, time: Union[datetime, timedelta]) -> ParameterDataBuilder:
        ...
        if isinstance(time, datetime):
            try:
                ...
                return ParameterDataBuilder(self.__interop.AddTimestamp(netdate_hptr))
            finally:
        ...
        if isinstance(time, timedelta):
            try:
                ...
                return ParameterDataBuilder(self.__interop.AddTimestamp2(nettimespan_uptr))
            finally:
    ...
    def add_timestamp_nanoseconds(self, nanoseconds: int) -> ParameterDataBuilder:
        ...
        :return: ParameterDataBuilder
        """
        return ParameterDataBuilder(self.__interop.AddTimestampNanoseconds(nanoseconds))
    ...
    def write(self, packet: Union[ParameterData, pd.DataFrame]) -> None:
        ...
        if isinstance(packet, ParameterData):
            self.__interop.Write(packet.get_net_pointer())
            return
