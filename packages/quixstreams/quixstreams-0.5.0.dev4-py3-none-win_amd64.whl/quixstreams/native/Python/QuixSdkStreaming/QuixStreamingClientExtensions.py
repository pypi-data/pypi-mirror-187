...
class QuixStreamingClientExtensions(object):
    ...
    __interop_func = InteropUtils.get_function("quixstreamingclientextensions_openinputtopic")
    __interop_func.restype = c_void_p
    __interop_func.argtypes = [c_void_p, c_void_p, c_void_p, ctypes.c_int, ctypes.c_int]
    @staticmethod
    def OpenInputTopic(client: c_void_p, topicId: str, consumerGroup: str = None, commitMode: CommitMode = CommitMode.Automatic, autoOffset: AutoOffsetReset = AutoOffsetReset.Earliest) -> c_void_p:
        """
        Parameters
        ----------
        ...
        result = InteropUtils.invoke("quixstreamingclientextensions_openinputtopic", client, topicId_ptr, consumerGroup_ptr, commitMode.value, autoOffset.value)
