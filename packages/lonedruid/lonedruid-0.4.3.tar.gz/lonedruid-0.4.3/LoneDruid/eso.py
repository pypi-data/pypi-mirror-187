...
def multieso(nums: list[int], path: str = 'LoneDruidOutput.py', eso_num: bool = True, eso_oper: bool = True):
    ...
    with open(path, 'w') as f: # * To create a file and write `[` to it (start a list)
        f.write(f"[{', '.join([int_to_eso(num, eso_num=eso_num, eso_oper=eso_oper) for num in nums])}] # type: ignore")