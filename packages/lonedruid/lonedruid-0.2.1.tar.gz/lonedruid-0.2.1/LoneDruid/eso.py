...
def multieso(nums: list[int], path: str = 'LoneDruidOutput.py'):
    ...
    with open(path, 'w') as f: # * To create a file and write `[` to it (start a list)
        f.write('[')
    with open(path, 'a') as f: # * To append to the list and close it with `]`
        f.write(f"{', '.join([int_to_eso(num) for num in nums])}] # type: ignore") # > type: ignore because expression value unused