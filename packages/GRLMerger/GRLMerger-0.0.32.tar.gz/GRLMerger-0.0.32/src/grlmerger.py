...
#     checkUniqueIDs(r_ID)
    ...
    for x in range(len(parent_actors_IDs)):
        ...
        if contribution_values[x] == 'hurt':
            ...
            table["contribution_value"].append(-75)
...
def findElementsThreshold(mergingMethod):
    ...
    elements_len = len(all_elements_similarity_df.head(25))
    print("Select the row number of the threshold value")
    while True:
        threshold_number = input()
        try:
            threshold_number = int(threshold_number)
            if threshold_number >= 0 and threshold_number < elements_len:
                break;
            else:
                print("Please enter a valid input (0 to ",str(elements_len)+")")
                continue;
        except ValueError:
            print("Please enter a valid input (0 to ",str(elements_len)+")")
