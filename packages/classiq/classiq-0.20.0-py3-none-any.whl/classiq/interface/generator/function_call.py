...
from classiq.exceptions import ClassiqControlError
...
class FunctionCall(BaseModel):
    ...
    def _validate_slices(
        ...
        name_slice_pairs = [FunctionCall.parse_io_slicing(input) for input in inputs]
    ...
    def partitioned_ios(self, io: IO) -> Mapping[RegisterPartition, WireName]:
        params_ios = (
            self.function_params.inputs_full(self.assign_zero_ios)
            if (io == IO.Input)
            else self.function_params.outputs
        )
        return FunctionCall._partition_registers(self.inputs_dict, params_ios)

    @staticmethod
    def _partition_registers(
        io_dict: Dict[IOName, WireName],
        params_ios: Dict[IOName, RegisterUserInput],
    ) -> Mapping[RegisterPartition, WireName]:
        names_with_slices = io_dict.keys()
        name_slice_pairs: List[Tuple[IOName, slice]] = [
            FunctionCall.parse_io_slicing(name) for name in names_with_slices
        ]

        slices_dict: Dict[IOName, List[Tuple[slice, WireName]]] = defaultdict(list)
        for wire_name, (name, slice_) in zip(io_dict.values(), name_slice_pairs):
            slices_dict[name].append((slice_, wire_name))

        names_to_ranges = {name: range(reg.size) for name, reg in params_ios.items()}

        if not (set(names_to_ranges) <= set(slices_dict)):
            raise ValueError("Slice without corresponding range")

        partitioned_register_list: List[Tuple[RegisterPartition, WireName]] = []

        for name, slice_wire_tuples in slices_dict.items():
            reg_range = names_to_ranges[name]
            partition_wire_pairs = slices_to_partitions_with_payload(
                name, slice_wire_tuples, reg_range
            )
            partitioned_register_list.extend(partition_wire_pairs)

        return dict(partitioned_register_list)
