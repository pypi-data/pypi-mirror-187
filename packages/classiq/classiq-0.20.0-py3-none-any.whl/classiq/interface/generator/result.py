...
_VENDOR_TO_INSTRUCTION_SET: Dict[str, QuantumInstructionSet] = {
    ...
    ProviderVendor.AWS_BRAKET: QuantumInstructionSet.QASM,
}
...
class GeneratedCircuit(VersionedModel, CircuitWithOutputFormats):
    ...
    data: Optional[GeneratedCircuitData]
    analyzer_data: Dict
    ...
    def get_registers(
        self, register_names: Collection[RegisterName]
    ) -> List[GeneratedRegister]:
        if self.data is None:
            raise ClassiqStateInitializationError(
                "The circuit doesn't contain the register data."
            )
    ...
    def get_registers_initialization(
        ...
        registers = self.get_registers(register_names=initial_values.keys())
