...
class FunctionHandler(abc.ABC):
    ...
    def _add_function_call(
        ...
        if parameters_dict:
            ...
            params = self._assign_parameters_to_func(params, parameters_dict)
    ...
    def _get_new_parameters_mapping(
        function_data: CompositeFunctionData,
    ...
    def _assign_parameters_in_logic_flow(
        ...
        if isinstance(func_params, CustomFunction):
            assigned = self._assign_parameters_to_func(func_params, parameters_dict)
        else:
            assigned = func_params._assign_parameters(parameters_dict)
        return func_call.copy(update={"function_params": assigned}, deep=True)

    def _assign_parameters_to_func(
        self, func: FunctionParams, parameters_dict: Dict[str, ParameterFloatType]
    ) -> FunctionParams:
        if not isinstance(func, CustomFunction):
            return func._assign_parameters(parameters_dict)
        if self._function_library is None:
            return func
        original_name = func.name.split(ASSIGNED)[0]
        function_data = self._function_library.data.function_dict.get(func.name)
        if not isinstance(function_data, CompositeFunctionData):
            return func
        ...
        if new_name not in self._function_library._params:
        ...
        return self._function_library.get_function(new_name)
