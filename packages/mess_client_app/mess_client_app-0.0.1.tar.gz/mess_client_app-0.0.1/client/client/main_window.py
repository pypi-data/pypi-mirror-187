...
from pack_client.client import AddContactDialog
...
#        self.ui.text_message.clicked.connect(self.send_message)
        ...
        # политика переноса слов в ярлыке
    ...
    def history_list_update(self):
        ...
        # Получаем историю сортированную по дате (item[3])
        ...
        # Заполнение модели записями, разделяем входящие и исходящие выравниванием и разным фоном.
        # Записи в обратном порядке, поэтому выбираем их с конца и не более 20
        for i in range(start_index, length):
            elem = hist_list[i]
            if elem[1] == 'in':
                mess = QStandardItem(f'Входящее от {elem[3].replace(microsecond=0)}:\n {elem[2]}')
                mess.setEditable(False)
                mess.setBackground(QBrush(QColor(255, 213, 213)))
                mess.setTextAlignment(Qt.AlignLeft)
                self.history_model.appendRow(mess)
            else:
                mess = QStandardItem(f'Исходящее от {elem[3].replace(microsecond=0)}:\n {elem[2]}')
                mess.setEditable(False)
    ...
    def select_active_user(self):
        """Функция обработчик даблклика по списку контаков"""
    ...
    def set_active_user(self):
        ...
        # Если все в порядке - ставим надпись и активируем кнопки
    ...
    def add_contact_window(self):
        """Функция создающая окно-диалог добавления контакта"""
    ...
    def add_contact(self, new_contact):
        ...
        try:
            ...
            self.messages.critical(self, 'Ошибка сервера при добавлении контакта (main_window.py 188)', err.text)
    ...
    def delete_contact(self, item):
        """
        Метод удаляющий контакт из серверной и клиентсткой БД.
        После обновления баз данных обновляет и содержимое окна.
        """
        ...
        try:
            ...
            self.messages.critical(self, 'Ошибка сервера при удалении контакта (main_window.py 218)', err.text)
        except OSError as err:
            if err.errno:
                self.messages.critical(self, 'Ошибка', 'Потеряно соединение с сервером!')
                self.close()
