...
class Night:
    ...
    def __repr__(self) -> str:
        ...
        out += ', '.join([f'{a}={typo(getattr(self, a) if hasattr(self, a) else "UNSET")}' for a in an])
    ...
    def __init__(self, *args):
        ...
        if len(an) != len(args):
            v = f'{self.__class__.__name__}({", ".join(["".join([f"{a}: {r[1].annotation.__name__}" for r in signature(an[a].build).parameters.items()]) for a in an])})'
