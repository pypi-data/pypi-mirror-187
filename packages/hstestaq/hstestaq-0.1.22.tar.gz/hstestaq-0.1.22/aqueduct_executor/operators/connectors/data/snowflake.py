...
class SnowflakeConnector(relational.RelationalConnector):
    ...
    def extract_spark(self, params: extract.RelationalParams, sparkSessionObj: SparkSession) -> Any:
        assert params.usable(), "Query is not usable. Did you forget to expand placeholders?"

        df = sparkSessionObj.read.format("snowflake").options(**self.snowflake_spark_options).option("query", params.query).load()
        return df
