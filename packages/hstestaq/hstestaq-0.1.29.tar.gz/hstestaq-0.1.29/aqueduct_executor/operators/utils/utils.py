...
def read_artifacts_spark(
    ...
    for (input_path, input_metadata_path) in zip(input_paths, input_metadata_paths):
        ...
        if artifact_type != ArtifactType.TABLE:
            ...
            #read from temp view
            try:
                globalTempDB = spark.conf.get("spark.sql.globalTempDatabase")
                viewPath = global_temp_db + "." + convert_path_to_view_name(input_path)
                sparkDF = sparkSessionObj.read.table(viewPath)
                inputs.append(sparkDF)
            except Exception as e:
                raise MissingInputPathsException(
...
def write_artifact_spark(
    ...
    if output_path is not None:
        if artifact_type == ArtifactType.TABLE:
            ...
            sparkDF = content
            viewName = convert_path_to_view_name(output_path)
            print("VIEWNAME")
            print(viewName)
            sparkDF.createOrReplaceGlobalTempView(viewName)
            # sampleDF = sparkDF.sample(fraction=0.1)
            pandasDF = sparkDF.limit(100).toPandas()
            serialized_val = serialize_val_wrapper(pandasDF, serialization_type)
            storage.put(output_path, serialized_val)
