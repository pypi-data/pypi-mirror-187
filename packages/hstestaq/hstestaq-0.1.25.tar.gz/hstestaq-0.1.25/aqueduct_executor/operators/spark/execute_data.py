...
def run(spec: Spec, sparkSessionObj: SparkSession) -> None:
    """
    ...
    try:
        _execute_spark(spec, storage, exec_state, sparkSessionObj)
...
def _execute_spark(spec: Spec, storage: Storage, exec_state: ExecutionState, sparkSessionObj: SparkSession) -> None:
    ...
    # we do both in `run_authenticate()`, and give a more helpful error message on failure.
        ...
        if spec.type == JobType.EXTRACT:
            run_extract_spark(spec, op, storage, exec_state, sparkSessionObj)
...
def run_extract_spark(
    spec: ExtractSpec, op: connector.DataConnector, storage: Storage, exec_state: ExecutionState, sparkSessionObj: SparkSession
) -> None:
    ...
    def _extract() -> Any:
        return op.extract_spark(spec.parameters, sparkSessionObj)
    ...
    if exec_state.status != ExecutionStatus.FAILED:
        utils.write_artifact_spark(
            ...
            output,
            system_metadata={},
            sparkSessionObj=sparkSessionObj,
        )
...
def run_load(
    spec: LoadSpec, op: connector.DataConnector, storage: Storage, exec_state: ExecutionState, sparkSessionObj: SparkSession,
) -> None:
