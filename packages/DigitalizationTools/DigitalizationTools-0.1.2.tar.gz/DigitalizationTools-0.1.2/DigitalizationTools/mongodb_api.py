...
# ------------------ collections -------------------------
@beartype
def insert_doc(collectionName:str, insert_dict:dict):
    mycol = collection(clientAddress, databaseName, collectionName)
    info  = mycol.insert(insert_dict)
    return info

@beartype
def delete_doc(collectionName:str, id:str):
    mycol       = collection(clientAddress, databaseName, collectionName)
...
@beartype
def query_doc(collectionName:str, id:str):
    mycol       = collection(clientAddress, databaseName, collectionName)
...
@beartype
def update_doc(collectionName:str, update_condition:dict, id:str):
    mycol       = collection(clientAddress, databaseName, collectionName)
...
@beartype
def get_all_doc(collectionName:str):
    mycol       = collection(clientAddress, databaseName, collectionName)
...
@beartype
def insert_file(collectionName:str,filecontent:bytes, filename:str, filetype:str):
    mycol   = collection(clientAddress, databaseName, collectionName)
...
@beartype
def delete_file(collectionName:str, id:str):
...
@beartype
def get_all_files(collectionName:str, get_condition:Optional[Union[dict,None]]=None):
    col    = collection(clientAddress, databaseName, collectionName)
    mycol  = col.get_collection()
    if get_condition == None:
        files = mycol.find()
    else:
        files = mycol.find(get_condition)
    all = []
    for x in files:
        all.append(x)
    return pd.DataFrame(all)
