...
class DfPipeline:
    ...
    def __init__(self, steps: Any = None, **kwargs) -> None:
        ...
        Args:
            steps (Any, optional): Dictionary of steps. Defaults to None.
        """
        Logger.update_logger(name="df_pipeline")
        self.steps = collections.OrderedDict({} if steps is None else steps)
        self.env = kwargs
        ...
        def __init__(
            ...
            **kwargs,
            ...
            Args:
                ...
                __pipeline (Any): DfPipeline instance.
                __func (Any): Function of the step.
            """
            self.pipeline = __pipeline
            self.is_leaf = __is_leaf
            self.output_name = __output_name
            self.func = __func
            self.args = args
            self.kwargs = kwargs
            self.name = f"step_{len(__pipeline.steps)}"
            self.pipeline.steps[self.name] = self
    ...
    def add_step(self, __func: Any, *args, **kwargs) -> Any:
        ...
        """
        DfPipeline.Step(False, None, self, __func, *args, **kwargs)
        return self
    ...
    def add_leaf(self, __output_name: Any, __func: Any, *args, **kwargs) -> Any:
        ...
        Returns:
            Any: return the pipeline updated with the new step.
        """
        DfPipeline.Step(True, __output_name, self, __func, *args, **kwargs)
        return self
