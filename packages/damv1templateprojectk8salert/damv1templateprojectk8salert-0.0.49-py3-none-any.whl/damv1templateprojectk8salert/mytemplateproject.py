...
<h2 style="margin-bottom:40px; margin-top:0; padding:0;">Contex id : {{value_str_contexid}}</h2>
{{value_strlst_grplines}}
<hr style = "margin-top:45px; margin-bottom:5px; padding:0;" />
...
'''
    tmplt_evrnt_restart_deploy_failed = f'''<h3 style = "margin:0; padding:0; line-height:15px;">The list of deployment rollout restart failed:</h3>
{{value_str_lst_deploy_failed}}
'''


    tmplt_evrnt_str_deploy_succeed = f'''<p style = "margin:0; padding:0; line-height:18px;">{{value_str_number}} - {{value_str_namespace}}::{{value_str_deployment}} --> {{value_str_scaledup_podname}} ( {{value_str_response_scaledup}} )</p>'''
    tmplt_evrnt_str_deploy_failed = f'''<p style = "margin:0; padding:0; line-height:18px;">{{value_str_number}} - {{value_str_namespace}}::{{value_str_deployment}}</p>'''


## - - - - - - - - - - - - - - - - - - - - - - - -
# DEPRICATED | 20 Januari 2023, still used in PRODUCTION
## - - - - - - - - - - - - - - - - - - - - - - - -
    tmplt_telemsg = f'''
