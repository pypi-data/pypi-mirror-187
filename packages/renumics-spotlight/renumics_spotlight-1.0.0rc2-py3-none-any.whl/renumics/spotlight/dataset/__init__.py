...
def prepare_path_or_url(path_or_url: PathOrURLType, workdir: PathType) -> PathOrURLType:
    """
...
def prepare_pandas_column(
    ...
    if issubclass(dtype, (np.ndarray, Window, BaseData)):
        null_mask = column.isnull()
        if issubclass(dtype, BaseFileBasedData):
...
def asarray(values: Any) -> np.ndarray:
    """
...
def convert_pandas_column(
    ...
    if dtype is not None:
        column = prepare_pandas_column(column, dtype, workdir)
        if issubclass(dtype, (np.ndarray, Window, BaseData)):
    ...
    null_mask = column.isnull()
    try:
        data = asarray(column[~null_mask].to_list())
...
def is_ref_column_type_name(type_name: str) -> bool:
    """
...
def get_column_name(column: h5py.Dataset) -> str:
    """
...
def is_ref_column(column: h5py.Dataset) -> bool:
    """
...
class Dataset:
    ...
    def _user_column_attributes(column_type: Type[ColumnType]) -> Dict[str, Type]:
        ...
        if issubclass(column_type, BaseFileBasedData):
    ...
    def _default_default(cls, column_type: Type[ColumnType]) -> Any:
        ...
        if column_type is np.ndarray or issubclass(column_type, BaseData):
            return None
    ...
    def __delitem__(self, item: Union[str, IndexType, Indices1DType]) -> None:
        ...
        if isinstance(item, str):
            ...
            for column_name in self.keys() + INTERNAL_COLUMN_NAMES:
                ...
                if get_column_type(column) is Embedding:
    ...
    def from_pandas(
        ...
        for column_name in df.columns:
            column, column_dtype, attributes = convert_pandas_column(
                df[column_name],
    ...
    def append_categorical_column(
        ...
        Example:
            Find an example usage in  :class:`renumics.spotlight.data_types.Category`.
        """
        self._append_column(
            name,
            Category,
    ...
    def append_embedding_column(
        ...
        Example:
            Find an example usage in  :class:`renumics.spotlight.data_types.Embedding`.
        """
    ...
    def append_sequence_1d_column(
        ...
        Example:
            Find an example usage in  :class:`renumics.spotlight.data_types.Sequence1D`.
        """
        if y_label is None:
            y_label = name
        self._append_column(
    ...
    def append_mesh_column(
        ...
        Example:
            Find an example usage in  :class:`renumics.spotlight.data_types.Mesh`.
        """
        self._append_column(
            name,
            Mesh,
    ...
    def append_image_column(
        ...
        Example:
            Find an example usage in  :class:`renumics.spotlight.data_types.Image`.
        """
        self._append_column(
            name,
            Image,
    ...
    def append_audio_column(
        ...
        Example:
            Find an example usage in :class:`renumics.spotlight.data_types.Audio`.
        """
        attrs = {}
    ...
    def append_window_column(
        ...
        Example:
            Find an example usage in :class:`renumics.spotlight.data_types.Window`.
        """
        self._append_column(
            name,
            Window,
    ...
    def get_column_type(
        ...
        Example:
            ...
            mesh <class 'renumics.spotlight.dataset.data_types.Mesh'>
            >>> with Dataset("docs/example.h5", "r") as dataset:
            ...     for column_name in sorted(dataset.keys()):
    ...
    def get_column_attributes(
        ...
        column_type = get_column_type(column_attrs)
    ...
    def _append_column(
        ...
        **attrs: Any,
        ...
        if column_type is Category:
            ...
            shape = (0, 2)
            maxshape = (None, 2)
        elif issubclass(column_type, BaseFileBasedData):
    ...
    def _get_column_names_and_length(self) -> Tuple[Set[str], int]:
        ...
        length_modes = statistics.multimode(lengths)
    ...
    def _encode_simple_values(
        ...
        # pylint: disable=too-many-branches, too-many-return-statements
        column_type = cast(Type[SimpleColumnType], get_column_type(column))
        if column_type is Category:
            mapping = dict(
    ...
    def _encode_value(
        ...
        column_type = get_column_type(attrs)
        if is_ref_column(column):
    ...
    def _encode_ref_value(
        ...
        elif issubclass(column_type, BaseFileBasedData) and isinstance(
