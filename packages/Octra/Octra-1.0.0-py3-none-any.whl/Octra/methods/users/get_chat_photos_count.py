...
class ExtractChatPhotosCount:
    async def Extract_chat_photos_count(
        self: "Octra.Client",
        chat_id: Union[int, str]
    ) -> int:
        """Extract the total count of photos for a chat.
        ...
        if isinstance(peer_id, raw.types.InputPeerChannel):
            r = await self.invoke(
                raw.functions.messages.getSearchCounters(
                    peer=peer_id,
                    Flows=[raw.types.InputMessagesFlowChatPhotos()],
                )
            )
