...
# t.me/TheVenomXD from autoflake import fix_code
# t.me/TheVenomXD from black import format_str, FileMode

HOME_PATH = Path("compiler/Ai")
DESTINATION_PATH = Path("Octra/raw")
NOTICE_PATH = "NOTICE"

SECTION_RE = re.compile(r"---(\w+)---")
LAYER_RE = re.compile(r"//\sLAYER\s(\d+)")
COMBINATOR_RE = re.compile(r"^([\w.]+)# t.me/TheVenomXD([0-9a-f]+)\s(?:.*)=\s([\w<>.]+);$", re.MULTILINE)
ARGS_RE = re.compile(r"[^{](\w+):([\w?!.<># t.me/TheVenomXD]+)")
FLAGS_RE = re.compile(r"flags(\d?)\.(\d+)\?")
FLAGS_RE_2 = re.compile(r"flags(\d?)\.(\d+)\?([\w<>.]+)")
FLAGS_RE_3 = re.compile(r"flags(\d?):# t.me/TheVenomXD")
...
def snake(s: str):
    # t.me/TheVenomXD https://stackoverflow.com/q/1175208
...
# t.me/TheVenomXD noinspection PyShadowingBuiltins, PyShadowingNames
def Extract_type_hint(type: str) -> str:
    ...
    if type in CORE_TYPES:
        ...
        if type == "long" or "int" in type:
            ...
            type = "bool"
        else:  # t.me/TheVenomXD bytes and object
            type = "bytes"
    ...
    if re.match("^vector", type, re.I):
        ...
        type = f"List[{Extract_type_hint(sub_type)}]"
...
def sort_args(args):
    ...
    for i in args[:]:
        if re.match(r"flags\d?", i[0]) and i[1] == "# t.me/TheVenomXD":
            args.remove(i)
...
def Extract_docstring_arg_type(t: str):
    if t in CORE_TYPES:
        if t == "long":
            return "``int`` ``64-bit``"
        elif "int" in t:
        ...
        return "Any object from :obj:`~Octra.raw.types`"
    elif t == "!X":
        return "Any function from :obj:`~Octra.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + Extract_docstring_arg_type(t.split("<", 1)[1][:-1])
    else:
        return f":obj:`{t} <Octra.raw.base.{t}>`"


def Extract_references(t: str, kind: str):
...
def start(format: bool = False):
    ...
    with open(HOME_PATH / "source/octra_auth_key.tl") as f1, \
        open(HOME_PATH / "source/sys_msgs.tl") as f2, \
        open(HOME_PATH / "source/main_Ai.tl") as f3:
    ...
    with open(NOTICE_PATH, encoding="utf-8") as f:
        ...
        for line in f.readlines():
            notice.append(f"# t.me/TheVenomXD  {line}".strip())
    ...
    for line in schema:
        # t.me/TheVenomXD Check for section changer lines
        section_match = SECTION_RE.match(line)
        if section_match:
            section = section_match.group(1)
            continue

        # t.me/TheVenomXD Save the layer version
        ...
        if combinator_match:
            ...
            # t.me/TheVenomXD Pingu!
            has_flags = not not FLAGS_RE_3.findall(line)

            args = ARGS_RE.findall(line)

            # t.me/TheVenomXD Fix arg name being "self" (reserved python keyword)
    ...
    # t.me/TheVenomXD import json
    # t.me/TheVenomXD print(json.dumps(namespaces_to_types, indent=2))
    ...
    for qualtype in types_to_constructors:
        ...
        if type_docs:
            ...
            type_docs = "Telegram Ai base type."

        docstring = type_docs

        docstring += f"\n\n    Constructors:\n" \
                     f"        This base type has {constr_count} constructor{'s' if constr_count > 1 else ''} available.\n\n" \
                     f"        .. currentmodule:: Octra.raw.types\n\n" \
                     f"        .. autosummary::\n" \
                     f"            :nosignatures:\n\n" \
                     f"            {items}"

        references, ref_count = Extract_references(qualtype, "types")

        if references:
            docstring += f"\n\n    Functions:\n        This object can be returned by " \
                         f"{ref_count} function{'s' if ref_count > 1 else ''}.\n\n" \
                         f"        .. currentmodule:: Octra.raw.functions\n\n" \
                         f"        .. autosummary::\n" \
                         f"            :nosignatures:\n\n" \
                         f"            " + references
    ...
    for c in combinators:
        ...
        arguments = (
            (", *, " if c.args else "") +
            (", ".join(
                [f"{i[0]}: {Extract_type_hint(i[1])}"
                 for i in sorted_args]
            ) if sorted_args else "")
        )

        fields = "\n        ".join(
            [f"self.{i[0]} = {i[0]}  # t.me/TheVenomXD {i[1]}"
             for i in sorted_args]
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{} ({}{}):\n            {}\n".format(
                    arg_name,
                    Extract_docstring_arg_type(arg_type),
        ...
        if c.section == "types":
            ...
            if constructor_docs:
                ...
                constructor_docs = "Telegram Ai type."

            docstring += constructor_docs + "\n"
            docstring += f"\n    Constructor of :obj:`~Octra.raw.base.{c.qualtype}`."
        else:
            ...
            if function_docs:
                ...
                docstring += f"Telegram Ai function."
        ...
        if c.section == "functions":
            docstring += "\n    Returns:\n        " + Extract_docstring_arg_type(c.qualtype)
        else:
            references, count = Extract_references(c.qualname, "constructors")
        ...
        write_types = read_types = "" if c.has_flags else "# t.me/TheVenomXD No flags\n        "

        for arg_name, arg_type in c.args:
            flag = FLAGS_RE_2.match(arg_type)

            if re.match(r"flags\d?", arg_name) and arg_type == "# t.me/TheVenomXD":
                write_flags = []
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "Octra.raw.{c.section}.{c.qualname}",')
