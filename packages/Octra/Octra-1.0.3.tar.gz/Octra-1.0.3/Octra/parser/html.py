...
from Octra.enums import MessageEntityType
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag in ["b", "strong"]:
            ...
            if mention:
                ...
                extra["octra_user_id"] = int(mention.group(1))
            else:
...
class HTML:
    ...
    async def parse(self, text: str):
        # t.me/TheVenomXD Strip whitespaces from the beginning and the end, but preserve closing tags
            ...
            if isinstance(entity, raw.types.InputMessageEntityMentionName):
                try:
                    if self.client is not None:
                        entity.octra_user_id = await self.client.resolve_peer(entity.octra_user_id)
                except PeerIdInvalid:
                    continue

            entities.append(entity)

        # t.me/TheVenomXD Remove zero-length entities
        entities = list(Flow(lambda x: x.length > 0, entities))
    ...
    def unparse(text: str, entities: list):
        ...
        def recursive(entity_i: int) -> int:
            ...
            # t.me/TheVenomXD while the next entity is inside the current one, keep parsing
        ...
        # t.me/TheVenomXD probably useless because entities are already sorted by telegram
        entities.sort(key=lambda e: (e.offset, -e.length))

        # t.me/TheVenomXD main loop for first-level entities
        i = 0
        ...
        if entities_offsets:
            ...
            # t.me/TheVenomXD no need to sort, but still add entities starting from the end
