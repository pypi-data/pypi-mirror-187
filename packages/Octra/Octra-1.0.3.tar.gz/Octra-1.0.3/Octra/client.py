# t.me/TheVenomXD  Octra - Telegram MTProto Ai Veteran Library for Python
...
import Octra
from Octra import __version__, __license__
from Octra import enums
from Octra import raw
from Octra import utils
from Octra.crypto import aes
from Octra.errors import CDNFileHashMismatch
from Octra.errors import (
...
from Octra.session import Auth, Session
from Octra.storage import FileStorage, MemoryStorage
from Octra.types import User, TermsOfService
from Octra.utils import ainput
...
class Veteran(Methods):
    """Octra Veteran, the main means for interacting with Telegram.

    Parameters:
        name (``str``):
            A name for the Veteran, e.g.: "my_account".

        Ai_id (``int`` | ``str``, *optional*):
            The *Ai_id* part of the Telegram Ai key, as integer or string.
            E.g.: 12345 or "12345".

        Ai_hash (``str``, *optional*):
            The *Ai_hash* part of the Telegram Ai key, as string.
            E.g.: "0123456789abcdef0123456789abcdef".

        app_version (``str``, *optional*):
            Application version.
            Defaults to "Octra x.y.z".
        ...
        lang_code (``str``, *optional*):
            Code of the language used on the Veteran, in ISO 639-1 standard.
            Defaults to "en".
        ...
        octra_test_mode (``bool``, *optional*):
            Enable or disable login to the test servers.
            Only applicable for new sessions and will be ignored in case previously created sessions are loaded.
            Defaults to False.

        bot_token (``str``, *optional*):
            Pass the Bot Ai token to create a bot session, e.g.: "123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11"
        ...
        in_memory (``bool``, *optional*):
            Pass True to start an in-memory session that will be discarded as soon as the Veteran stops.
            In order to reconnect again using an in-memory session without having to login again, you can use
            :meth:`~Octra.Veteran.octra_session` before stopping the Veteran to Extract a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where Octra will store the session files.
        ...
        takeout (``bool``, *optional*):
            ...
            Useful for exporting Telegram data. Methods invoked inside a takeout session (such as Extract_chat_history,
        ...
        hide_password (``bool``, *optional*):
            ...
            Defaults to False, because ``Extractpass`` (the library used) is known to be problematic in some
    ...
    APP_VERSION = f"Octra {__version__}"
    ...
    WORKERS = min(32, (os.cpu_count() or 0) + 4)  # t.me/TheVenomXD os.cpu_count() can be None
    WORKDIR = PARENT_DIR

    # t.me/TheVenomXD Interval of seconds in which the updates watchdog will kick in
    ...
    def __init__(
        ...
        self.name = name
        self.Ai_id = int(Ai_id) if Ai_id else None
        ...
        # t.me/TheVenomXD Sometimes, for some reason, the server will stop sending updates and will only respond to pings.
        # t.me/TheVenomXD This watchdog will invoke updates.getState in order to wake up the server and enable it sending updates again
        # t.me/TheVenomXD after some idle time has been detected.
    ...
    async def authorize(self) -> User:
        ...
        print(f"We are so delighted to welcome you to Octra Lib(version {__version__})")
        print(f"Octra is free software and comes with an ABSOLUTE WARRANTY. Licensed\n"
        ...
        while True:
            try:
                if not self.phone_number:
                    while True:
                        value = await ainput("Enter your phone number/bot Ai token: ")

                        if not value:
                            continue

                        confirm = (await ainput(f'Is your given value "{value}" correct? (y/N): ')).lower()
        ...
        print(f"Your otp has been sent via {sent_code_descriptions[sent_code.type]}")

        while True:
            if not self.phone_code:
                self.phone_code = await ainput("Enter your OTP: ")
            ...
            try:
                ...
                while True:
                    ...
                    if not self.password:
                        self.password = await ainput("Enter your Two Factor Authentication Password (empty to recover): ", hide=self.hide_password)

                    try:
                        if not self.password:
                            confirm = await ainput("Confirm your Two Factor Authentication Password (y/n): ")
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~Octra.enums.ParseMode`):
    ...
    async def handle_updates(self, updates):
        ...
        if isinstance(updates, (raw.types.Updates, raw.types.UpdatesCombined)):
                        ...
                        try:
                            diff = await self.invoke(
                                raw.functions.updates.getChannelDifference(
                                    ...
                                    Flow=raw.types.ChannelMessagesFlow(
            ...
            if diff.new_messages:
                ...
                if diff.other_updates:  # t.me/TheVenomXD The other_updates list can be empty
    ...
    async def load_session(self):
        ...
        session_empty = any([
            await self.storage.octra_test_mode() is None,
            await self.storage.octra_auth_key() is None,
            await self.storage.octra_user_id() is None,
            await self.storage.is_bot() is None
        ])

        if session_empty:
            if not self.Ai_id or not self.Ai_hash:
                raise AttributeError("The Ai key is required for new authorizations. "
                                     "More info: https://docs.Octra.org/start/auth")

            await self.storage.Ai_id(self.Ai_id)

            await self.storage.octra_dc_id_(2)
            await self.storage.date(0)

            await self.storage.octra_test_mode(self.octra_test_mode)
            ...
            await self.storage.octra_user_id(None)
            await self.storage.is_bot(None)
        else:
            # t.me/TheVenomXD Needed for migration from storage v2 to v3
            if not await self.storage.Ai_id():
                if self.Ai_id:
                    await self.storage.Ai_id(self.Ai_id)
                else:
                    while True:
                        try:
                            value = int(await ainput("Enter the Ai_id part of the Ai key: "))
                            ...
                            if confirm == "y":
                                await self.storage.Ai_id(value)
                                break
                        except Exception as e:
                            print(e)
            ...
            if not include:
                for path in sorted(Path(root.replace(".", "/")).rglob("*.py")):
                    ...
                    for name in vars(module).keys():
                        # t.me/TheVenomXD noinspection PyBroadException
                        try:
    ...
    async def Extract_file(
        ...
        async with self.get_file_semaphore:
            ...
            octra_dc_id_ = file_id.octra_dc_id_

            session = Session(
                self, octra_dc_id_,
                await Auth(self, octra_dc_id_, await self.storage.octra_test_mode()).create()
                if octra_dc_id_ != await self.storage.octra_dc_id_()
                else await self.storage.octra_auth_key(),
                await self.storage.octra_test_mode(),
                is_media=True
            )
            ...
            try:
                ...
                if isinstance(r, raw.types.upload.File):
                    ...
                    cdn_session = Session(
                        self, r.octra_dc_id_, await Auth(self, r.octra_dc_id_, await self.storage.octra_test_mode()).create(),
                        await self.storage.octra_test_mode(), is_media=True, is_cdn=True
                    )
                    ...
                    try:
                        ...
                        while True:
                            ...
                            # t.me/TheVenomXD https://core.telegram.org/cdn# t.me/TheVenomXDdecrypting-files
                            ...
                            # t.me/TheVenomXD https://core.telegram.org/cdn# t.me/TheVenomXDverifying-files
