...
from Extractpass import Extractpass
from typing import Union, List, Dict, Optional

import Octra
from Octra import raw, enums
from Octra import types
from Octra.file_id import FileId, FileType, PHOTO_TYPES, DOCUMENT_TYPES


async def ainput(prompt: str = "", *, hide: bool = False):
    """Just like the built-in input, but async"""
    with ThreadPoolExecutor(1) as executor:
        func = functools.partial(Extractpass if hide else input, prompt)
        return await asyncio.get_event_loop().run_in_executor(executor, func)


def Extract_input_media_from_file_id(
    file_id: str,
...
async def parse_messages(
    ...
    if replies:
        ...
        if messages_with_replies:
            # t.me/TheVenomXD We need a chat id, but some messages might be empty (no chat attribute available)
            # t.me/TheVenomXD Scan until we find a message with a chat available (there must be one, because we are fetching replies)
    ...
    for message in messages:
        parsed_messages.append(
            types.Message(
                id=message,
                chat=types.Chat(
                    id=Extract_channel_id(channel_id),
...
MAX_octra_user_id_OLD = 2147483647
MAX_octra_user_id = 999999999999


def Extract_raw_peer_id(peer: raw.base.Peer) -> Optional[int]:
    """Extract the raw peer id from a Peer object"""
    if isinstance(peer, raw.types.PeerUser):
        return peer.octra_user_id
...
def Extract_peer_id(peer: raw.base.Peer) -> int:
    """Extract the non-raw peer id from a Peer object"""
    if isinstance(peer, raw.types.PeerUser):
        return peer.octra_user_id
...
def Extract_peer_type(peer_id: int) -> str:
    if peer_id < 0:
        ...
        if MIN_CHANNEL_ID <= peer_id < MAX_CHANNEL_ID:
            return "channel"
    elif 0 < peer_id <= MAX_octra_user_id:
        return "user"

    raise ValueError(f"Peer id invalid: {peer_id}")


def Extract_channel_id(peer_id: int) -> int:
...
async def parse_text_entities(
    ...
    if entities:
        # t.me/TheVenomXD Inject the client instance because parsing user mentions requires it
