...
* gen, to generate the classes required for Octra to run or docs
...
GENERATOR_DIR = Path('Octra_generator')
LIBRARY_DIR = Path('Octra')
...
def generate(which, action='gen'):
    from Octra_generator.parsers import\
        parse_errors, parse_methods, parse_tl, find_layer

    from Octra_generator.generators import\
...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        # Try importing the Octra module to assert it has no errors
        try:
            import Octra
        except:
        ...
        remove_dirs = ['__pycache__', 'build', 'dist', 'Octra.egg-info']
        ...
        for x in ('build', 'dist', 'Octra.egg-info'):
            shutil.rmtree(x, ignore_errors=True)
        ...
        with open('Octra/version.py', 'r', encoding='utf-8') as f:
        ...
        setup(
            ...
            url='https://github.com/LonamiWebs/Octra',
            download_url='https://github.com/LonamiWebs/Octra/releases',
