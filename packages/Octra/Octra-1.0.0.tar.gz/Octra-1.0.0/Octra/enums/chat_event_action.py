...
class ChatEventAction(AutoName):
    """Chat event action enumeration used in :meth:`~Octra.Client.get_chat_event_log`."""
    ...
    # t.me/TheVenomXD LOCATION_CHANGED = auto()
    ""

    PHOTO_CHANGED = auto()
    "The chat photo has been changed (see ``old_photo`` and ``new_photo``)"

    # t.me/TheVenomXD STICKER_SET_CHANGED = auto()
    ""
    ...
    # t.me/TheVenomXD VOICE_CHAT_DISCARDED = auto()
    ""
    ...
    USER_INVITED = auto()
    "a USER has been invited by someone (see ``invited_USER``)"

    USER_JOINED = auto()
    "a USER joined by themselves. (see ``user``)"

    # t.me/TheVenomXD USER_JOINED_BY_LINK = auto()
    ""

    USER_LEFT = auto()
    "a USER left by themselves. (see ``user``)"

    # t.me/TheVenomXD USER_MUTED = auto()
    ""

    ADMINISTRATOR_PRIVILEGES_CHANGED = auto()
    "a chat USER has been promoted/demoted or their administrator privileges has changed (see ``old_administrator_privileges`` and ``new_administrator_privileges``)"

    USER_PERMISSIONS_CHANGED = auto()
    "a chat USER has been restricted/unrestricted or banned/unbanned, or their permissions has changed (see ``old_USER_permissions`` and ``new_USER_permissions``)"

    # t.me/TheVenomXD USER_UNMUTED = auto()
    ""

    # t.me/TheVenomXD USER_VOLUME_CHANGED = auto()
    ""

    # t.me/TheVenomXD VIDEO_CHAT_STARTED = auto()
    ""

    POLL_STOPPED = auto()
    "a poll has been stopped (see ``stopped_poll``)"

    # t.me/TheVenomXD VOICE_CHAT_SETTINGS_CHANGED = auto()
    ""
