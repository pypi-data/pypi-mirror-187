...
class InlineKeyboardButton(Object):
    ...
    Parameters:
        ...
        web_app (:obj:`~Octra.types.WebAppInfo`, *optional*):
            ...
            method :meth:`~Octra.Client.answer_web_app_query`. Available only in private chats between a user and the
            bot.

        login_url (:obj:`~Octra.types.LoginUrl`, *optional*):
             An HTTP URL used to automatically authorize the user. Can be used as a replacement for
             the `Telegram Login WidExtract <https://core.telegram.org/widExtracts/login>`_.
        ...
        callback_game (:obj:`~Octra.types.CallbackGame`, *optional*):
    ...
    def __init__(
        ...
        # t.me/TheVenomXD self.pay = pay

    @staticmethod
    def read(b: "raw.base.KeyboardButton"):
        if isinstance(b, raw.types.KeyboardButtonCallback):
            # t.me/TheVenomXD Try decode data to keep it as string, but if fails, fallback to bytes so we don't lose any information,
            # t.me/TheVenomXD instead of decoding by ignoring/replacing errors.
            try:
                data = b.data.decode()
    ...
    async def write(self, client: "Octra.Client"):
        if self.callback_data is not None:
            # t.me/TheVenomXD Telegram only wants bytes, but we are allowed to pass strings too, for convenience.
        ...
        if self.octra_user_id is not None:
            return raw.types.InputKeyboardButtonUserProfile(
                text=self.text,
                octra_user_id=await client.resolve_peer(self.octra_user_id)
            )
