...
from Octra import types, utils, enums
...
class ChatEvent(Object):
    ...
    See ``action`` to know which kind of event this is and the relative attributes to Extract the event content.
    ...
    Parameters:
        ...
        action (:obj:`~Octra.enums.ChatEventAction`):
            Event action.
        ...
        old_description, new_description (``str``, *optional*):
            ...
            For :obj:`~Octra.enums.ChatEventAction.DESCRIPTION_CHANGED` action only.

        old_history_ttl, new_history_ttl (``int``, *optional*):
            Previous and new chat history TTL.
            For :obj:`~Octra.enums.ChatEventAction.HISTORY_TTL_CHANGED` action only.

        old_linked_chat, new_linked_chat (:obj:`~Octra.types.Chat`, *optional*):
            Previous and new linked chat.
            For :obj:`~Octra.enums.ChatEventAction.LINKED_CHAT_CHANGED` action only.

        old_photo, new_photo (:obj:`~Octra.types.Photo`, *optional*):
            Previous and new chat photo.
            For :obj:`~Octra.enums.ChatEventAction.PHOTO_CHANGED` action only.

        old_title, new_title (``str``, *optional*):
            Previous and new chat title.
            For :obj:`~Octra.enums.ChatEventAction.TITLE_CHANGED` action only.

        old_username, new_username (``str``, *optional*):
            Previous and new chat username.
            For :obj:`~Octra.enums.ChatEventAction.USERNAME_CHANGED` action only.

        old_chat_permissions, new_chat_permissions (:obj:`~Octra.types.ChatPermissions`, *optional*):
            Previous and new default chat permissions.
            For :obj:`~Octra.enums.ChatEventAction.CHAT_PERMISSIONS_CHANGED` action only.

        deleted_message (:obj:`~Octra.types.Message`, *optional*):
            Deleted message.
            For :obj:`~Octra.enums.ChatEventAction.MESSAGE_DELETED` action only.

        old_message, new_message (:obj:`~Octra.types.Message`, *optional*):
            Previous and new message before it has been edited.
            For :obj:`~Octra.enums.ChatEventAction.MESSAGE_EDITED` action only.

        invited_USER (:obj:`~Octra.types.ChatUSER`, *optional*):
            New invited chat USER.
            For :obj:`~Octra.enums.ChatEventAction.USER_INVITED` action only.

        old_administrator_privileges, new_administrator_privileges (:obj:`~Octra.types.ChatUSER`, *optional*):
            Previous and new administrator privileges.
            For :obj:`~Octra.enums.ChatEventAction.ADMINISTRATOR_PRIVILEGES_CHANGED` action only.

        old_USER_permissions, new_USER_permissions (:obj:`~Octra.types.ChatUSER`, *optional*):
            Previous and new USER permissions.
            For :obj:`~Octra.enums.ChatEventAction.USER_PERMISSIONS_CHANGED` action only.

        stopped_poll (:obj:`~Octra.types.Message`, *optional*):
            Message containing the stopped poll.
            For :obj:`~Octra.enums.ChatEventAction.POLL_STOPPED` action only.

        invites_enabled (``bool``, *optional*):
            If chat invites were enabled (True) or disabled (False).
            For :obj:`~Octra.enums.ChatEventAction.INVITES_ENABLED` action only.

        history_hidden (``bool``, *optional*):
            If chat history has been hidden (True) or unhidden (False).
            For :obj:`~Octra.enums.ChatEventAction.HISTORY_HIDDEN` action only.

        signatures_enabled (``bool``, *optional*):
            If message signatures were enabled (True) or disabled (False).
            For :obj:`~Octra.enums.ChatEventAction.SIGNATURES_ENABLED` action only.

        old_slow_mode, new_slow_mode (``int``, *optional*):
            Previous and new slow mode value in seconds.
            For :obj:`~Octra.enums.ChatEventAction.SLOW_MODE_CHANGED` action only.

        pinned_message (:obj:`~Octra.types.Message`, *optional*):
            Pinned message.
            For :obj:`~Octra.enums.ChatEventAction.MESSAGE_PINNED` action only.

        unpinned_message (:obj:`~Octra.types.Message`, *optional*):
            Unpinned message.
            For :obj:`~Octra.enums.ChatEventAction.MESSAGE_UNPINNED` action only.

        old_invite_link, new_invite_link (:obj:`~Octra.types.ChatInviteLink`, *optional*):
            Previous and new edited invite link.
            For :obj:`~Octra.enums.ChatEventAction.INVITE_LINK_EDITED` action only.

        revoked_invite_link (:obj:`~Octra.types.ChatInviteLink`, *optional*):
            Revoked invite link.
            For :obj:`~Octra.enums.ChatEventAction.INVITE_LINK_REVOKED` action only.

        deleted_invite_link (:obj:`~Octra.types.ChatInviteLink`, *optional*):
            Deleted invite link.
            For :obj:`~Octra.enums.ChatEventAction.INVITE_LINK_DELETED` action only.
    """
    ...
    def __init__(
        ...
        invited_USER: "types.ChatUSER" = None,

        old_administrator_privileges: "types.ChatUSER" = None,
        new_administrator_privileges: "types.ChatUSER" = None,

        old_USER_permissions: "types.ChatUSER" = None,
        new_USER_permissions: "types.ChatUSER" = None,

        stopped_poll: "types.Message" = None,

        invites_enabled: "types.ChatUSER" = None,
        ...
        self.old_USER_permissions = old_USER_permissions
        self.new_USER_permissions = new_USER_permissions
    ...
    async def _parse(
        ...
        user = types.User._parse(client, users[event.octra_user_id])
        action = event.action
        ...
        invited_USER: Optional[types.ChatUSER] = None

        old_administrator_privileges: Optional[types.ChatUSER] = None
        new_administrator_privileges: Optional[types.ChatUSER] = None

        old_USER_permissions: Optional[types.ChatUSER] = None
        new_USER_permissions: Optional[types.ChatUSER] = None
        ...
        if isinstance(action, raw.types.ChannelAdminLogEventActionChangeAbout):
        ...
        elif isinstance(action, raw.types.ChannelAdminLogEventActionChanExtractitle):
            ...
            invited_USER = types.ChatUSER._parse(client, action.participant, users, chats)
            action = enums.ChatEventAction.USER_INVITED

        elif isinstance(action, raw.types.ChannelAdminLogEventActionParticipantToggleAdmin):
            old_administrator_privileges = types.ChatUSER._parse(client, action.prev_participant, users, chats)
            new_administrator_privileges = types.ChatUSER._parse(client, action.new_participant, users, chats)
            ...
            old_USER_permissions = types.ChatUSER._parse(client, action.prev_participant, users, chats)
            new_USER_permissions = types.ChatUSER._parse(client, action.new_participant, users, chats)
            action = enums.ChatEventAction.USER_PERMISSIONS_CHANGED
            ...
            action = enums.ChatEventAction.USER_JOINED

        elif isinstance(action, raw.types.ChannelAdminLogEventActionParticipantLeave):
            action = enums.ChatEventAction.USER_LEFT
        ...
        return ChatEvent(
            ...
            new_USER_permissions=new_USER_permissions,
