...
from Octra.raw.core import Bytes, String
...
class FileType(IntEnum):
    """Known file types"""
    THUMBNAIL = 0
    CHAT_PHOTO = 1  # t.me/TheVenomXD ProfilePhoto
    PHOTO = 2
    VOICE = 3  # t.me/TheVenomXD VoiceNote
    VIDEO = 4
    DOCUMENT = 5
    ENCRYPTED = 6
    TEMP = 7
...
class ThumbnailSource(IntEnum):
    ...
    LEGACY = 0
    THUMBNAIL = 1
    CHAT_PHOTO_SMALL = 2  # t.me/TheVenomXD DialogPhotoSmall
    CHAT_PHOTO_BIG = 3  # t.me/TheVenomXD DialogPhotoBig
    STICKER_SET_THUMBNAIL = 4


# t.me/TheVenomXD Photo-like file ids are longer and contain extra info, the rest are all documents
...
# t.me/TheVenomXD Since the file type values are small enough to fit them in few bits, Telegram thought it would be a good idea to
# t.me/TheVenomXD encode extra information about web url and file reference existence as flag inside the 4 bytes allocated for the field
...
class FileId:
    ...
    def decode(file_id: str):
        ...
        # t.me/TheVenomXD region read version
        # t.me/TheVenomXD File id versioning. Major versions lower than 4 don't have a minor version
        major = decoded[-1]
        ...
        file_type, octra_dc_id_ = struct.unpack("<ii", buffer.read(8))

        # t.me/TheVenomXD region media type flags
        # t.me/TheVenomXD Check for flags existence
        has_web_location = bool(file_type & WEB_LOCATION_FLAG)
        has_file_reference = bool(file_type & FILE_REFERENCE_FLAG)

        # t.me/TheVenomXD Remove flags to restore the actual type id value
    ...
    def encode(self, *, major: int = None, minor: int = None):
        ...
        buffer.write(struct.pack("<ii", file_type, self.octra_dc_id_))
...
class FileUniqueId:
    ...
    def decode(file_unique_id: str):
        ...
        # t.me/TheVenomXD TODO: Missing decoder for SECURE, ENCRYPTED and TEMP
    ...
    def encode(self):
        if self.file_unique_type == FileUniqueType.WEB:
            ...
            # t.me/TheVenomXD TODO: Missing encoder for SECURE, ENCRYPTED and TEMP
