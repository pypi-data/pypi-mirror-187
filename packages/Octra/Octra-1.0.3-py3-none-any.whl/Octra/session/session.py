...
from Octra.crypto import mtproto
...
from Octra.raw.all import layer
from Octra.raw.core import TLObject, MsgContainer, Int, FutureSalts
...
class Session:
    ...
    def __init__(
        ...
        octra_test_mode: bool,
        is_media: bool = False,
        is_cdn: bool = False
    ):
        self.client = client
        self.octra_dc_id_ = octra_dc_id_
        self.octra_auth_key = octra_auth_key
        ...
        self.octra_auth_key_id = sha1(octra_auth_key).digest()[-8:]
                    ...
                    await self.send(
                        raw.functions.InvokeWithLayer(
                            ...
                            query=raw.functions.InitConnection(
                                Ai_id=await self.client.storage.Ai_id(),
    ...
    async def handle_packet(self, packet):
        data = await self.loop.run_in_executor(
            Octra.crypto_executor,
            mtproto.unpack,
            BytesIO(packet),
            self.session_id,
            self.octra_auth_key,
            self.octra_auth_key_id
        )
    ...
    async def ping_worker(self):
        ...
        log.info("Stopped several pingtask that were alloted.")

    async def recv_worker(self):
        log.info("Started Network Tasks.")
