...
`Octra.client.telegramclient.TelegramClient` which is subclass of them
all to provide the final unified interface while the methods can live in
different subclasses to be more maintainable.

The ABC is `Octra.client.telegrambaseclient.TelegramBaseClient` and the
first implementor is `Octra.client.users.UserMethods`, since calling
