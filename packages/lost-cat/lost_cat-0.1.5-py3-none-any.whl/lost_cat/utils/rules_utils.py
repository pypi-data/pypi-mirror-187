    ...
    def run(self, phrase: str) -> dict:
        ...
        if self.engine == RuleEngine.REGEX:
            if self.options.get("findall", False) is True:
                ...
                if m := re.findall(self.expr, phrase):
                    for tagvalue in self.tags:
                        ...
                        if grpkey := tagvalue.get("group"):
                            ...
                            resvalue = tagvalue.get("value")
                        if tagkey not in data.get("results", {}):
                            data["results"][tagkey] = set()
                        data["results"][tagkey].add(resvalue)

            elif m := re.match(self.expr, phrase):
                data["passed"] = True
            ...
            if data.get("passed") is True:
                for tagvalue in self.tags:
                    tagkey = tagvalue.get("key")
                    data["results"][tagkey] = tagvalue.get("value")
