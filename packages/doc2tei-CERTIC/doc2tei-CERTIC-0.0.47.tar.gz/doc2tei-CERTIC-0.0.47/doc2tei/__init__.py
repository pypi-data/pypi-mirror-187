...
def _log_xml_error(saxon_output: bytes, logger: logging.Logger) -> bool:
    has_error = False
    control_xml = ET.fromstring(saxon_output.decode("utf-8"))
    for error_node in control_xml.findall(".//{http://www.tei-c.org/ns/1.0}ERROR"):
...
def _process_doc(
    ...
    # Normalize, step 1
    success, output = _call_saxon(
        xsl_path="normalisation/normalizea.xsl",
    ...
    # Normalize step 2
    success, output = _call_saxon(
        xsl_path="normalisation/normalizeb.xsl",
    ...
    # Normalize, step 3
    success, output = _call_saxon(
        xsl_path="normalisation/normalizec.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="floatingText/figure.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="floatingText/floatingText.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="floatingText/hierarchize.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="control/control-hierarchy.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="hierarchize/hierarchize.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="totei-modules/totei.xsl",
    ...
    #
    success, output = _call_saxon(
        xsl_path="control/control-styles.xsl",
        source_path=f"{doc_file_no_extension}_07_tei.xml",
        destination_path=f"{doc_file_no_extension}_08_tei_metopes.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    if validation_schema == "metopes":
        ...
        #
        success, output = _call_saxon(
            xsl_path="toOpenedition/toOpenedition.xsl",
            source_path=f"{doc_file_no_extension}_08_tei_metopes.xml",
            destination_path=f"{doc_file_no_extension}_09_tei_openedition.xml",
            logger=logger,
            transient_files=transient_files,
        )
        if not success:
