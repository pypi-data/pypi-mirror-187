...
def rewrite_pipeline():
    for df_name, query in cache_outputs.items():
        ...
        headers.rewrite_var_definition(df_name, query, NOTEBOOK_CODE)
        NOTEBOOK_CODE = NOTEBOOK_CODE + get_sql_conn_code(query=query, df_name=df_name)
        print('*********************************')
        print(NOTEBOOK_CODE)
...
def fetch_notebook_code():
    ...
    NOTEBOOK_CODE = headers.get_code_from_notebook(notebook_str)
