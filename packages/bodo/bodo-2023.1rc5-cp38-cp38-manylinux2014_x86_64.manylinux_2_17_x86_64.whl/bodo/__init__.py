...
def _global_except_hook(exctype, value, traceback):
    ...
    uft__icdu = MPI.COMM_WORLD
    sheh__sxh = uft__icdu.Get_rank()
    sdoa__hjjse = 3.0
    rajyh__uqr = True
    vclgi__kxmid = uft__icdu.Ibarrier()
    arfyh__iiqk = time.time()
    while time.time() - arfyh__iiqk < sdoa__hjjse:
        time.sleep(0.1)
        if vclgi__kxmid.Test():
            rajyh__uqr = False
            break
    try:
        ...
        if rajyh__uqr:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {sheh__sxh}. \n')
...
def set_numba_environ_vars():
    if (revg__yiicj := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = revg__yiicj
        os.environ['NUMBA_CACHE_DIR'] = revg__yiicj
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    cshrj__ths = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(cshrj__ths)
