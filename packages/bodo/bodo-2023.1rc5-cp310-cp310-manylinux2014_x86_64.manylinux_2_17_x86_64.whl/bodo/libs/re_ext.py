...
def box_regex_flag(typ, val, c):
    frbq__eajwx = c.box(types.int64, val)
    yugoe__hsh = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(yugoe__hsh, (frbq__eajwx,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    frbq__eajwx = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, frbq__eajwx)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    awy__liozp = context.get_python_api(builder)
    wxu__uul = builder.icmp_signed('==', val, awy__liozp.borrow_none())
    with builder.if_then(wxu__uul):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    rdrs__djdjt = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(rdrs__djdjt)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        zuzr__ozp = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if zuzr__ozp > 1:
            typ = types.List(types.Tuple([string_type] * zuzr__ozp))
...
def overload_match_group(m, *args):
    ...
    nsh__qzwm = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            gka__sstxe = args[0]
            with numba.objmode(out=nsh__qzwm):
                out = m.group(gka__sstxe)
            return out
        return _match_group_impl_one
    pbf__oze = 'tuple_str_{}'.format(len(args))
    setattr(types, pbf__oze, types.Tuple([nsh__qzwm] * len(args)))
    oxvqh__lsuk = ', '.join('group{}'.format(ebz__suupn + 1) for ebz__suupn in
        range(len(args)))
    nwwg__ego = 'def _match_group_impl(m, *args):\n'
    nwwg__ego += '  ({}) = args\n'.format(oxvqh__lsuk)
    nwwg__ego += "  with numba.objmode(out='{}'):\n".format(pbf__oze)
    nwwg__ego += '    out = m.group({})\n'.format(oxvqh__lsuk)
    nwwg__ego += '  return out\n'
    aow__otrg = {}
    exec(nwwg__ego, globals(), aow__otrg)
    udq__pjwxx = aow__otrg['_match_group_impl']
    return udq__pjwxx
...
def overload_match_groups(m, default=None):
    jjkbg__pilb = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=jjkbg__pilb):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(aggj__jsc is None for aggj__jsc in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=nsh__qzwm):
