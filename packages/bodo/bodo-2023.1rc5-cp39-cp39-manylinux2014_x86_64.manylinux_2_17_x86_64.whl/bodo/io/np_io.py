...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        spnq__upnjy = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, spnq__upnjy)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            njrdm__mkn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(njrdm__mkn.dtype)
            file_write(unicode_to_utf8(fname), njrdm__mkn.ctypes, 
                dtype_size * njrdm__mkn.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            njrdm__mkn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(njrdm__mkn.dtype)
            ekwnd__pmroa = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(njrdm__mkn.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), njrdm__mkn.ctypes,
                start, count, ekwnd__pmroa)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            vbz__yckei = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                vbz__yckei = min(vbz__yckei, count * dtype_size)
            if vbz__yckei < 0:
                return -1
            return vbz__yckei
        return impl
