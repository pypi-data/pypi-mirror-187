...
def _install_mpl_types():
    puiuw__xgd = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for wpyq__wmd, kbft__qlsq in puiuw__xgd:
        install_py_obj_class(types_name=wpyq__wmd, python_type=kbft__qlsq,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    ekf__rgc = ', '.join(f'e{plolx__nlys}' for plolx__nlys in range(len(args)))
    if ekf__rgc:
        ekf__rgc += ', '
    xsklg__lmn = ', '.join(f"{zab__dtcdp} = ''" for zab__dtcdp in kws.keys())
    fbd__padem = 'matplotlib_obj, ' if obj_typ is not None else ''
    wdzgr__xune = f'def mpl_stub({fbd__padem} {ekf__rgc} {xsklg__lmn}):\n'
    wdzgr__xune += '    pass\n'
    wta__ehblh = {}
    exec(wdzgr__xune, {}, wta__ehblh)
    oclvx__qcog = wta__ehblh['mpl_stub']
    gmy__utsyx = numba.core.utils.pysignature(oclvx__qcog)
    exb__imtj = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(exb__imtj)).replace(pysig=
        gmy__utsyx)


def generate_axes_typing(mod_name, nrows, ncols):
    lcy__rhgyj = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(lcy__rhgyj.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(lcy__rhgyj.format(mod_name, 'ncols'))
    npybp__ijxuy = get_overload_const_int(nrows)
    mbf__vnmf = get_overload_const_int(ncols)
    if npybp__ijxuy < 1:
        raise BodoError(lcy__rhgyj.format(mod_name, 'nrows'))
    if mbf__vnmf < 1:
        raise BodoError(lcy__rhgyj.format(mod_name, 'ncols'))
    if npybp__ijxuy == 1 and mbf__vnmf == 1:
        ynhm__aayig = types.mpl_axes_type
    else:
        if mbf__vnmf == 1:
            iptge__shhro = types.mpl_axes_type
        else:
            iptge__shhro = types.Tuple([types.mpl_axes_type] * mbf__vnmf)
        ynhm__aayig = types.Tuple([iptge__shhro] * npybp__ijxuy)
    return ynhm__aayig


def generate_pie_return_type(args, kws):
    tjtrx__ydgs = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if tjtrx__ydgs == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    kjp__wparc = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(kjp__wparc):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(kjp__wparc):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        rwamh__chfd = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, rwamh__chfd]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    rwamh__chfd = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    wpyq__wmd = str(rwamh__chfd)
    if not hasattr(types, wpyq__wmd):
        wpyq__wmd = f'objmode_type{ir_utils.next_label()}'
        setattr(types, wpyq__wmd, rwamh__chfd)
    wdzgr__xune = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{wpyq__wmd}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    wta__ehblh = {}
    exec(wdzgr__xune, {'numba': numba, 'np': np}, wta__ehblh)
    impl = wta__ehblh['impl']
    return impl
