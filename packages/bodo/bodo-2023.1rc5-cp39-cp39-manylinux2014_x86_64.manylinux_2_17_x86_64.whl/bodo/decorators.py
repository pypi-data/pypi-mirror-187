...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        zsgz__hlop = self.overloads[sig]
        htjod__poub = zsgz__hlop.metadata.get('distributed_diagnostics', None)
        if htjod__poub is None:
            dlcvk__gxha = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(dlcvk__gxha)
        htjod__poub.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        kxna__okui = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(kxna__okui)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        oqy__blvy = options.pop('distributed')
        pipeline_class = (pipeline_class if oqy__blvy else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        ilmka__pgm = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if ilmka__pgm else
            pipeline_class)
    zos__ley = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(zos__ley, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(zos__ley)
        else:
            return master_mode_wrapper(zos__ley)
    else:
        return zos__ley
