...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        qdy__hkom = []
        yxf__ydtb = h5g_get_num_objs(obj_id)
        for ygtu__wwoh in range(yxf__ydtb):
            fhpu__poyep = h5g_get_objname_by_idx(obj_id, ygtu__wwoh)
            qdy__hkom.append(fhpu__poyep)
        return qdy__hkom
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    kphfa__dwa = parse_dtype(dtype)
    uqg__fbhd = np.int32(numba_to_c_type(kphfa__dwa))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            uht__ggvz = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    befrw__porgu = tuple_to_ptr(starts)
    waus__ttfz = tuple_to_ptr(counts)
    wbfuh__jzsc = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, befrw__porgu,
        waus__ttfz, is_parallel, out_arr.ctypes, wbfuh__jzsc)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    waus__ttfz = tuple_to_ptr(counts)
    wbfuh__jzsc = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, befrw__porgu,
        waus__ttfz, is_parallel, out_arr.ctypes, wbfuh__jzsc)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        mwd__dyqlu = types.Array(dtype, ndim, 'C')
        ozmc__rnyt = args[3]
        mwd__dyqlu = self.context.resolve_function_type(operator.getitem, [
            mwd__dyqlu, ozmc__rnyt], {}).return_type.copy(layout='C')
        return signature(mwd__dyqlu, *args)
...
def get_filter_read_indices(bool_arr):
    txtet__jtr = bool_arr.nonzero()[0]
    jyt__bsfq = bodo.libs.distributed_api.get_rank()
    wjs__nvo = bodo.libs.distributed_api.get_size()
    ljylq__kbs = np.empty(wjs__nvo, np.int64)
    lcp__jftw = len(bool_arr)
    bodo.libs.distributed_api.allgather(ljylq__kbs, lcp__jftw)
    egy__qyfqg = ljylq__kbs.cumsum()[jyt__bsfq] - lcp__jftw
    txtet__jtr += egy__qyfqg
    uyuf__zuigs = bodo.libs.distributed_api.dist_reduce(len(txtet__jtr), np
        .int32(sum_op))
    fbgqy__miflr = bodo.libs.distributed_api.gatherv(txtet__jtr)
    if jyt__bsfq == 0:
        dsgdh__otadl = fbgqy__miflr
    else:
        dsgdh__otadl = np.empty(uyuf__zuigs, txtet__jtr.dtype)
    bodo.libs.distributed_api.bcast(dsgdh__otadl)
    dwiis__ablff = bodo.libs.distributed_api.get_start(uyuf__zuigs,
        wjs__nvo, jyt__bsfq)
    etx__zuug = bodo.libs.distributed_api.get_end(uyuf__zuigs, wjs__nvo,
        jyt__bsfq)
    return dsgdh__otadl[dwiis__ablff:etx__zuug]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        tic__xltz = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], tic__xltz)
        return builder.bitcast(tic__xltz, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    waus__ttfz = tuple_to_ptr(counts)
    wbfuh__jzsc = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, befrw__porgu,
        waus__ttfz, is_parallel, out_arr.ctypes, wbfuh__jzsc, read_indices.
