...
public class Server_3{
    public static void main(String[] args) throws Exception {
        ...
        ms.joinGroup(InetAddress.getByName("239.2.1.3"));

        byte[] b = new byte[1024];

        while(true){
            Thread.sleep(5000);
            String s = "Pranav Calling";
            b = s.getBytes();
            DatagramPacket dp = new DatagramPacket(b,b.length,InetAddress.getByName("239.2.1.3"),5000);
            ms.send(dp);
