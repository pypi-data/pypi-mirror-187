    ...
    if ALLOW_PARAM == True:
        ...
        if len(lst_OBJECT_DETECTED)!=0:
            ...
            for deployment in list(material_execute):
                status_wrapper_completed, get_exception_message_handler, res_rolloutrestart, res_slUp, res_slDown, lst_failed = fCls2.execute_wrapperRollout_restartDeployment(deployment['namespace'],deployment['case deployment name'], file)
                ...
                if len(lst_failed)!=0:
                    failed = {}
                    failed.clear()
                    failed['identifier'] = f'{deployment["namespace"]}::{deployment["case deployment name"]}'
                    failed['position'] = f'For Loop: ACTION ROLLOUT RESTART DEPLOYMENT ( ssh exec )' 
                    failed['details'] = html.escape(str(lst_failed))
                    lst_sshexecparamiko_failed.append(failed)
                if status_wrapper_completed == False:
                    failed = {}
                    failed.clear()
                    failed['identifier'] = f'{deployment["namespace"]}::{deployment["case deployment name"]}'
                    failed['position'] = f'For Loop: ACTION ROLLOUT RESTART DEPLOYMENT ( wrapper )' 
                    failed['message'] = html.escape(get_exception_message_handler)
                    lst_sshexecparamiko_failed.append(failed)
