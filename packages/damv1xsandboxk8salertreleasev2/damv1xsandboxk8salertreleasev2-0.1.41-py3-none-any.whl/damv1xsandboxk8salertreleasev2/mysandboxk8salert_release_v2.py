    ...
    if ALLOW_PARAM == True:
                ...
                if status_wrapper_completed == False:
                    ...
                    if "'dict'" in str(type(res)):
                        if len(res)!=0:
                            lst_sshexecparamiko_failed.append(res)
                    ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        lst_event_result_rollout_restart_filter_successful = list(filter(lambda x: x['response rollout restart'] == 'False', lst_event_result_rollout_restart))
        lst_deployment_rollout_restart = []
        print('CHECK len lst_event_result_rollout_restart_filter_successful:', len(lst_event_result_rollout_restart_filter_successful))
        if len(lst_event_result_rollout_restart_filter_successful)!=0:
            for event in lst_event_result_rollout_restart_filter_successful:
                line_deploy_restart = tCls.tmplt_telemsg_str_restart_deploy.value.format(\
                    value_str_deploy = event['deployment'], \
                    value_pods_scaledup = event['pod scaled up name'])
                lst_deployment_rollout_restart.append(line_deploy_restart)
        ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if len(lst_dtect_obj)!=0:
            ...
            namespace_deployment_success_restart = ''
            lines_deploymeny_success_restart = ''
            if len(lst_deployment_rollout_restart)!=0:
                count_deployment_success_restart = len(lst_event_result_rollout_restart_filter_successful)
                namespace_deployment_success_restart = _namespace
                lines_deploymeny_success_restart = '\n  \=\| '.join(lst_deployment_rollout_restart)
            ...
            telemsg = tCls.tmplt_telemsg_v2.value.format(\
                        value_str_messagename = mpl.telegram.escape_strparse_markdownv1(nameof_msg_rpt),\
                        value_str_contextid = mpl.telegram.escape_strparse_markdownv1(contexid),\
                        value_str_StartDateTime7 = mpl.telegram.escape_strparse_markdownv1(START_DATE_TIME7),\
                        value_str_ns = mpl.telegram.escape_strparse_markdownv1(_namespace), \
                        ...
                        value_str_deployment_ns = mpl.telegram.escape_strparse_markdownv1(str(namespace_deployment_success_restart)),\
                        value_str_deployment_rollout_restart = mpl.telegram.escape_strparse_markdownv1(str(lines_deploymeny_success_restart)),\
                        value_str_dtect_obj = dtect_obj,\
                        value_str_url_shareable = evrnt_urlshareable)
