    ...
    if ALLOW_PARAM == True:
        ...
        if len(lst_event_result_rollout_restart_filter_successful)!=0:
            for event in lst_event_result_rollout_restart_filter_successful:
                line_deploy_restart_successful = tCls.tmplt_telemsg_str_restart_deploy.value.format(\
        ...
        if len(lst_event_result_rollout_restart_filter_fail)!=0:
            for event in lst_event_result_rollout_restart_filter_fail:
                line_deploy_restart_fail = tCls.tmplt_telemsg_str_restart_deploy.value.format(\
                    value_str_deploy =  mpl.telegram.escape_strparse_markdownv1(event['deployment']), \
                    value_pods_scaledup =  mpl.telegram.escape_strparse_markdownv1(event['pod scaled up name'])\
                    )
                lst_event_result_rollout_restart_filter_fail.append(line_deploy_restart_fail)
        ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if len(lst_dtect_obj)!=0:
            ...
            dtect_obj = '\n  \-\| '.join(lst_dtect_obj)
            namespace_deployment_restart = _namespace;
            count_deployment_success_restart = 0; lines_deploymeny_success_restart = mpl.telegram.escape_strparse_markdownv1('')
            count_deployment_fail_restart = 0; lines_deploymeny_fail_restart = mpl.telegram.escape_strparse_markdownv1('')
            ...
            if len(lst_event_result_rollout_restart_filter_fail)!=0:
                count_deployment_fail_restart = len(lst_event_result_rollout_restart_filter_fail)
                lines_deploymeny_fail_restart = '\n  \=\| '.join(lst_event_result_rollout_restart_filter_fail)
            ...
            telemsg = tCls.tmplt_telemsg_v2.value.format(\
                        ...
                        value_str_ns = mpl.telegram.escape_strparse_markdownv1(_namespace), \
                        value_str_count_dtect_obj = mpl.telegram.escape_strparse_markdownv1(str(len(lst_dtect_obj))),\
                        value_str_deployment_ns = mpl.telegram.escape_strparse_markdownv1(str(namespace_deployment_restart)),\
                        ...
                        value_str_dtect_obj = dtect_obj,\
                        value_str_url_shareable = evrnt_urlshareable)
            print('CHECK telemsg template release:')
            print(telemsg)
