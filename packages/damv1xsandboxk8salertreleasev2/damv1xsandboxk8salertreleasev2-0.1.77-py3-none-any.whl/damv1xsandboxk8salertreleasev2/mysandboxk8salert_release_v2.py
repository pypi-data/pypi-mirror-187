    ...
    if ALLOW_PARAM == True:
        ...
        if len(lst_grplines) != 0:
            ...
            value_evrnt_deploy_success = tCls.tmplt_evrnt_str_deploy_succeed.value.format(\
                value_str_namespace = html.escape('sit'), \
                value_str_deployment = html.escape('deploymentofname'), \
                value_str_scaledup_podname = html.escape('podname_new_from_scaledup'), \
                value_str_response_scaledup = html.escape(str(True))
            )

            wrapper_evrnt_restart_deploy_success = tCls.tmplt_evrnt_restart_deploy_succeed.value.format(\
                value_str_lst_deploy_succeed = value_evrnt_deploy_success
                )
