    ...
    if ALLOW_PARAM == True:
        ...
        if len(lst_dtect_obj)!=0:
            ...
            telemsg = tCls.tmplt_telemsg_v2.value.format(\
                        ...
                        value_str_ns = mpl.telegram.escape_strparse_markdownv1(_namespace), \
                        value_str_count_dtect_obj = mpl.telegram.escape_strparse_markdownv1(str(len(lst_dtect_obj))),\
                        value_str_count_deployment_success_restart = mpl.telegram.escape_strparse_markdownv1(str(len(lst_event_result_rollout_restart))),\
                        value_str_deployment_ns = mpl.telegram.escape_strparse_markdownv1(str(_namespace)),\
                        value_str_deployment_rollout_restart = mpl.telegram.escape_strparse_markdownv1(str('Test check deployment')),\
                        value_str_dtect_obj = dtect_obj,\
                        value_str_url_shareable = evrnt_urlshareable)
