...
def main():
  ...
  if(len(sys.argv)<5):
    print(f"incorrect number of parameters are passed")
    sys.exit()

  try:
    input_data=pd.read_csv(inputdatafile)
  except:
    print(f"input data file selected is not found")
    sys.exit()

  if(len(list(input_data.columns))<=2):
    print(f"input data file must contain 3 or more columns")
    sys.exit()
  ...
  try:
    ...
    print(f"weights or Impacts are not seperated by commas")
    sys.exit()
  ...
  if len(weights)!=cols:
    print(f"no. of weights are sparse")
    sys.exit()
        
  if len(impacts)!=cols:
    print(f"no. of impacts are sparse")
    sys.exit()
  ...
  res.to_csv(resultfilename,index=False)
