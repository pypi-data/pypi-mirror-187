...
class Postgres_Handler:
    ...
    def update_record(self, kwargs):
        ...
        for key, value in kwargs.items():
            if key.lower() == "id" or key.lower() == table_name: continue
        ...
        values_dict['id'] = kwargs['id']
        
        sql_query = sql.SQL(query_string).format(sql.Identifier(table_name = kwargs[self.table_name_key]))
        
        results = self.__run_query(sql_query)
        return self.__format_results(results)
    ...
    def deleteRow(self, **kwargs):
        ...
        sql_query = sql.SQL(query_string).format(
            table_name = sql.Identifier(kwargs[table_name_key]),
            Id= sql.Identifier(kwargs[id_key])
        )
