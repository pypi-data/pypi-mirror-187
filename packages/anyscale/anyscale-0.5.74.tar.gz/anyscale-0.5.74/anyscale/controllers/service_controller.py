...
class ServiceConfig(BaseHAJobConfig):
    ...
    @root_validator
    def contains_either_entry_point_or_serve_config(cls, values) -> Dict[str, Any]:
        is_entrypoint_present = bool(values.get("entrypoint"))
        is_serve_config_present = bool(values.get("ray_serve_config"))
        assert (
            is_entrypoint_present != is_serve_config_present
        ), "Either entrypoint or ray serve config needs to be present for the Service config"
        return values
