...
def create_schema(df, spark_df_dtypes):
    """
    ...
    index = [{"name": "_pysparkgui_index_column", "type": "int64"}]

    spark_df_dtypes_dict = {}
    for column, dtype in spark_df_dtypes:
        spark_df_dtypes_dict[column] = dtype

    columns = [
            {
                "name": column,
                # "type": str(df[column].dtype)
                "type": spark_df_dtypes_dict[column]
                # "type": "boolean"
            } for column in df.columns
        ]
    schema["fields"] = index + columns

    schema["primaryKey"] = ["_pysparkgui_index_column"]
...
def create_grid_data_for_normalized_df(df):
    ...
    for row_index, row in df.iterrows():
        row_dict = {}
        row_dict["_pysparkgui_index_column"] = str(row_index)
...
def create_df_json(df, spark_df_dtypes):
    """Create a JSON string from a dataframe."""
    df_json = {}
    df_json["schema"] = create_schema(df, spark_df_dtypes)
...
class BamGridWidget(widgets.DOMWidget):
    ...
    _view_name = Unicode("PysparkguiGridView").tag(sync=True)
    _model_name = Unicode("PysparkguiGridModel").tag(sync=True)
    ...
    def update_df(self, df, df_column_indices, spark_df_dtypes):
        self.df = df
        self.df_column_indices = df_column_indices
        self.spark_df_dtypes = spark_df_dtypes
    ...
    def _update_table(
        ...
        if update_columns:
            ...
            if type(df.index) == pd.MultiIndex:
                ...
                if df.index.name:
                    ...
                    col_name = "_pysparkgui_index_column"
                    self._primary_key_display[col_name] = ""
        ...
        df_json = create_df_json(df, self.spark_df_dtypes)
    ...
    def _handle_qgrid_msg_helper(self, content):
        """Handle incoming messages from the PysparkguiGridView"""
        ...
        if content["type"] == "edit_cell":
            ...
            # Note(PROD-27100): PysparkguiGridView.slick_grid.onViewportChanged always expects a reply
