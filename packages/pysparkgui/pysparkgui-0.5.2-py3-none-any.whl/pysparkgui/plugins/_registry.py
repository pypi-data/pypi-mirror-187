...
def register(plugin_base_class, plugin_class, index=None, before=None, after=None):
    ...
    if plugin_name in _all_plugins:
        ...
        from pysparkgui.helper import AuthorizedPlugin, notification
        ...
        if is_authorized(plugin_class):
            ...
            display(
                notification(
                    f"Could not register plugin <b>{plugin_class.__name__}</b> because this is a Pro feature. If you want to use plugins, please <a href='https://pysparkgui.com/pricing/' target='_blank'>get a Pro license</a>",
                    type="error",
                )
            )
