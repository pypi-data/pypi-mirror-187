...
# also, we can use an obscure S3 bucket for shipping pysparkgui (and updates)
...
from pysparkgui._path import (
    pysparkgui_LIBRARY_CONFIG_PATH,
    pysparkgui_LIBRARY_INTERNAL_CONFIG_PATH,
)
...
from pysparkgui.helper import (
    ...
    # calling the log methods during init of pysparkgui leads to circular dependency errors
...
from pysparkgui._server_api import (
...
class BackgroundUpdatePolicy_v1(Policy_v1):
    ...
    def is_valid(self):
        ...
        #   - if the user uses pysparkgui after 3 weeks but is offline, license should still work. But then, the offline period should start.
...
class ExpirationPolicy_v1(Policy_v1):
    ...
    def _show_expiration_message(self, outlet):
        ...
        email = self.auth.get_user_email()
        message = notification(
            f"""Sorry, your license with the key '{self.data['key']}' expired. Please activate another license or <a href='https://pysparkgui.com/pricing/' target='_blank'>purchase a new license</a>""",
            type="error",
        )
...
class FloatingSeatPolicy_v1(Policy_v1):
    ...
    def _get_my_seat(self):
        ...
        """
        client_id = auth._browser_info.get("pysparkgui_client_id")
...
class UnimplementedLicensePolicy_v1(Policy_v1):
    """Handles the case the pysparkgui client doesn't support the license type on the server."""
    ...
    def get_validation_error(self):
        return notification(
            "The current pysparkgui version is outdated and does not support the license that you want to activate. Please upgrade pysparkgui to the newest version. If you still see the error after upgrading pysparkgui please contact us via support@8080labs.com",
            type="error",
        )
...
Please execute your Jupyter cell again in order to see the pysparkgui widget.
Have a great day!
"""
    ...
    def get_license_user_info(self):
        ...
        if schema == TWO_WEEK_TRIAL_LICENSE_v1:
            ...
            if remaining_days == 0:
                ...
                return notification(
                    f"Your trial expires in {remaining_days} day(s). It’s the perfect time to upgrade! Buy your license key <a href='https://pysparkgui.8080labs.com/pricing/' target='_blank'>here</a>.",
                    type="info",
                )
            elif remaining_days < 7:
                return notification(
                    f"Your trial expires in {remaining_days} day(s). If you want to upgrade you can buy a license key <a href='https://pysparkgui.8080labs.com/pricing/' target='_blank'>here</a>.",
                    type="info",
                )
...
class CredentialsStorage:
    ...
    def __init__(self, auth_service):
        ...
        self._public_credentials_file = pysparkgui_LIBRARY_CONFIG_PATH / "LICENSE"
        self._internal_credentials_file = (
            pysparkgui_LIBRARY_INTERNAL_CONFIG_PATH / "ek261"
        )
        ...
        # because this method is run during normal pysparkgui import
    ...
    def _maybe_create_credential_folders(self):
        pysparkgui_LIBRARY_CONFIG_PATH.mkdir(parents=True, exist_ok=True)
        pysparkgui_LIBRARY_INTERNAL_CONFIG_PATH.mkdir(parents=True, exist_ok=True)
...
class AuthorizationService:
    """
    Handles parts of the authorization process, e.g. deciding if user can use pysparkgui for free.
    """
    ...
    def _is_free_user(self) -> bool:
        """Returns True if the user may use pysparkgui for free."""
        whitelist = [
            self.is_databricks(),
    ...
    def has_unlimited_plugins(self):
        """Returns True if the user may use pysparkgui plugins."""
