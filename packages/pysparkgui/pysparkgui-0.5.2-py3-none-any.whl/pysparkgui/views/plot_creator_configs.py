...
import pysparkgui._environment as env
...
from pysparkgui.widgets.block_manager import Block
...
from pysparkgui.widget_combinations import (
    ListInput,
    DictInput,
    IndentationDictItem,
    SideBySideDictItem,
...
from pysparkgui.views import plotly_color_themes
from pysparkgui.views import plotly_marker_options
...
# e.g. currently this is the pattern pysparkgui-width-{LABEL_WIDTH}
# and thus pysparkgui-width-180px
LABEL_WIDTH = "180px"
ALL_COLUMNS = "pysparkgui_all_columns_placeholder"

FIGURE = "figure__pysparkgui_template_name"
...
class PlotCreatorBlock(Block):
    ...
    def __init__(self, *args, plot_creator=None, focus_after_init=False, **kwargs):
        ...
        self.name_label = widgets.HTML(f"{self.name}").add_class(
            f"pysparkgui-width-{LABEL_WIDTH}"
        )
    ...
    def set_content(
        ...
        *embeddables,
        ...
        core_content.add_class("pysparkgui-overflow-visible")
        ...
        hbox.add_class("pysparkgui-overflow-visible")
...
class ResultRenderer(PlotCreatorBlock):
    ...
    def render(self):
        self.set_content(
            self.code_outlet, self.result_outlet, css_class="pysparkgui-width-100pct"
        )
        ...
        if self._current_render_id == render_id:
            try:
                ...
                symbols_with_masked_df[
                    df_manager.get_latest_df_name()
                ] = df_manager.get_latest_df()
    ...
    def _get_stacktrace(self, exception):
        ...
        output.add_class("pysparkgui-output-wrap-text")
...
class Sampling(Configuration):
    ...
    def _recommend_sampling(self):
        df = self.plot_creator.df_manager.get_latest_df()
...
class ColorDiscreteTheme(Configuration):
    ...
    def init_configuration(self):
        ...
        self.mode_outlet.add_class("pysparkgui-overflow-visible")
        ...
        self.reverse_scale.add_class("pysparkgui-checkbox")
...
class ShapeDrawLine(Configuration):
    ...
    def render(self):
        ...
        linestyle_line.add_class("pysparkgui-overflow-visible")
...
class AnyAxisLinearScale(Configuration):
    ...
    def init_configuration(self):
        ...
        self.input.add_class("pysparkgui-checkbox")
        self.input.observe(
            lambda _: self.plot_creator.request_figure_update(), names="value"
        )
...
class CandlestickProperties(DataframeConfig, Configuration):
    ...
    def get_options(self):
        return list(self.plot_creator.df_manager.get_latest_df().columns)
...
class FakeDoorDemandTest(HTMLHint):
    ...
    def __init__(self, *args, **kwargs):
        ...
        <a href="mailto:pysparkgui-feedback@databricks.com?subject=Feature request '{feature_name}'&body=Hi, I would like to request the plotting feature '{feature_name}'. It is important to me because ..." target="_blank">pysparkgui-feedback@databricks.com</a>"""
...
class XAxisWithMaybeSortColumn(XAxis):
    def __init__(self, *args, **kwargs):
        ...
        self.sort_column.add_class("pysparkgui-checkbox")
...
class HoverInfoColumns(MultipleDataframeColumnsKwarg):
    ...
    def get_code(self):
        if self.columns.value is []:
            ...
            return Code(
                kwargs=f"{self.kwarg}={list(self.plot_creator.df_manager.get_latest_df().columns)}"
            )
        else:
            return Code(kwargs=f"{self.kwarg}={self.columns.value}")
    ...
    def get_options(self):
        ...
        single_columns = [
            (name, name)
            for name in self.plot_creator.df_manager.get_latest_df().columns
        ]
