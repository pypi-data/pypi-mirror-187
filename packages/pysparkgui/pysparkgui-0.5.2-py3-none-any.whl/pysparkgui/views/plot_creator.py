...
from pysparkgui.widgets.block_manager import BlockManager
from pysparkgui.widgets import Singleselect

from pysparkgui.plugins import ViewPlugin, create_plugin_base_class

from pysparkgui.views.plot_creator_configs import *
...
class Figure:
    ...
    def __bam_figure_got_selected__(self):
        """
        This pysparkgui dunder method is an internal lifecycle method that is called when the figure got selected

        Some more infos about pysparkgui dunder methods:
        its usage communicates the following:
        this is like a public method for this class BUT
        the method shall only be called by pysparkgui and not by the user
...
{figure_name}
        ...
        code = replace_code_placeholder(
            code, old_df_name=self.plot_creator.df_manager.get_latest_df_name()
        )
        return code
    ...
    def _get_figure_code(self, code_results):
        ...
        df_name = self.plot_creator.df_manager.get_latest_df_name()
