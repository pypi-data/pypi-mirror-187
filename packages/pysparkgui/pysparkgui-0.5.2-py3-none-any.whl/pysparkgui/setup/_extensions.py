...
def install_nbextensions() -> None:
    """
    Installs the Jupyter Notebook extensions that are required for pysparkgui
    """
    print("Trying to install pysparkgui nbextension...")

    try:
        from notebook import nbextensions
    except ImportError:
        print(
            "Could not install pysparkgui Jupyter Notebook extension because Jupyter Notebook is not available"
        )
        return

    extensions = ["pysparkgui", "ipyslickgrid", "widgetsnbextension", "plotlywidget"]
    for extension in extensions:
        nbextensions.install_nbextension_python(extension, user=True)
        nbextensions.enable_nbextension_python(extension)

    print("Finished installing the pysparkgui Jupyter Notebook nbextension")
    print("Please reload your Jupyter notebook browser window")


def install_labextensions() -> None:
    """
    Installs the Jupyter Lab extensions that are required for pysparkgui
    """
    print("Trying to install pysparkgui labextensions...")

    try:
        from jupyterlab import commands
    except ImportError:
        print(
            "Could not install pysparkgui Jupyter Lab extension because Jupyter Lab is not available"
        )
        return
    ...
    def get_jupyterlab_version_pattern(version_object) -> str:
        ...
        raise Exception(
            "pysparkgui does not support JupyterLab version %s. pysparkgui only supports JupyterLab>=1.0. If you have troubles, please reach out to support@8080labs.com"
            % major_minor_version
        )
    ...
    extensions = [
        ...
        "pysparkgui@%s" % __widgets_version__,
    ]
...
def install_extensions() -> None:
    """
    Installs the extensions that are required for pysparkgui for both Jupyter Notebook and Lab
    """
    print(
        "Starting to install pysparkgui extensions for Jupyter Notebook and Jupyter Lab"
    )
    print("")
    ...
    print("")
    print("Finished installing the pysparkgui Jupyter extensions")
