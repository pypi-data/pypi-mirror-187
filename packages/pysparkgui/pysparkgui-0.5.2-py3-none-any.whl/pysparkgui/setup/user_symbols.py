...
def get_user_symbols():
    ...
    if env.TESTING_MODE:
        # When we are running the pysparkgui tests, we need to get the symbols from another frame
        ...
        # When pysparkgui is run by the user, we get the symbols from the __main__ script
        for index, item in enumerate(inspect.stack()):
            try:
                name = item[0].f_globals["__name__"]
                if name == "__main__":
