...
from pysparkgui.helper import log_jupyter_action, log_setup
...
log_jupyter_action("other", "JupyterCell", "import pysparkgui")


IPYTHON_STARTUP_FOLDER = Path.home() / ".ipython" / "profile_default" / "startup"
STARTUP_FILE = IPYTHON_STARTUP_FOLDER / "pysparkgui_autoimport.py"
...
def _create_auto_startup_file(auto_import=False, extend_pandas_df=False):
    auto_import_code = "import pysparkgui as bam"
    if not auto_import:
...
# if you dont want to auto-import pysparkgui, you have two options:
# 0) if you want to disable the auto-import but sometimes keep using pysparkgui
#    you can uncomment the import statement below
# 1) if you never want to use pysparkgui again, you can delete this file
...
def _update_startup_file(auto_import=False, extend_pandas_df=False):
    ...
    try:
        ...
        maybe_print(
            "pysparkgui error: there was an error when updating the startup file",
            should_print=True,
        )
    return True
...
def _enable(print_messages=True):
    ...
    maybe_print(
        "Success: the pysparkgui extension was enabled successfully. You can disable it via 'bam.disable()'. You will now see a magic pysparkgui button when you display your dataframes, for example via 'df'",
        print_messages,
    )
    return True
...
def enable(print_messages=True):
    """
    When this method is called, pysparkgui will add the interactive Dataframe view to pandas.DataFrames.
...
def _enable_rich_pandas_df():
    ...
    # we could use this call to set a global flag that marks that pysparkgui was auto-imported
    log_jupyter_action("other", "ipython startup", "auto_import pysparkgui")


def _disable(print_messages=True):
    from pysparkgui.setup.ipython_display import reset_pandas_ipython_display
    ...
    maybe_print(
        f"The pysparkgui extension was disabled. You can enable it again via 'bam.enable()'. In case that pysparkgui was not helpful to you, we are sorry and would like to fix this. Please write us a quick mail to info@8080labs.com so that we can serve you better in the future. Best regards, Tobias and Florian",
        print_messages,
    )
    return True
...
def disable(print_messages=True):
    """
    When this method is called, pysparkgui's interactive Dataframe view for pandas.DataFrames will be removed/disabled.
...
def _maybe_add_autostartup_file():
    if IPYTHON_STARTUP_FOLDER.exists() and (not STARTUP_FILE.exists()):
        try:
            ...
            print("pysparkgui error: unable to add autostartup file")
