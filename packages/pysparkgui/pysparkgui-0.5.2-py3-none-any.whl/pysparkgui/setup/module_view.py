...
from pysparkgui.views.data_loader import (
    CSVLoader,
    CSVOptions,
    ExcelLoader,
    ExcelOptions,
...
from pysparkgui.plugins import LoaderPlugin
...
def setup_module_view(module_name):
    """
    Change the representation of the module (e.g. pysparkgui, bam) to be an interactive widget instead of just a string to the module's file

    :param module_name: str - the name of the module e.g. usually "pysparkgui"
    ...
    sys.modules[module_name] = pysparkguiModuleWrapper(original_module)


class pysparkguiModuleWrapper:
    """
    ...
    def _ipython_display_(self, *args, **kwargs):
        ...
        display(pysparkguiModuleWindow(self))


class pysparkguiModuleWindow(widgets.VBox):
    """
    The main class for the pysparkgui module representation that holds the output window and the modal outlet.
    """
...
class AboutInfoToggle(widgets.VBox):
    """
    This widget contains all the information about pysparkgui we don't want to display to
    ...
    def __init__(self, module_wrapper, module_outlet):
        ...
        self.about_info = [
            self._grey_html(f"pysparkgui version {__version__}"),
