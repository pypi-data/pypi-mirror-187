...
from pysparkgui.helper import notification, VSpace, execute_asynchronously
from pysparkgui.widgets import Singleselect, Multiselect, Button, Text
from pysparkgui.edaviz.base import AsyncEmbeddable, get_loading_widget
...
class UnivariateTimeseriesPlot(AsyncEmbeddable):
    ...
    def _init_gui(self):
        ...
        count_per.add_class("pysparkgui-element-next-to-selectize")

        of_column = widgets.HTML(f"""of <b>{self.state["datetime_column"]}</b>""")
        of_column.add_class("pysparkgui-element-next-to-selectize")

        frequency_line = widgets.HBox(
            [count_per, self.frequency_amount, self.frequency_type, of_column]
        )
        frequency_line.add_class("pysparkgui-overflow-visible")
...
class BivariateTimeseriesPlot(AsyncEmbeddable):
    ...
    def _init_gui(self):
        ...
        per_label.add_class("pysparkgui-element-next-to-selectize")

        of_target_column_per = widgets.HTML(f"""of <b>{self.target_column}</b>""")
        of_target_column_per.add_class("pysparkgui-element-next-to-selectize")

        of_datetime_column = widgets.HTML(f"""of <b>{self.datetime_column}</b>""")
        of_datetime_column.add_class("pysparkgui-element-next-to-selectize")

        aggregation_line = widgets.HBox([self.aggregations, of_target_column_per])
        aggregation_line.add_class("pysparkgui-overflow-visible")
