...
from pysparkgui._authorization import auth as authorization_service
...
from pysparkgui.widgets import Button, CopyButton, TracebackOutput
...
def get_function_hint(hint_string, df_manager, **kwargs):
    ...
    hint_string = replace_code_placeholder(
        hint_string, old_df_name=df_manager.get_latest_df_name()
    )
    ...
    return CopyButton(
        ...
        style=None,
        css_classes=["pysparkgui-button-function-hint"],
...
class Embeddable(widgets.VBox):
    """
    Makes sure all our EDA widgets are displayed where they belong inside of pysparkgui.
        ...
        class EmbeddableClass(base_class):
            def init_embeddable(
                ...
                if loading is None:
                    loading = get_loading_widget()
                if border:
                    outlet.add_class("pysparkgui-border-1px-solid-grey")
...
def user_exposed_function(original_function):
    ...
    def new_function(df, *args, df_manager=None, parent_tabs=None, **kwargs):
        ...
        if return_tabs:
            from pysparkgui.viz import EmbeddableToTabviewable
...
def catch_empty_df(original_function):
    ...
    def new_function(*args, **kwargs):
        ...
        if df_manager is not None:
            row_count = df_manager.get_latest_df().shape[0]
            if row_count == 0:
                return widgets.HTML(
...
class ColumnsReducer(AsyncEmbeddable):
    ...
    def init_embeddable(
        ...
        **kwargs,
        ...
        self.content_outlet = widgets.VBox().add_class("pysparkgui-min-height-250px")
        ...
        show_label.add_class("pysparkgui-element-next-to-selectize")
        ...
        if self.show_update_button:
            ...
            show_line_contents.append(
                Button(
                    ...
                    css_classes=["pysparkgui-element-next-to-selectize"],
                    style=style,
                )
            )

        show_line = widgets.HBox(show_line_contents)
        show_line.add_class("pysparkgui-overflow-visible")
