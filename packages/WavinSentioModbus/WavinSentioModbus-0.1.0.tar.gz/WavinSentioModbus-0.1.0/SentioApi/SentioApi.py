...
class SentioModbus:
    ...
    def detectRooms(self):
        ...
        for roomCtr in range(Defaults.MaxNumberOfRooms):
            try:
                value = self.readRegister(SentioRegisterMap.RoomName, _subIndex = roomCtr)
                if value != -1:
    ...
    def setRoomSetpoint(self, roomIndex, setpointdouble):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                mode = self.readRegister(SentioRegisterMap.RoomModeOverride, _subIndex = roomIndex)
                if mode != SentioRoomModeSetting.RM_NONE:
                    self.writeRegister(SentioRegisterMap.RoomModeOverride, SentioRoomModeSetting.RM_NONE.value, _subIndex = roomIndex)

                setpoint_fp100 = int(setpointdouble * 100)
                logging.debug(" ---------- Set room {0} temp setpoint to {1}".format(roomIndex, setpoint_fp100))
                value = self.writeRegister(SentioRegisterMap.RoomTempSetpChange, setpoint_fp100, _subIndex = roomIndex)
                return value
    ...
    def getRoomSetpoint(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomTempSetpoint, _subIndex = roomIndex) / 100
                return value
        return -1
    ...
    def getRoomActualTemperature(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomAirTempActual, _subIndex = roomIndex)
                return value / 100
        return -1
    ...
    def getRoomRelativeHumidity(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomRelHumidity, _subIndex = roomIndex)
                return value / 100
        return -1
    ...
    def getRoomFloorTemperature(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomFloorTempActual, _subIndex = roomIndex)
                return value / 100
        return -1
    ...
    def getRoomCalculatedDewPoint(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomCalcDewPoint, _subIndex = roomIndex)
                return value / 100
        return -1
    ...
    def getRoomHeatingState(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomHeatingState, _subIndex = roomIndex)
    ...
    def getRoomMode(self, roomIndex):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                value = self.readRegister(SentioRegisterMap.RoomMode, _subIndex = roomIndex)
    ...
    def setRoomMode(self, roomIndex, roomMode:SentioRoomMode):
        for room in self.detectedRooms:
            if room.index == roomIndex:
                ...
                value = self.writeRegister(SentioRegisterMap.RoomMode, roomMode.value, _subIndex = roomIndex)
                return True
        logging.error("Failed to set room {0} to mode {1}".format(roomIndex, roomMode))
        return False

    def detectDHW(self):
        output = self.readRegister(SentioRegisterMap.DHWDeviceName)
        return output != -1 #if -1 is returned, it is not valid

    def detectCMV(self):
        output = self.readRegister(SentioRegisterMap.CMVDeviceName)
        return output != -1

    def detectDehumidifiers(self):
        returnValue=[]
        for i in range(4):
            returnValue.append((self.readRegister(SentioRegisterMap.DehumDeviceName, _subIndex=i) != -1))
        return returnValue
