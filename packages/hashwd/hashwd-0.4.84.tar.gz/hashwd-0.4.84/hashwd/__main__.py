...
from defaults import WORDS_DEFAULT, NUMBERS_DEFAULT, SYMBOLS_DEFAULT, SYMBOLS, MAX_SYMBOLS, MAX_NUMBERS, DICTIONARY_FILE
...
def main():
    ...
    if args.command == "defaults":
        ...
        if args.all:
            ...
            if new_dictionary_file:
                ...
                defaults_contents = re.sub(r"DICTIONARY_FILE = '.*?'", "DICTIONARY_FILE = '{}'".format(DICTIONARY_FILE),
                                           defaults_contents)
        else:
    ...
    num_words = defaults.WORDS_DEFAULT
    num_numbers = defaults.NUMBERS_DEFAULT
    num_symbols = defaults.SYMBOLS_DEFAULT

    # Use custom values instead of the default value if specified
    if args.words:
        num_words = args.words if args.words else WORDS_DEFAULT
    if args.numbers:
        num_numbers = args.numbers if args.numbers else NUMBERS_DEFAULT
    if args.symbols:
        num_symbols = args.symbols if args.symbols else SYMBOLS_DEFAULT
    ...
    if args.command == "generate":
        hashwd.generate_password(defaults.DICTIONARY_FILE, word_quantity=defaults.WORDS_DEFAULT,
                          number_quantity=defaults.NUMBERS_DEFAULT, symbol_quantity=defaults.SYMBOLS_DEFAULT)

        password = hashwd.generate_password(defaults.DICTIONARY_FILE, num_words, num_numbers, num_symbols)
