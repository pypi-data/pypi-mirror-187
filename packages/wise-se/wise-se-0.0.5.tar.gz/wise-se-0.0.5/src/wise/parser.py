...
class ImpEmitter(ImpLanguageListener):
    ...
    def exitBlt(self, ctx: ImpLanguageParser.BltContext):
        left = self.__aexprs[ctx.aexpr(0)]
        right = self.__aexprs[ctx.aexpr(1)]

        # a < b <==> a <= b and not a == b
        self.__bexprs[ctx] = Band(Ble(left, right), Bnot(Beq(left, right)))
    ...
    def exitBge(self, ctx: ImpLanguageParser.BgeContext):
        ...
        # a >= b <==> b <= a
        self.__bexprs[ctx] = Ble(right, left)
    ...
    def exitBgt(self, ctx: ImpLanguageParser.BgeContext):
        ...
        # a > b <==> b <= a and not a == b
        self.__bexprs[ctx] = Band(Ble(right, left), Bnot(Beq(left, right)))
    ...
    def exitAssert(self, ctx: ImpLanguageParser.AssertContext):
        self.__progs[ctx] = Assert(self.__bexprs[ctx.bexpr()])
