...
from servey.servey_starlette.action_endpoint.action_endpoint_abc import ActionEndpointABC
from servey.servey_starlette.action_endpoint.factory.action_endpoint_factory_abc import ActionEndpointFactoryABC
...
class WebPageActionEndpointFactory(ActionEndpointFactoryABC):
    ...
    def create(
        ...
        if len(triggers) > 1:
            raise ServeyError(
                f"multi_triggers_not_supported:{triggers}"
            )
        trigger = triggers[0]
        ...
        endpoint = WebPageActionEndpoint(
            ...
            result_marshaller=self.marshaller_context.get_marshaller(result_type) if result_type else None,
