...
def get_item_field(opts=None, item_id=None, field=None):
    ...
    if not validators.uuid(item_id):
        log.error('%s Value for "item_id" must be a UUID', LOG_PREFIX)
        return False
    if field not in ["username", "password", "uri", "totp", "notes", "exposed"]:
        log.error(
            '%s Value for "field" must be one of "%s", "%s", "%s", "%s", "%s", "%s"',
            LOG_PREFIX,
            "username",
            "password",
            "uri",
    ...
    field_results = {}
    field_ret = salt.utils.http.query(field_url, method="GET", header_dict=headers, decode=True)
    # Check status code for API call
    if "error" in field_ret:
        log.error(
        ...
        field_results = json.loads(field_ret["body"])
        if field_results.get("success"):
