...
:depends: pyhumps, pyotp, validators
...
def get(key, profile=None):
    ...
    if args[0] == "by-uuid":
        if args[2] in [
            ...
            elif args[2] == "totp_now":
                return bitwarden_general.get_totp(seed=item["login"]["totp"])
            elif args[2] == "notes":
                return item["notes"]
            ...
            log.error(
                'Supplied object "%s" is not one of: %s, %s, %s, %s, %s, %s, %s, %s, %s, %s.',
                args[2],
                "username",
                "password",
                "totp",
