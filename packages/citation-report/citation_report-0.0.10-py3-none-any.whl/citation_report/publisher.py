...
class PublisherStyle(BaseModel):
    """Each style will contain a `label` which is used
    in the final `Report` object; the `regex_exp`"""

    label: str = Field(
        ..., title="Report style", description="Use for uniformity"
    )
    description: str = Field(..., title="Full name of the repository")
    ...
    def pattern(self) -> Pattern:
        return re.compile(self.regex_exp, re.I | re.X)
