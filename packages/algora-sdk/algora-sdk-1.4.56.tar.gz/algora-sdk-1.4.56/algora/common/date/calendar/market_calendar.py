...
from pandas_market_calendars import (
    get_calendar,
    MarketCalendar as PandasMarketCalendar
)
...
class MarketCalendar:
    ...
    def schedule(
        ...
        """
        return self.calendar.schedule(start_date, end_date, tz, start, end, force_special_times, market_times)
    ...
    def is_business_day(self, as_of_date: date) -> bool:
        ...
        """
        schedule = self.calendar.schedule(start_date=as_of_date, end_date=as_of_date)
    ...
    def is_holiday(self, as_of_date: date) -> bool:
        ...
        is_adhoc_holiday = as_of_date in [ts.date() for ts in self.calendar.adhoc_holidays]
        if self.calendar.regular_holidays:
            is_general_holiday = any(self.calendar.regular_holidays.holidays().isin([as_of_date]))
    ...
    def is_weekend(self, as_of_date: date) -> bool:
        ...
        special_cases = not self.is_holiday(as_of_date) and not self.is_business_day(as_of_date)
    ...
    def end_of_month(self, as_of_date: date) -> date:
        ...
        df = self.calendar.schedule(first_day, last_day)
