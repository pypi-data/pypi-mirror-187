...
def called_with_args(f, args_list=None, kwargs_list=None):
    """
...
def called_once_with_args(f, args=None, kwargs=None):
    """
    A predicate to check if `f` was called with specific arguments exactly once.
    Note that `f` must be a spied function.
    """
    return called_with_args(f, [args], [kwargs])


def called_at_least_once_with_args(f, args=None, kwargs=None):
    """
