...
class KubeDataset(data.Dataset, ABC):
    ...
    def __load_data(self, minibatches: range, validation=False):
        ...
        for batch in batches:
            ...
            logging.debug(f"Loading data data type, {type(d)}")
            logging.debug(f"Loading data data shape, {d.shape}")
            logging.debug(f"Loading data labal type, {type(l)}")
            logging.debug(f"Loading data labal shape, {l.shape}")
