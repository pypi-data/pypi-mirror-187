    ...
    def copy_to_table(dbconnection, destination_table, tmp_uuid, sep):
        ...
        try:
            cursor.execute("COPY " + destination_table + " FROM 's3://qualifyze-temporals/df-batch-uploads/" + str(
                tmp_uuid) + ".csv' CREDENTIALS 'aws_access_key_id="+servicer_credentials['aws_access_key_id']+";aws_secret_access_key="+servicer_credentials['aws_secret_access_key']+"' DELIMITER '" + sep + "' IGNOREHEADER as 1 ACCEPTINVCHARS;")
