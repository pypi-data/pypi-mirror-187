...
def do_discovery(conn_config):
    ...
    LOGGER.info('do_discovery streams: %s', streams)
...
def do_sync(conn_config, catalog, default_replication_method, state, state_file=None):
    ...
    """
    LOGGER.info('Current streams catalog %s', catalog)
...
def main_impl():
    ...
    if args.discover:
        ...
        LOGGER.info('args.properties: %s', args.properties)
        LOGGER.info('args.catalog: %s', args.catalog)
        state = args.state
