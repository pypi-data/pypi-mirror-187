...
class WorkingSite(Site):
    ...
    with flow.group('AfterEffects settings'):
        ae_render_settings_templates = flow.HashParam().ui(
            label='Render settings templates')
        ae_output_module_templates   = flow.HashParam().ui(
            label='Output module templates')
        ae_output_module_audio       = flow.Param().ui(
            label='Audio output module template')
        ae_comp_name_patterns        = flow.OrderedStringSetParam().ui(
