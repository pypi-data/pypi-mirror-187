...
class CryptodotcomContext:
    ...
    close_position: Callable[[str, Literal['market', 'limit'], Optional[str]], Observable[int]]
    create_order: Callable[[str, str, str, float, Optional[float], Optional[Any]], Observable[dict[str, Any]]]
    exchange: cryptocom
    feed_messages: Observable[CryptodotcomResponseMessage]
    get_deposit_address: Callable[[], Observable[list[dict]]]
...
def cryptodotcom_sockets(
    add_token: Callable[[Observable[CryptodotcomResponseMessage]], Callable[[EnhancedWebsocket], Observable[EnhancedWebsocket]]],
    ...
    logger.info("Settings up emergency trigger")
    all_subscriptions = CompositeDisposable()
    # Set up sockets
    ...
    response_messages = private_messages.pipe(
        keep_response_messages_only(),
        share()
    )
    feed_messages = private_messages.pipe(
        exclude_response_messages(),
        share()
    )

    book_observables = {}
    for pair, depth in books:
        book_observables[f'{pair}_{depth}'] = public_sockets.pipe(
            keep_new_socket_only(), subscribe_order_book(pair, str(depth), public_messages),  # type: ignore
            share()
        )
    ...
    get_user_open_orders = get_user_open_orders_factory(response_messages, socket_bs, exchange)
    open_orders = guaranteed_socket.pipe(
        subscribe_open_orders(private_messages, exchange),
        share()
    )
    open_orders_reloaded = open_orders.pipe(
        flat_map(lambda _: get_user_open_orders())
    )

    context = CryptodotcomContext(
        all_subscriptions=all_subscriptions,
        books=book_observables,
        cancel_all=cancel_all_factory(response_messages, open_orders, exchange, socket_bs),
        ...
        get_deposit_address=get_deposit_address_factory(response_messages, socket_bs),
