...
to play sounds directly, and sounds can also be played even without
`simpleaudio` when running in a Jupyter notebook.
...
import math, random, struct, wave, io
...
def playTrack(wait=True):
    ...
    Always waits if run in a Jupyter notebook, but in that case it also
    doesn't depend on `simpleaudio`, which otherwise must be installed or
