...
class COINNLearner:
    ...
    def to_reduce(self):
        ...
        grads = np.array(_tu.extract_grads(self.trainer.nn[first_model], dtype=self.dtype))
        #sps_grads = sps.coo_matrix(grads)
        non_zero_grad_indices = np.nonzero(grads)
        values_nonzero = grads[np.nonzero(grads)]

        #Create sparse tensor
        i = np.array(non_zero_grad_indices)
        v = values_nonzero
        sps_grads = _torch.sparse_coo_tensor(i, v)
        _tu.save_arrays(
            self.state['transferDirectory'] + _sep + out['grads_file'],
            sps_grads
        )
        _tu.save_arrays(
            self.state['outputDirectory'] + _sep + "gradsfilesample.pt",
            sps_grads
        )
        out['reduce'] = True
        return it, out
