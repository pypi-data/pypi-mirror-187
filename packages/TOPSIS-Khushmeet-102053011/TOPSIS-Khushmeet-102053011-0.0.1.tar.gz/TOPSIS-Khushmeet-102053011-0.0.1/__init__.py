...
def rank_df(p, m):
    ...
    df = pd.DataFrame(p, index=index, columns=['Topsis Score'])
...
def main():
    ...
    parser.add_argument(
        "InputDataFile", help="Enter the name of CSV file with .csv extention", type=str)
    parser.add_argument(
        "Weights", nargs=1, help="Enter the weight vector comma separated", type=str)
    parser.add_argument(
        "Impacts", nargs=1, help="Enter the impact vector comma separated", type=str)
    ...
    try:
        weights = np.array(
            (args['Weights'][0].replace(" ", "")).split(','), dtype=float)
    ...
    Topsis_Score = performance(x, m, n, v_best, v_worst)

    ranked_df = rank_df(Topsis_Score, m)

    df = pd.read_csv(filename)

    ranked_df = pd.merge(left=df, right=ranked_df,
