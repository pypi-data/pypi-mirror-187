...
class Qlearner:
    ...
    def __init__( self , Q_approx , actions , M_I_state , gamma=0.9 , max_memory_size = None ):
        
        self._replay_memory = []
        self._max_mem_size = max_memory_size
    ...
    def add_to_memory( self , episode ):
        """
        Add episode to the instance's internal historical replay memory.  If memory exceeds maximum size, drop the oldest.
        """        
        self._replay_memory += episode
        if self._max_mem_size :
            if len( self._replay_memory ) > self._max_mem_size :
                self._replay_memory = self._replay_memory[ -self._max_mem_size: ]
