...
def cast_assignments(assignments: Optional[dict], casts: Optional[dict], splitter: str = "=") -> dict:
    """
    It tries to casts the assignments in `assignments` variable using `casts` variable map.
    ...
    for variable in assignments:
        if variable in casts.keys():
            try:
                cassignments[variable] = eval("{}({})".format(casts[variable], assignments[variable]))
            except:
                cassignments[variable] = assignments[variable]
                print("{} is not really a {} variable".format(variable, casts[variable]))
