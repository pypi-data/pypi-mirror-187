...
def pd_to_sql(df, table = None, db = None, server = None, schema = None, index = None, columns = None,
    ...
    Returns:
    --------
    The function returns the parameters of a calling function that can read the data back to the user!
    Just pass the output to pyg.decode and you can read the data back

    Example:
    ---------

    >>> from pyg import * 
    >>> server = 'DESKTOP-GOQ0NSM' 
    ...
    Example: bitemporal support.
    ----------------------------
    >>> self = sql_table(table = 'bitemporal_stock_prices', db = 'test_db', schema = 'dbo', server = server, create = True,
                         non_null = dict(country = str, stock = str,                 
                                         updated = datetime.datetime,  ## new field
                                         date = datetime.datetime, 
                                         price = float, 
                                         volume = int))

    >>> encoded = pd_to_sql(df = tsla, db = db, server = server, schema = schema, method = 'bitemporal',
                            table = 'bitemporal_stock_prices', country = 'US', stock = 'tsla', index = 'date')
