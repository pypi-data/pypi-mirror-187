...
__import__("ndm_road_segment_utils")
        ...
        if cc_index not in cc_set and len(cc_list[cc_index]) > 1:
            ...
            if "REGULAR_ROAD" in in_fows or "REGULAR_ROAD" in out_fows:
                ...
                for fow, road in zip(fows, road_ids):
                    if fow != "REGULAR_ROAD":
                        parsed_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[road])
...
def find_address_based_ap(lat, lng, street, segments, graph, interior_graph_reversed, cc_dict, cc_list, tree, cutoff=1000, num_nearest_sources=8, non_named_road_allowed=True, num_aptas_allowable=2, alternative_apta_distance_cap=10, shift_distance=2):
    ...
    if len(street_name_match_segment_ids) > 0:
        ...
        for s_id in street_name_match_segment_ids:
            wkt_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[s_id])
                ...
                for i, n in enumerate(path):
                    ...
                    if (i == 0):
                        ...
                        if "REGULAR_ROAD" in in_fows:
                            ...
                            in_fows = set(in_fows).intersection(ndm_road_segment_utils.INTERIOR_ROADS)
                        
                        for in_fow in in_fows:
                            for out_fow in set(out_fows).intersection(ndm_road_segment_utils.INTERIOR_ROADS):
                        ...
                        in_fows = graph.nodes[current_node_id].get('in_fow', [])
                        allowable_fows = set(ndm_road_segment_utils.INTERIOR_ROADS).union(set(["REGULAR_ROAD"]))
                        in_fows = set(in_fows).intersection(allowable_fows)
                        
                        # If the value below > 0, indicates a transition occurs at this node.
                        if len(set(in_fows).difference(set([fow_last_segment]))) > 0 and fow_last_segment not in set(ndm_road_segment_utils.NON_NAVIGABLE_ROADS):
                ...
                for i, n in enumerate(path):
                    ...
                    if (i == 0):
                        ...
                        if "REGULAR_ROAD" in out_fows:
                            ...
                            out_fows = set(out_fows).intersection(ndm_road_segment_utils.INTERIOR_ROADS)

                        for out_fow in out_fows:
                            for in_fow in set(in_fows).intersection(ndm_road_segment_utils.INTERIOR_ROADS):
                        ...
                        # If the value below > 0, indicates a transition occurs at this node.
                        if len(set(out_fows).difference(set([fow_last_segment]))) > 0 and fow_last_segment not in set(ndm_road_segment_utils.NON_NAVIGABLE_ROADS):
                            if "REGULAR_ROAD" in out_fows:
                                # There is a transition at this node indicating an incoming regular road and an outging interior road.
                                # We know the outgoing is interior, since it is the fow of the last segment crawled, and this crawler
                                # would not crawl non-interior roads.
...
def featurize_graph_crawl_aps(lat, lng, address, ap_data, segments, graph, tree, generated_ap_identifier):
    ...
    for i in range(len(ap_nodes)):
        ...
        for j in range(1, len(ap_segments_along_path_to_node[i])):
            ...
            prev_wkt_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[prev_seg])
            cur_wkt_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[cur_seg])
            pivot = None
            pos1 = None
            pos2 = None
        ...
        pct_incoming_edges_interior_node_ap = sum([1 if s in ndm_road_segment_utils.INTERIOR_ROADS else 0 for s in graph.nodes[ap_nodes[i]]['in_fow']])/\
                                              len(graph.nodes[ap_nodes[i]]['in_fow'])
        pct_incoming_edges_exterior_node_ap = 1 - pct_incoming_edges_interior_node_ap

        # What % of incoming edges at the node ap are interior vs. exterior
        pct_outgoing_edges_interior_node_ap = sum([1 if s in ndm_road_segment_utils.INTERIOR_ROADS else 0 for s in graph.nodes[ap_nodes[i]]['out_fow']])/\
...
def shift_aps(ap, segments, shift_distance = 10):
    ...
    wkt_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[shifted_ap_segment])
    ...
    incoming_wkt_coords = ndm_road_segment_utils.get_wkt_coords(segments.loc[incoming_ap_segment])
