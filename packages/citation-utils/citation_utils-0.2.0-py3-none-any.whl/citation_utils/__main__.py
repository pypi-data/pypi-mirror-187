...
from citation_docket.__main__ import extract_docket_from_data
...
class Citation(BaseModel):
    """
    A Philippine Supreme Court `Citation` is the end result of processing these
    two constructs:

    1. `Docket` - as defined in [citation-docket](https://github.com/justmars/citation-docket)
    2. `Report` - as defined in [citation-report](https://github.com/justmars/citation-report)

    Note that the fields contain a `col` and `index`. These are populated as extra
    Pydantic fields in in anticipation of [sqlpyd](https://github.com/justmars/sqlpyd).
    ...
    docket: str | None = Field(
        ...
        description=(
            "Cleaned docket based on the contents of the itemized docket parts:"
            " category, serial, and date."
        ),
        col=str,
        index=True,
    )
