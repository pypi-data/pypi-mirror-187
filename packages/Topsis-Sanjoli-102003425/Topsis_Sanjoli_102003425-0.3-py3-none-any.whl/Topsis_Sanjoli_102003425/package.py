...
def main():
    ...
    # File extension not xlsx
    elif ".xlsx" != (os.path.splitext(sys.argv[1]))[1]:
        print(f"ERROR : {sys.argv[1]} is not xlsx!!")
        exit(1)

    else:
        read_file = pd.read_excel (sys.argv[1])

        read_file.to_csv ("102003425-data.csv", 
                  index = None,
                  header=True)
        dataset, temp_dataset = pd.read_csv(
            "102003425-data.csv"), pd.read_csv("102003425-data.csv")
