...
def resize_3Dextended(data, size=(4, 1054, 1054)):
    ...
    for index, layer in enumerate(data):
        tmpdata[index] = cv2.resize(
            layer, dsize=(size[-2], size[-1]), interpolation=cv2.INTER_LINEAR
        )
    return tmpdata
...
def resize_4Dextended(data, size=(16, 4, 158, 158)):
    ...
    for index1, layer1 in enumerate(data):
        for index2, layer2 in enumerate(layer1):
            tmpdata[index1, index2] = cv2.resize(
                layer2, dsize=(size[-2], size[-1]), interpolation=cv2.INTER_NEAREST
            )
    return tmpdata
...
def hallucination_map(
    lowimg, hrimg, methods=["nearest", "linear", "cubic", "lanczos4"]
):
    clist = dict()

    if "nearest" in methods:
        clist["nearest"] = cv2.resize(
            lowimg, dsize=hrimg.shape, interpolation=cv2.INTER_NEAREST
        )
    if "linear" in methods:
        clist["linear"] = cv2.resize(
            lowimg, dsize=hrimg.shape, interpolation=cv2.INTER_LINEAR
        )
    if "cubic" in methods:
        clist["cubic"] = cv2.resize(
            lowimg, dsize=hrimg.shape, interpolation=cv2.INTER_CUBIC
        )
    if "lanczos4" in methods:
        clist["lanczos4"] = cv2.resize(
            lowimg, dsize=hrimg.shape, interpolation=cv2.INTER_LANCZOS4
        )
...
def downloadfile(url, path):
    """
    Download a file from a given url and save it in a given path.
    """
...
def dataset_generator(dataset_metadata: str, nimg: int, force: bool = False):
    """
    ...
    # Download the dataset - LR images if it is not in the path
    infile = dirpath / pathlib.Path(dataset_metadata.lr_url).name
    if not (infile.exists() or force):
        # Download the dataset
        print("Downloading LR images from %s." % dataset_metadata.name)
        downloadfile(dataset_metadata.lr_url, infile)

    # Download the dataset - HR images if it is not in the path
    targetfile = dirpath / pathlib.Path(dataset_metadata.hr_url).name
    if not (targetfile.exists() or force):
        # Download the dataset
        print("Downloading HR images from %s." % dataset_metadata.name)
        downloadfile(dataset_metadata.hr_url, targetfile)

    # Load the dataset
    X = np.memmap(infile, dtype=np.int16, mode="r", shape=dataset_metadata.lr_shape)
    y = np.memmap(targetfile, dtype=np.int16, mode="r", shape=dataset_metadata.hr_shape)

    # Filter by the number of images
    return X[:, 0:nimg, :, :, :], y


def metrics_generator(hr_layer, hr_layer_p, lr_layer, thresholds=[0.01, 0.02, 0.05]):
    # MAE
    ...
    for threshold in thresholds:
        mask = (np.abs(hallucination) > threshold) * 1.0
        mask[mask == 0] = np.nan
        # mask MAE
