...
def send_reads_with_pyguppy(client, input_path, batch_size=1):
    ...
    while num_reads_sent < read_count:
        ...
        new_called_reads, new_called_ids, new_num_reads_split = get_called_reads_from_pyguppy(client)
...
def basecall_with_pyguppy(client, input_path, save_file=None, batch_size=1):
    ...
    try:
        called_reads, called_ids, all_reads, num_reads_split = send_reads_with_pyguppy(client, input_path, batch_size)
        read_count = len(all_reads)
        num_reads_called = len(called_reads)

        result = client.finish(FINISH_TIMEOUT)
        if GuppyClient.success != result:
            raise Exception("Call to finish() method did not complete quickly enough. Return value is {}.".format(result))
