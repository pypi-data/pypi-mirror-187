...
def extend_orfs(p = None, as_df = False, fasta_path = None, cds_id = None, 
    ...
    # Check if all input ranges are divisible by 3
    p_df['length'] = p_df['End'] - p_df['Start'] 

    # Compute module 3
    p_df['module_3'] = p_df['length'] % 3

    # Those not divisible by 3
    ndv_df = p_df.loc[~p_df['module_3'].isin([0])]

    if len(ndv_df.index) > 0: 
        printerr("\nWARNING!\nSome input lenghts are not divisible by 3:\n",
             how='bright,yellow')
        printerr(ndv_df, how='bright,yellow')

    # Drop unnecessary columns
    p_df.drop(labels=['length','module_3'], inplace=True, axis=1)
