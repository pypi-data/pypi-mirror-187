...
@typechecked
def run_backend_validation_with_schema(
    objects: List[unified_common.BaseTectonObject],
) -> bool:
    # Dummy validation for now
    fco_args, _ = _get_declared_fco_args(objects)

    try:
        for args in fco_args:
            validation_request = _get_validation_request_for_fco_args(args)
        ...
        print(e)
        return False
    print("Vlidation succeeded!")
    return True


def _get_validation_request_for_fco_args(
    fco_args: fco_args_pb2.FcoArgs,
) -> metadata_service_pb2.ValidateLocalFcoRequest:
    if fco_args.HasField("entity"):
        return metadata_service_pb2.ValidateLocalFcoRequest(
            sdk_version=tecton_version.get_semantic_version() or "",
            validation_request=validator_pb2.ValidationRequest(
                entity_validation_request=validator_pb2.EntityValidationRequest(
                    args=fco_args.entity,
                ),
            ),
        )
    else:
        raise NotImplementedError("Unsupported fco type")
