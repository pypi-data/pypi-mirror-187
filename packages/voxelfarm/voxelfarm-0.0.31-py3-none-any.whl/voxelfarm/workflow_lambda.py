...
class request:
    def __init__(self, lambda_host):
        self.vf_api = voxelfarmclient.rest(lambda_host.get_vf_api)

        self.lambda_host = lambda_host

        self.scrap_folder = lambda_host.get_scrap_folder()          
        self.raw_entity_id = lambda_host.input_string('raw_entity_id', 'Entity Id', '')
        self.project_id = lambda_host.input_string('project_id', 'Project Id', '')
        self.product_id = lambda_host.input_string('product_id', 'Product Id', '')
        self.product_folder_id = lambda_host.input_string('product_folder_id', 'Product Folder', '')
        self.version_folder_id = lambda_host.input_string('version_folder_id', 'Version Folder', '')
        self.user = lambda_host.input_string('user', 'User', '')
        self.comment = lambda_host.input_string('comment', 'Comment', '')
        self.capture_date = lambda_host.input_string('capture_date', 'Capture Date', '')

        self.update_type = lambda_host.get_callback_update_type()
        self.caller_entity_id = lambda_host.get_callback_entity_id()
        self.caller_entity_state = lambda_host.get_callback_entity_state()

        self.crs = {}
        # Get the coordinate system of the project
        lambda_host.log('Retrieving project CRS...')
        result = self.vf_api.get_project_crs(self.project_id)
        if result.success:
            self.crs = result.crs
            lambda_host.log('Retrieved project CRS')

        product_entity = self.vf_api.get_entity(self.product_folder_id, self.project_id)
        self.lambda_host.log(f'product_entity:{product_entity}')
    ...
    def get_product_property(self, product_id, property):
        self.lambda_host.log(f'getting {product_id} property:{property}...')
        ...
        if workflow_entity and (field_id in workflow_entity):
            ...
            self.lambda_host.log(f'product_folder_id:{product_folder_id}')
            ...
            if product_entity:
                ...
                self.lambda_host.log(f'active_version:{active_version}')
                extended_prop = 'version_' + property
                self.lambda_host.log(f'extended_prop:{extended_prop}')
    ...
    def get_product_singleton(self, product_id, singleton_id):
        self.lambda_host.log(f'getting {singleton_id} for {product_id}...')
    ...
    def define_product_alias(self, alias, value):
        self.lambda_host.log(f'Define alias {alias} for product {self.product_id}')
        updatedProperties = {}
...
class workflow_lambda_host:
    ...
    def get_vf_api(self):
        return self.lambda_framework.get_vf_api()
    ...
    def set_workflow_definition(self, vf, workflow_definition):

        request = request(vf, self.lambda_framework)

        product_definition = self.find_product_definition(request.product_id, workflow_definition)
        if product_definition:
           
            if request.update_type:
                product_definition.on_stage_done(request)

                # Auto-activate version
                if not 'activated' in request.properties:
                    self.lambda_host.log('Updating product version_active folder...')
                    product_entity = self.vf_api.get_entity(self.product_folder_id, self.project_id)
                    ...
                    if product_entity and 'version_active' in product_entity:
                        ...
                        self.lambda_host.log(f'Previously active version: {previous_active_version}')
                        result = self.vf_api.update_entity(
                            project=self.project_id,
                            id=self.product_folder_id,
                            fields={
                                'version_active' : str(request.version_folder_id)
                            }
                        )

                        if not result.success:
                            return {'success': False, 'error_info': result.error_info}
                        request.properties['activated'] = '1'
                        self.lambda_host.log('Updated product active_version.')        
            else:    
                product_definition.on_receive_data(request)

            # write properties
            self.lambda_host.log('Saving properties...')
            updatedProperties = {}
            for prop in request.properties:
                version_property_id = 'version_' + prop
                updatedProperties[version_property_id] = request.properties[prop]
            self.vf_api.update_entity(
                project=request.project_id,
                id=request.version_folder_id,
                fields=updatedProperties  
            )                          
    ...
    def get_entity_file(self, vf : voxelfarmclient.rest, request : request, file_name:str, projectId:str, entityId:str):
            local_attr_file = os.path.join(request.scrap_folder, file_name)
    ...
    def publish_reports(self, vf : voxelfarmclient.rest, request : request, update_type : str, report_id:str, spatialId: str, objectId: str, propertyIds:tuple):
            self.lambda_framework.log(f'Publish reports calling timeseries')
            mesh_id = request.properties['mesh_id'] 
            ...
            if mesh_id != "":
                mesh_entity = vf.get_entity(mesh_id, request.project_id)
                ...
                report_entity = vf.get_entity(report_id, request.project_id)
                report_name = 'Report'
                ...
                result = vf.create_process_entity(
                    ...
                    fields={
                        'file_folder' : request.version_folder_id,
                        'code': 'publish-reports.py',
                        'workflow_callback': update_type,
                        'workflow_id': request.workflow_folder_id,
                        'workflow_product': request.product_id,
                    ...
                    crs=request.crs,
                    files=[os.path.join(script_dir, 'lambdas/publish-reports.py')],
                    callback=request.get_callback(update_type))
                if not result.success:
    ...
    def create_view(self, vf : voxelfarmclient.rest, request : request, name : str, view_type : str, view_lambda : str, inputs : dict, props:dict):
            self.lambda_framework.log(f'create_view:name:{name}|view_type:{view_type}|view_lambda:{view_lambda}|inputs:{inputs}|props:{props}')
            if view_type == None:
                script_dir = os.path.dirname(__file__)
                lambda_file = open(os.path.join(script_dir, view_lambda))
    ...
    def create_report(self, vf : voxelfarmclient.rest, request : request, update_type : str, name : str, report_lambda : str, region : str, lod : int, inputs : dict, fields : dict = None):
            script_dir = os.path.dirname(__file__)
            lambda_file = open(os.path.join(script_dir, report_lambda))
            result = vf.create_lambda_python(
                project=request.project_id, 
            ...
            fields['file_folder'] = request.version_folder_id
            fields['workflow_callback'] = update_type
            fields['workflow_id'] = request.workflow_folder_id
            fields['workflow_product'] = request.product_id
    ...
    def create_lambda(self, vf : voxelfarmclient.rest, request : request, name : str, type : str, report_lambda : str):
            script_dir = os.path.dirname(__file__)
            lambda_file = open(os.path.join(script_dir, report_lambda))
            result = vf.create_lambda_python(
                project=request.project_id, 
    ...
    def create_export(self, vf : voxelfarmclient.rest, request : request, update_type : str, name : str, report_lambda : str, region : str, lod : int, inputs : dict):
            script_dir = os.path.dirname(__file__)
            lambda_file = open(os.path.join(script_dir, report_lambda))
            result = vf.create_lambda_python(
                project=request.project_id, 
    ...
    def get_meta_attributes(self, vf : voxelfarmclient.rest, request : request, projectId:str):
            attribute_product = request.get_product_property('META_ATTR_BM', 'report_entity')
