...
class Flexi:
    '''link to documentation- https://github.com/suryavenom/Flexi/blob/main/README.md'''
    ...
    def d_exchange(self, no_of_exchanges=1,entanglement = True,return_type="no_update"):
        """
        ...
        :param entanglement: if True, it will exchange the dictionary even if it has identical values,
        ...
        :return: The exchanged dictionary is being returned.
        """
        if isinstance(no_of_exchanges, int) == False:
            raise Exception("type(no_of_exchanges) == int")
        if entanglement not in [True,False]:
            raise Exception("entanglement must be True or False")
        ...
        if no_of_exchanges % 2 != 0:
            ...
            if entanglement == True:
                exchanged_dictionary = self.dictionary.copy()
                new_object = Flexi(exchanged_dictionary)
                index_list = []
                for values in self.dictionary.values():
                    key_list = Flexi.d_get(self,values)
                    if len(key_list) > 1:
                        exchanged_dictionary.update({key_list:exchanged_dictionary.get(key_list[0])})
                        index_list = [new_object.d_index(keys) for keys in key_list]
                if return_type == "no_update":
                    new_exchanged_dictionary = {}
                    if len(index_list)!=0:
                        new_exchanged_dictionary = new_object.d_remove(index_list,return_type="no_update")
                        return {value: key for key,value in new_exchanged_dictionary.items()}
                    else:
                        return {value: key for key,value in exchanged_dictionary.items()}

                
                elif return_type == "update":
                    if len(index_list)!=0:

                        new_object.d_remove(index_list,return_type="update")
                    self.dictionary.clear()
                    self.dictionary.update({value: key for key,value in exchanged_dictionary.items()})
        else:
            if return_type == "update":
                pass
            if return_type == "no_update":
    ...
    def sort_values(self, return_type="no_update"):
        """
        It sorts the values of dictionary
    ...
    def d_remove(self, index_list: list,return_type = "no_update"):
        """
        It takes the dictionary, copies it, and then removes the key at the index
        
        :param index_list: list
        :type index_list: list
        :param return_type: It's either "no_update" or "update". If it's "no_update", it returns a new
        dictionary with the key removed. If it's "update", it updates the dictionary, defaults to no_update
        (optional)
        :return: A new dictionary with the key removed.
        """
        if return_type not in ["no_update","update"]:
            raise Exception("return_type should be either no_update or update")
    ...
    def swap_keys(self, initial_index: int, final_index: int, return_type="no_update"):
        """
        ...
        """
        if initial_index not in range(len(self.dictionary)) or final_index not in range(len(self.dictionary)):
            raise IndexError("invalid index")
        # checking the keywords
        if return_type not in ["no_update", "update"]:
            raise Exception("return_type should be either update or no_update")

        exchanged_list = [[key, value]for key, value in list(self.dictionary.items())]
        exchanged_list[initial_index][0], exchanged_list[final_index][0] = exchanged_list[final_index][0], exchanged_list[initial_index][0]
    ...
    def swap_values(self, initial_index: int, final_index: int, return_type="no_update"):
        """
        ...
        exchanged_list[initial_index][1], exchanged_list[final_index][1] = exchanged_list[final_index][1], exchanged_list[initial_index][1]
        exchanged_dictionary = dict(exchanged_list)
    ...
    def auto_arrange(self,index_list:list,return_type = "no_update"):
        ...
        if return_type not in ["no_update", "update"]:
            raise Exception("return_type should be either update or no_update")
        if all(type(ele) for ele in index_list) == False:
        ...
        if len(items_tuple) != 0:
            for items in items_tuple:
                if items_tuple.count(items) == 2:
                    if common_item_list.count(items) == 0:
                        common_item_list.append(items)
