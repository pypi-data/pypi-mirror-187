...
class Flexi:
    ...
    def d_remove(self, index_list: list):
        """
        ...
        new_object = Flexi(new_dictionary)
        index_list.sort()
        if index_list == list(range(index_list[-1]+1)):
            return new_object.d_split(index_list[-1]+1)[1]
...
start = perf_counter()
x = Flexi({"a":1, "b":2,"c":3,"d":4,"e":5,"f":6,"g":7})
print(x.d_replace(1,"j","k",return_type="update"))
stop = perf_counter()
print(stop - start)
