...
parser_modify_group.add_argument(
    ...
    default=str_with_index_list(
        default := "rename invert sum multiply resample groupby mean trend drop only"
    ),
...
""".strip()
    ...
    data = data.rename(columns={col: sanitize_text(col) for col in euro_cols})
    ...
    with plt.xkcd(
        ...
        ax.set_title(sanitize_text(args.axtitle or " ".join(hledger_args)))
        if args.groupby:
