...
class Highscores(OSRSBase):
    ...
    def __process_data(self):
        ...
        skills = fullList[:fullList.index(EndOfSkill)]
        minigames = fullList[fullList.index(EndOfSkill):fullList.index(StartOfBosses)]
        bosses = fullList[fullList.index(StartOfBosses):]
        count = 0
        for sk in skills:
            data = self.data[count].split(',')
            categories.Skill[sk.lower()] = Dict({
                'rank': data[0],
                'level': data[1],
                'xp': data[2],
        ...
        for mini in minigames:
            data = self.data[count].split(',')
            categories.MiniGame[mini.lower()] = Dict({
                'rank': data[0],
                'score': data[1]
            })
            count += 1
        for boss in bosses:
            data = self.data[count].split(',')
            categories.Boss[boss.lower()] = Dict({
                'rank': data[0],
                'kills': data[1]
            })
            count += 1
        ...
        self.skill = Dict(categories.Skill)
        self.minigame = Dict(categories.MiniGame)
        self.boss = Dict(categories.Boss)
