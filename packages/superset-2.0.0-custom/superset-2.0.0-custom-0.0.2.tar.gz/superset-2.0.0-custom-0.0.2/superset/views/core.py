...
class Superset(BaseSupersetView):  # pylint: disable=too-many-public-methods
    ...
    def generate_json(
        ...
        elif response_type == ChartDataResultFormat.XLSX:
            return XlsxResponse(
                viz_obj.get_xlsx(), headers=generate_download_headers("xlsx"),
            )

        elif response_type == ChartDataResultType.QUERY:
            return self.get_query_string_response(viz_obj)

        elif response_type == ChartDataResultType.RESULTS:
            return self.get_raw_results(viz_obj)

        elif response_type == ChartDataResultType.SAMPLES:
    ...
    def explore_json(
        ...
        if (
            ...
            or response_type == ChartDataResultFormat.XLSX)
    ...
    def import_dashboards(self) -> FlaskResponse:
        ...
        if request.method == "POST" and import_file:
            ...
            if success:
                flash(
                    _(
                        "Dashboard(s) have been imported"
                    ), 
                    "success"
                )
            ...
            payload.append(
                {
                    ...
                    "time": log.dttm,
                    "time_delta_humanized": naturaltime(
        ...
        payload = [
            {
                ...
                "url": o.slice_url,
                "dttm": o.changed_on,
                "changed_on_humanized": o.changed_on_humanized,
                "viz_type": o.viz_type,
            }
            for o in qry.all()
        ]
