...
def run_forecast_ensemble(
    ...
    # Add horizon column
    first_prediction_date = return_dataframe.index.min()
    return_dataframe["horizon"] = return_dataframe.index.apply(
        lambda x: relativedelta(first_prediction_date, x).years + 1
    )
