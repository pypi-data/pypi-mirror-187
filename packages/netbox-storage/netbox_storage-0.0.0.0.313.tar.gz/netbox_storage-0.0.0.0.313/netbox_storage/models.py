...
class Drive(NetBoxModel):
    ...
    def device_name(self):
        return f"/dev/sd{chr(ord('`') + str(self.identifier[-1]))}"
...
# In this example, a Reporter (vg) can be associated with many Article (pv) objects, but an Article (pv) can only have one Reporter (vg) object:
...
class PhysicalVolume(NetBoxModel):
    ...
    drive = models.ForeignKey(
        Drive,
        on_delete=models.CASCADE,
        related_name='drive_physicalvolume',
    )
    description = models.CharField(
        max_length=255,
        blank=True,
