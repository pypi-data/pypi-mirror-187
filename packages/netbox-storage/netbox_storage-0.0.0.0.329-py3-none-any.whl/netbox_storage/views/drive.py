...
from netbox_storage.filters import DriveFilter, PartitionFilter, PhysicalVolumeFilter
...
from netbox_storage.models import Drive, Partition, PhysicalVolume
from netbox_storage.tables import DriveTable, PartitionTable, PhysicalVolumeTable
...
@register_model_view(Drive, "drive_physicalvolumes_view")
class DrivePhysicalVolumeListView(generic.ObjectChildrenView):
    queryset = Drive.objects.all()
    child_model = PhysicalVolume
    table = PhysicalVolumeTable
    filterset = PhysicalVolumeFilter
    template_name = "netbox_storage/drive/physicalvolume.html"
    hide_if_empty = True

    tab = ViewTab(
        label="Physical Volumes",
        badge=lambda obj: obj.physicalvolumes_in_drive_count(),
        hide_if_empty=True,
    )

    def get_children(self, request, parent):
        return PhysicalVolume.objects.all()
