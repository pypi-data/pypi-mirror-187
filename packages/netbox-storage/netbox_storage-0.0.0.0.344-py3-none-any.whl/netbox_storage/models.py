...
class Drive(NetBoxModel):
    ...
    def left_free_space(self):
        return self.size - Partition.objects.filter(drive=self).aggregate(sum=Sum("size")).get("sum") or 0
...
class Partition(NetBoxModel):
    ...
    def clean(self, *args, **kwargs):
        ...
        diff_to_allocated_space = self.size.to_python() - current_partition_size
