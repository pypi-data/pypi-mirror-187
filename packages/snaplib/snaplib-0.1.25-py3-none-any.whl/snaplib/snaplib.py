...
class Snaplib:
    ...
    def features_selection_clsf(self, algorithms, df, target, metric, K_FOLDS, verbose=0):
        '''select bests features for modeling'''
        ...
        k_folds_dict_data = self.k_folds_split(df[features], target, K_FOLDS)
        score_with_all_features  = self.cross_val(algorithms, k_folds_dict_data, metric, task='clsf', cv=K_FOLDS, verbose=0)
        if verbose:
        ...
        while score_with_all_features <= cv_score_of_bad_feature:
            ...
            for without_feature in features:
                if without_feature != target:
                    ...
                    k_folds_dict_data = self.k_folds_split(df[fit_faetures], target, K_FOLDS)
                    score  = self.cross_val(algorithms, k_folds_dict_data, metric, task='clsf', cv=K_FOLDS, verbose=0)
