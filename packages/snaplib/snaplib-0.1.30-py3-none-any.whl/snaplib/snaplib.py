    ...
    def fit_stacked(self, 
        ...
        for alg in algorithms_list:
            ...
            if X_val is not None and y_val is not None and alg_name in ['LGB', 'XGB', 'Cat']:
                model = alg.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=10, verbose=verbose)
            else:
                model = alg.fit(X_train, y_train)
            return_trained_models.append(model)

        return return_trained_models
