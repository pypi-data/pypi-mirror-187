...
class ProgressBar:
    def __init__(self,stdscr,max_value: int, step_value=1,show_percent=True,show_log=False,message="Progress"):
        """Display a progress bar. Note that show_log currently has no effect. It will be added in a future update."""
        self.screen = stdscr
        self.max = max_value
...
def __test__(stdscr):
    ...
    rlist = [str(random.randint(1,999999)) for _ in range(random.randint(100,100000))]
    p = ProgressBar(stdscr,len(rlist),1,True,True,"Test")
    for i in rlist:
        p.step(str(i))
    p.done()
    del p
    displaymsg(stdscr,"Hi!")
