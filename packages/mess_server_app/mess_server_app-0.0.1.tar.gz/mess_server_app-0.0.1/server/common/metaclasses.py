...
class ServerMaker(type):
    ...
    def __init__(cls, clsname, bases, clsdict):
        ...
        attrs = []
        # перебираем ключи словаря атрибутов и методов
        for func in clsdict:
            try:
                # Возвращает итератор по всем инструкциям функции func
                ret = dis.get_instructions(clsdict[func])
                # ret - <generator object _get_instructions_bytes at 0x00000062EAEAD7C8>
                # Если не функция то ловим исключение и идем к следующей
            except TypeError:
                pass
            else:
                ...
                for i in ret:
                    ...
                    # opname - имя для операции, argval - имена функций
...
class ClientMaker(type):
    ...
    def __init__(cls, clsname, bases, clsdict):
        ...
        methods = []
        for func in clsdict:
            # Пробегаемся по каждой инструкции проверяем все выполняемые ей методы
            try:
                ret = dis.get_instructions(clsdict[func])
