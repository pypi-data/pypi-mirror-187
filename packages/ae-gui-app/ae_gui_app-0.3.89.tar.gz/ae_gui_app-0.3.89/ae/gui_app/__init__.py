    ...
    def change_flow(self, new_flow_id: str, **event_kwargs) -> bool:
        ...
        if action in ACTIONS_EXTENDING_FLOW_PATH:
            if action == 'edit' and self.flow_path_action() == 'edit' \
                ...
                self.vpo(f"{prefix}PATH EDIT: removed '{_flow_id}' and adding {new_flow_id} to {self.flow_path}")
            else:
            ...
            if not self.flow_path:
                self.vpo(f"{prefix}PATH REDUCE: cancelled flow change because of empty flow path")
                return False
