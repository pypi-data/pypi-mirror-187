...
def write_digraphs_list(G, output_prefix= 'tl_project', n_attrs= ['accession', 'species']):
    df= []
    for og, X in zip(G.index, G):
        df+= [ _get_edge_row(X, e, og, n_attrs) for e in X.out_edges ]
    df= pd.DataFrame(df, columns= ['OG'] + [mod+'_'+attr
                                            for mod in _quey_target
                                            for attr in n_attrs] #+ _quey_target
                    )
    df.to_csv(output_prefix, sep= '\t', index= False)
