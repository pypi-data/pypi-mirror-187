...
from lighter.constants import ROUTER_CONTRACT
from lighter.constants import FACTORY_CONTRACT
from lighter.constants import ORDERBOOK_CONTRACT
...
class Blockchain(object):
    ...
    def router_contract(self) -> Contract:
        contract_address = ROUTER_CONTRACT.get(
            self.id,
        )
        if contract_address is None:
            raise ValueError(
                "No router contract on blockchain {}".format(
                    self.id,
                )
            )
    ...
    def factory_contract(self) -> Contract:
        contract_address = FACTORY_CONTRACT.get(
            self.id,
        )
        if contract_address is None:
            raise ValueError(
                "No factory contract on blockchain {}".format(
                    self.id,
                )
            )
        contract_address = Web3.toChecksumAddress(contract_address)
    ...
    def orderbook_contract(self, orderbook_symbol: str) -> Contract:
        contract_address = ORDERBOOK_CONTRACT.get(self.id, {}).get(orderbook_symbol)
        if contract_address is None:
            raise ValueError(
    ...
    def _get_factory_contract(self) -> Contract:
        contract_address = FACTORY_CONTRACT.get(
            self.id,
        )
        if contract_address is None:
