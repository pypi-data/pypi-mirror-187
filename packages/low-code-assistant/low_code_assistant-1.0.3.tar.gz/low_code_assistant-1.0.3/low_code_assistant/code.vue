<template>
  <div :class="['solara-code-highlight', 'solara-code-highlight__box', no_controls && 'solara-code-highlight--no_controls' ]">
    <div v-for="(chunk, index) in highlighted_chunks" class="low_code_assistant-code__chunk">
      ...
      <div v-if="!no_controls" :class="index < highlighted_chunks.length - 1 ? 'low_code_assistant-code__controls' : ''">
...
<style id="low_code_assistant-code">
.low_code_assistant-code__chunk {
  position: relative;
  display: flex;
  flex-direction: row;
}

.low_code_assistant-code__controls {
  position: absolute;
  right: -7px;
  top: -7px;
  opacity: 0;
...
:not(.solara-code-highlight--no_controls) .low_code_assistant-code__chunk:hover {
  border-bottom: 1px solid grey;
}

.low_code_assistant-code__chunk:hover .low_code_assistant-code__controls {
  opacity: 1;
  z-index: 1;
}
