...
from low_code_assistant.serialize import dumps, loads
from low_code_assistant.settings import settings
from low_code_assistant.util import get_vars
from low_code_assistant.viz.state import Viz
...
def AppPanel(edit_data, set_code, open, edit, on_close, markdown_cells: Optional[Dict] = None, overwrite_warning=None):
    layout, on_layout = sol.use_state(open and edit and edit_data and edit_data["type_card_grid_layout"] or [])
...
def AssistantMain():
    ...
    def sync_notebook_browser_path():
        if notebook_path:
            low_code_assistant.assistant.notebook.notebook_browser_path.value = notebook_path
