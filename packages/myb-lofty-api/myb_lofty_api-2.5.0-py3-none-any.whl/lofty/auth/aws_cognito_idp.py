# Copied from https://github.com/capless/warrant
import base64
import binascii
import datetime
import hashlib
...
_n_hex = 'FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1' + '29024E088A67CC74020BBEA63B139B22514A08798E3404DD' + \
...
def _hex_to_long(hex_string):
    return int(hex_string, 16)


def _long_to_hex(long_num):
    return '%x' % long_num


def _get_random(nbytes):
    random_hex = binascii.hexlify(os.urandom(nbytes))
    return _hex_to_long(random_hex)
...
def _pad_hex(long_int):
    ...
    if not isinstance(long_int, six.string_types):
        hash_str = _long_to_hex(long_int)
    else:
        hash_str = long_int
    if len(hash_str) % 2 == 1:
...
def _compute_hkdf(ikm, salt):
    ...
    info_bits_update = _info_bits + bytearray(chr(1), 'utf-8')
...
class AwsCognitoIdp(object):

    NEW_PASSWORD_REQUIRED_CHALLENGE = 'NEW_PASSWORD_REQUIRED'
    PASSWORD_VERIFIER_CHALLENGE = 'PASSWORD_VERIFIER'

    def __init__(self, pool_id, client_id, pool_region=None,
    ...
    def _generate_random_small_a(self):
        ...
        """
        random_long_int = _get_random(128)
    ...
    def get_password_authentication_key(self, username, password, server_b_value, salt):
        ...
        """
        u_value = _calculate_u(self.large_a_value, server_b_value)
        if u_value == 0:
        ...
        x_value = _hex_to_long(_hex_hash(_pad_hex(salt) + username_password_hash))
        g_mod_pow_xn = pow(self.g, x_value, self.big_n)
        int_value2 = server_b_value - self.k * g_mod_pow_xn
        s_value = pow(int_value2, self.small_a_value + u_value * x_value, self.big_n)
        hkdf = _compute_hkdf(bytearray.fromhex(_pad_hex(s_value)),
                             bytearray.fromhex(_pad_hex(_long_to_hex(u_value))))
        return hkdf

    def _get_auth_params(self, username: str):
        auth_params = {'USERNAME': username,
                       'SRP_A': _long_to_hex(self.large_a_value)}
        if self.client_secret is not None:
            auth_params.update({
                "SECRET_HASH":
                self._get_secret_hash(username, self.client_id, self.client_secret)})
        return auth_params
    ...
    def _process_challenge(self, password: str, challenge_parameters):
        ...
        if self.client_secret is not None:
            response.update({
                "SECRET_HASH":
                self._get_secret_hash(self.username, self.client_id, self.client_secret)})
        return response

    def authenticate_user(self, username: str, password: str, client=None):
        boto_client = self.client or client
        auth_params = self._get_auth_params(username)
        ...
        if response['ChallengeName'] == self.PASSWORD_VERIFIER_CHALLENGE:
            challenge_response = self._process_challenge(password, response['ChallengeParameters'])
            auth_tokens = boto_client.respond_to_auth_challenge(
                ClientId=self.client_id,
                ChallengeName=self.PASSWORD_VERIFIER_CHALLENGE,
                ChallengeResponses=challenge_response
            )

            if auth_tokens.get('ChallengeName') == self.NEW_PASSWORD_REQUIRED_CHALLENGE:
                raise Exception('Change password before authenticating')

            return auth_tokens.get('AuthenticationResult')
        else:
            raise NotImplementedError('The %s challenge is not supported' % response['ChallengeName'])

    def set_new_password_challenge(self, username: str, old_password: str, new_password: str, client=None):
        boto_client = self.client or client
        auth_params = self._get_auth_params(username)
        response = boto_client.initiate_auth(
            AuthFlow='USER_SRP_AUTH',
        ...
        if response['ChallengeName'] == self.PASSWORD_VERIFIER_CHALLENGE:
            challenge_response = self._process_challenge(old_password, response['ChallengeParameters'])
