...
fn simple_potentiation_short_term_stdp() {
    ...
    assert_approx_eq!(
        f32,
        tick_2_result.state_snapshot.unwrap().synapse_states[0].short_term_stdp_offset,
        0.1 * (-2.0f32).exp()
    );
...
#[test]
fn hyper_spikes_long_term_stdp() {
    let mut params = InstanceParams::default();
    ...
    let mut conn_params = LayerConnectionParams::defaults_for_layer_ids(0, 1);
    let mut stdp_params = STDP_PARAMS.clone();
    stdp_params.use_hyper_spikes = true;
    conn_params.projection_params.long_term_stdp_params = Some(stdp_params);
    conn_params.conduction_delay_position_distance_scale_factor = 10.0;
    conn_params.projection_params.synapse_params.max_weight = 0.5;
    conn_params.initial_syn_weight = InitialSynWeight::Constant(0.25);
    ...
    let mut tick_input = TickInput::new();
    tick_input.force_spiking_nids.push(2);
    tick_input.extract_state_snapshot = true;

    let synapse_states = instance
        .tick(&tick_input)
        .unwrap()
        .state_snapshot
        .unwrap()
    ...
    // synapse 0 -> 2
    assert_approx_eq!(f32, synapse_states[0].weight, 0.25);
    assert_approx_eq!(f32, synapse_states[0].short_term_stdp_offset, 0.0);

    // synapse 0 -> 3
    assert_approx_eq!(f32, synapse_states[1].weight, 0.35);
    assert_approx_eq!(f32, synapse_states[1].short_term_stdp_offset, 0.0);

    // synapse 1 -> 2
    assert_approx_eq!(f32, synapse_states[2].weight, 0.25);
    assert_approx_eq!(f32, synapse_states[2].short_term_stdp_offset, 0.0);

    // synapse 1 -> 3
    assert_approx_eq!(f32, synapse_states[3].weight, 0.35);
    assert_approx_eq!(f32, synapse_states[3].short_term_stdp_offset, 0.0);
}

// #[test]
// fn hyper_spikes_short_term_stdp() {
//     let mut params = InstanceParams::default();
...
//     let mut stdp_params = STDP_PARAMS.clone();
//     stdp_params.use_hyper_spikes = true;
//     conn_params.projection_params.short_term_stdp_params = Some(ShortTermStdpParams {
//         stdp_params,
//         tau: 10.0,
//     });
//     conn_params.conduction_delay_position_distance_scale_factor = 10.0;
...
//     let expected_short_term_stdp_offset = 0.1 * (-1.0f32).exp();
...
//     // synapse 0 -> 3
//     assert_approx_eq!(f32, synapse_states[1].weight, 0.25);
//     assert_approx_eq!(
//         f32,
//         synapse_states[1].short_term_stdp_offset,
//         expected_short_term_stdp_offset
//     );
...
//     // synapse 1 -> 3
//     assert_approx_eq!(f32, synapse_states[3].weight, 0.25);
//     assert_approx_eq!(
//         f32,
//         synapse_states[3].short_term_stdp_offset,
//         expected_short_term_stdp_offset
//     );
// }
