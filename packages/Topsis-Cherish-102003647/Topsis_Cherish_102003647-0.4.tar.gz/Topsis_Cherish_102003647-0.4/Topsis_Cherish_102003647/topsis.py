...
def topsis(A,weight,imp) :
    ...
    for i in range(1,(a[1]+1)):       
        if(impact[i-1]=='+'):
            df.at[a[0],i]=df.iloc[:(a[0]),i].max()
            df.iloc[a[0],i]=df.iloc[:(a[0]),i].max()
            df.at[(a[0]+1),i]=df.iloc[:(a[0]),i].min()
            df.iloc[(a[0]+1),i]=df.iloc[:(a[0]),i].min()
        elif(impact[i-1]=='-'):
            df.at[a[0],i]=df.iloc[:(a[0]),i].min()
            df.iloc[a[0],i]=df.iloc[:(a[0]),i].min()
            df.at[(a[0]+1),i]=df.iloc[:(a[0]),i].max()
            df.iloc[(a[0]+1),i]=df.iloc[:(a[0]),i].max()
        else:
