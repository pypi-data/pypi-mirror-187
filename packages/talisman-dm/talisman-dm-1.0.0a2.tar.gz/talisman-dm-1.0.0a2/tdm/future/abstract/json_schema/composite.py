...
class ModelsGenerator(Generic[_DATA]):
    ...
    def generate_union_model(self) -> Tuple[Dict[Type[_DATA], Type[ElementModel[_DATA]]], Callable[[_DATA], ElementModel[_DATA]]]:
        if self._finalized:
            raise ValueError
        self._finalized = True
        ...
        for cls, clss in self._cls_hierarchy.items():
            if len(clss) == 1:
                ...
                result[cls] = Union[tuple(map(lambda cls_: self._cls2label_model[cls_][1], clss))]
