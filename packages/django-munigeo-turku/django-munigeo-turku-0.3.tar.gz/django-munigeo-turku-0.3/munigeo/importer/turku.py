...
# NOTE Turku region uses EPSG:3877 & ETRS-Gk23 coordinate systems.
GK23_SRID = 3877
GK23_SRS = SpatialReference(GK23_SRID)

PROJECTION_SRS = SpatialReference(PROJECTION_SRID)
WEB_MERCATOR_SRS = SpatialReference(3877)
...
class TurkuImporter(Importer):
    ...
    def _import_one_division_type(self, muni, div):
        ...
        # Cache the list of possible parents. Assumes parents are imported first.
        if 'parent' in div:
        ...
        if 'file' in div:
            ...
            if '?' in wfs_url:
                ...
                sep = '?'
            url = wfs_url + sep + 'typeName=' + div['wfs_layer'] + '&' + \
                "srsName=EPSG:%d" % PROJECTION_SRID + '&' + "outputFormat=application/json"
            ds = DataSource(url)
        lyr = ds[0]
        assert len(ds) == 1
    ...
    def import_addresses(self):
        addressFilePath = r'/home/linkedevents/codepoint/linkedevents/turku_address_fi_sv_001.csv'
        self.logger.info("Loading master data from CSV datasource")
        ds = DataSource(addressFilePath, encoding='iso-8859-1')  # encoding='utf-8'
        lyr = ds[0]
        assert len(ds) == 1

        muni_names = ('Turku', 'Kaarina', 'Aura', 'Lieto', 'Marttila', 'Masku', 'Mynämäki',
        ...
        for feat in lyr:
            ...
            if muni_name in muni_names:
                ...
                if not num:
                    ...
                    if num == '0':
                        self.logger.debug(
                            "Rejecting {}, due to {} not being valid street number".format(street_name, num))
                        continue
                letter = ""
                num2 = ""
                ...
                location = convert_from_gk23(coord_n, coord_e)
    ...
    def import_pois(self):
        URL_BASE = 'https://palvelukartta.turku.fi/api/v2/unit/?format=json'  # This is not a usable on right now.
        ...
        for srv_id in list(SERVICE_CATEGORY_MAP.keys()):
            ...
            ret = requests.get(URL_BASE % srv_id)  # NOTE! The service number is not in use.
