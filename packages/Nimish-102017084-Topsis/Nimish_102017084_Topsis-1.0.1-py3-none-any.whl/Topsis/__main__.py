...
def main():
    if len(sys.argv) != 5:
        ...
        print(f"ERROR : {sys.argv[1]} Dosn't exist!!")
        exit(1)
        ...
        try:
            ...
            print("ERROR : In weights  check again")
            exit(1)
        impact = sys.argv[3].split(',')
        for i in impact:
            if not (i == '+' or i == '-'):
                print("ERROR : In impact check again")
                exit(1)

        if nCol != len(weights) + 1 or nCol != len(impact) + 1:
            print(
                "ERROR : Number of weights, impacts and columns not same")
            exit(1)

        if (".csv" != (os.path.splitext(sys.argv[4]))[1]):
            print("ERROR : Output file extension  wrong")
            exit(1)
...
def topsis_pipy(temp_dataset, nCol, weights, impact):
    ...
    temp_dataset['Score'] = score

    temp_dataset['Rank'] = (temp_dataset['Score'].rank(method='max', ascending=False))
