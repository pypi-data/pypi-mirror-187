...
class AssertionModel:
    ...
    def add_bidirectional_lstm(self, n_hidden=30, num_layers=3):
        with tf.device(self._device):
            ...
            for layer_num in range(1, num_layers + 1):
                ...
                lstm_fw_cell = BasicLSTMCell(n_hidden, forget_bias=1.0, name='fw' + str(layer_num))
                lstm_fw_cell = DropoutWrapper(lstm_fw_cell, output_keep_prob=self.output_keep_prob)

                # Backward direction cell
                lstm_bw_cell = BasicLSTMCell(n_hidden, forget_bias=1.0, name='bw' + str(layer_num))
                lstm_bw_cell = DropoutWrapper(lstm_bw_cell, output_keep_prob=self.output_keep_prob)
            ...
            # calculation.
            outputs, _, _ = \
                stack_bidirectional_dynamic_rnn(fw_cells, bw_cells,
            ...
            with tf.compat.v1.variable_scope("context_repr"):
                tf.identity(self.bi_lstm, "scores")
    ...
    def add_optimizer(self):
        with tf.device(self._device):
            with tf.variable_scope("training"):
                ...
                cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred, labels=self.y),
                                      name="loss")
            ...
            for epoch in range(1, epochs + 1):
                for _ in range(1, num_batches + 1):
                    ...
                    self.sess.run(self.optimizer, feed_dict={self.x: batch_x, self.y: batch_y,
                                                             ...
                                                             self.output_keep_prob: 1 - dropout,
                                                             self.rate: rate})
                if epoch > epoch_acc:
    ...
    def calc_accuracy(self, dataset, batch_size):
        ...
        with tf.device(self._device):
            ...
            for _ in range(1, n_test_batches + 1):
                batch_x, batch_y, batch_seqlen = dataset.next(batch_size)
                global_matches += self.sess.run(self.match_count,
                                                feed_dict={self.x: batch_x, self.y: batch_y, self.seqlen: batch_seqlen})
    ...
    def confusion_matrix(self, dataset, sess, batch_size):
        with tf.device(self._device):
            ...
            for _ in range(1, n_test_batches + 1):
                batch_x, _, batch_seqlen = dataset.next(batch_size)
    ...
    def persist_graph(self, model_location, model_filename):
        # Add ops to save and restore all the variables (not used here but we need it in the graph).
        with tf.device(self._device):
            tf.train.Saver()
            tf.io.write_graph(self.sess.graph, model_location, model_filename, False)
            self.sess.close()
