...
def readAndValidateArguments():
    ...
    if weights.__contains__(',') == False:
        print("seperate weights by by comma")
        exit(0)
    ...
    if resultFilePath.__contains__('.csv') == False:
        ...
        exit(0)
    return inputFilePath, weights, criteria, resultFilePath, df
...
def main():
    inputFilePath, weightVector, criteriaVector, resultFilePath = sys.argv[1:]
    weightVector = [float(x) for x in weightVector.split(',')]
    criteriaVector = criteriaVector.split(',')
