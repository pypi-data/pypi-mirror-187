...
class AnnotationNode:
    ...
    def __init__(self, annotation_type, id_, meta: 'Meta'):
        ...
        self.namespace = copy.deepcopy(meta)
...
class Variable(Definition):
    ...
    def __init__(self, var: mypy.nodes.Var, meta: 'Meta'):
        ...
        if var.type is None:
            self.type = get_annotation(any_type_instance, meta)
        else:
            self.type = get_annotation(var.type, meta)
...
class ClassDef(Definition):
    ...
    def __init__(self, type_info: mypy.nodes.TypeInfo, meta: 'Meta'):
        super().__init__(self.kind)
        self.type: Annotation = get_annotation(mypy.types.Instance(type_info, []), meta)
...
class FuncDef(Definition):
    ...
    def __init__(self, func_def: mypy.nodes.FuncDef, meta: 'Meta'):
        ...
        for x in func_def.arguments:
            defn = get_definition_from_node(x.variable, meta)
            assert defn is not None
            self.args.append(defn)

        if func_def.type is None:
            node = FunctionNode(str(id(func_def)), meta, func_def)
            self.type = Annotation(node.id_)
        else:
            self.type = get_annotation(func_def.type, meta)
...
class OverloadedFuncDef(Definition):
    ...
    def __init__(self, func_def: mypy.nodes.OverloadedFuncDef, meta: 'Meta'):
        ...
        for x in func_def.items:
            cur = get_definition_from_node(x, meta)
...
class TypeVarNode(AnnotationNode):
    ...
    def __init__(self, type_var: mypy.types.TypeVarType, id_: str, namespace: 'Meta'):
        super().__init__(self.annotation_type, id_, namespace)
...
class FunctionNode(AnnotationNode):
    ...
    def __init__(self, id_: str, meta: 'Meta', type: tp.Union[mypy.types.CallableType, mypy.nodes.FuncItem]):
        ...
        if isinstance(type, mypy.types.CallableType):
            ...
            self.arg_types = [get_annotation(x, meta=self.namespace) for x in type.arg_types]
            self.return_type = get_annotation(type.ret_type, self.namespace)
            ...
            self.type_vars = []
            self.arg_types = [get_annotation(any_type_instance, meta=self.namespace) for _ in type.arguments]
...
class CompositeAnnotationNode(AnnotationNode):
    ...
    def __init__(self, annotation_type: str, symbol_node: mypy.nodes.TypeInfo, id_, namespace: 'Meta'):
        ...
        for name in symbol_node.names.keys():
            ...
            definition = get_definition_from_symbol_node(inner_symbol_node, self.namespace)
...
class ConcreteAnnotationNode(CompositeAnnotationNode):
    ...
    def __init__(self, symbol_node: mypy.nodes.TypeInfo, id_, namespace: 'Meta'):
        ...
        super().__init__(self.annotation_type, symbol_node, id_, namespace)
