...
def update_q_asset_inventory_table(
...
def update_q_asset_inventory_software_assetid_table(
        asset_item_dict: dict,
        sqlite_obj: etld_lib_sqlite_tables.SqliteObj,
        counter_obj):
...
def update_q_asset_inventory_software_os_unique_table(
        asset_item_dict: dict,
        sqlite_obj: etld_lib_sqlite_tables.SqliteObj,
        counter_obj):
...
def insert_one_asset_into_tables(asset_item, sqlite_obj, counter_objects):
    item = transform_epoch_dates(asset_item)
    update_q_asset_inventory_software_os_unique_table(
        asset_item_dict=item,
        sqlite_obj=sqlite_obj,
        counter_obj=counter_objects['counter_obj_software_os'])
    update_q_asset_inventory_software_assetid_table(
        asset_item_dict=item,
        sqlite_obj=sqlite_obj,
        counter_obj=counter_objects['counter_obj_software_assetid'])
    update_q_asset_inventory_table(
        sqlite_obj=sqlite_obj,
        table_name=etld_lib_config.asset_inventory_table_name,
        ...
        for json_file in json_file_list:
            with open(str(json_file), "r", encoding='utf-8') as read_file:
                ...
                for item in all_items['assetListData']['asset']:
                    insert_one_asset_into_tables(item, sqlite_asset_inventory, counter_objects)
