...
#
#
# Copyright 2017-2023 Simon Kennedy, G0FCU, g0fcu at g0fcu.com
...
class startControl(QThread):
    ...
    def run(self):    
        ...
        if gqrx_freq.decode("utf-8").find('\n') != -1:
            #print('1111', gqrx_freq.decode("utf-8"))
            #gqrx_freq = gqrx_freq[0:gqrx_freq.find('\n')]
            gqrx_freq = gqrx_freq.decode("utf-8")[0:gqrx_freq.decode("utf-8").find('\n')]
            #print('2222', gqrx_freq)
        else:
            print('ERROR')
        ...
        while self.control > 0:       
            ...
            if self.control == 1 or self.control == 3:
                ...
                if endpoint == 'hamlib':
                    #print('rig freq', rig_freq)
                    if rig_freq.decode("utf-8").find('\n') != -1:
                        rig_freq = rig_freq.decode("utf-8")[0:rig_freq.decode("utf-8").find('\n')]
                        #print('rig freq', rig_freq)
                else:
                    if rig_freq.decode("utf-8").find('.') != -1:
                        rig_freq = rig_freq.decode("utf-8")[0:rig_freq.decode("utf-8").find('.')]
                #print('rig freq', rig_freq)
...
class gqrxHamlib(QtWidgets.QMainWindow, gqrxHamlibGUI.Ui_MainWindow):      
    ...
    def readConfig(self):
        ...
        try:
            ...
            cf = open(homedir + '/.config/.gqrxHamlib.config', 'r')
        configLine = cf.read()
        # handle upgrade from 2.5 to 2.6 where there are two additional parameters for IF panadaptor functionality
        # and from 3.1 to 3.2 where the fldigi stuff was removed
