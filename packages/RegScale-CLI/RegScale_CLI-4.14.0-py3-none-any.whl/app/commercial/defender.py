...
def assign_severity(score) -> str:
    """
    Function to return RegScale severity string, used for payload to create new issue via RegScale API
    :param score: Score of the recommendation from Microsoft Defender
    :return: String for RegScale issue severity
    :rtype: str
    """
...
def format_description(rec: dict, tenant_id: str) -> str:
    ...
    # iterate through recommendation keys and uncamelcase them
...
def change_issue_status(
    ...
    if rec is not None and rec_type == "defender365":
        ...
        issue["severityLevel"] = (
            assign_severity(determine_alert_score(rec["properties"]["severity"])),
        )
        issue["issueOwnerId"] = config["userId"]
        issue["dueDate"] = get_due_date(
            score=determine_alert_score(rec["properties"]["severity"]),
            config=config,
            key="defenderCloud",
        )
    ...
    for i in range(len(threads)):
        ...
        if not alert.created:
            ...
            issue = Issue(
                ...
                dateCreated=get_current_datetime("%Y-%m-%dT%H:%M:%S"),
                description=description,
                severityLevel=assign_severity(
                    determine_alert_score(alert.data["properties"]["severity"])
                ),
                issueOwnerId=config["userId"],
                dueDate=get_due_date(
                    score=determine_alert_score(alert.data["properties"]["severity"]),
                    config=config,
                    key="defenderCloud",
                ),
                identification="Vulnerability Assessment",
...
def determine_alert_score(severity: str) -> int:
    """
    Function to convert the alert severity from Microsoft Defender for Cloud to a numerical value
    :param str severity: The alert severity from Microsoft Defender for Cloud
    :return: Numerical value of severity
    :rtype: int
    """
    score = 0
